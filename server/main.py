...
async def feature_flags():
    from lib.features import current_flags
...
async def transition_mlflow_model(
    model_name: str,
    version: str,
    stage: str,
...
async def startup_event():
    ...
    if settings.DATABASE_ENABLED:
        try:
            ...
            if settings.ENVIRONMENT.lower() == "production":
                logger.error(f"Falha ao inicializar banco de dados em produção: {e}")
                raise
    ...
    set_cache_instance(smart_cache)
    if settings.REDIS_ENABLED:
        try:
            from lib.redis_cache import initialize_cache

            await initialize_cache(settings.REDIS_URL)
            logger.info("✓ Redis cache initialized")
        except Exception as e:
            logger.warning(f"⚠ Redis cache unavailable, using in-memory cache: {e}")
...
async def shutdown_event():
    ...
    if settings.REDIS_ENABLED:
        try:
            ...
            logger.warning(f"Redis shutdown: {exc}")
