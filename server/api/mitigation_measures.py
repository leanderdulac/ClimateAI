...
async def calculate_mitigation_score_endpoint(
    drainage_capacity: float = Query(0.1, ge=0, description="Drainage capacity in m³/s per m² of area"),
    area_drained: float = Query(10000, gt=0, description="Total area served by drainage system (m²)"),
    structural_resistance_class: float = Query(5.0, ge=1, le=10, description="Wind/flood structural resistance class (1-10)"),
    iot_sensors_count: int = Query(5, ge=0, description="Number of IoT sensors and monitoring devices"),
    local_ndvi: float = Query(0.4, ge=0, le=1, description="Normalized Difference Vegetation Index (0-1 scale)"),
    refuge_distance_km: float = Query(2.0, ge=0, description="Distance to nearest refuge in km"),
    effectiveness_rating: float = Query(0.7, ge=0, le=1, description="Overall effectiveness rating (0-1 scale)"),
    implementation_date: str = Query(None, description="Implementation date (YYYY-MM-DD)"),
    ...
    try:
        ...
        if structural_resistance_class < 1 or structural_resistance_class > 10:
            raise HTTPException(status_code=400, detail="Structural resistance class must be between 1 and 10")
        ...
        raise HTTPException(status_code=500, detail=f"Mitigation score calculation failed: {str(e)}")
...
async def calculate_final_scr_score_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Final SCR score calculation failed: {str(e)}")
...
async def portfolio_evaluation_endpoint(
    ...
    refuge_distance_km: float = Query(2.0, ge=0, description="Distance to nearest refuge in km"),
    effectiveness_rating: float = Query(0.7, ge=0, le=1, description="Overall effectiveness rating (0-1 scale)"),
    desired_risk_reduction: float = Query(0.3, ge=0, le=1, description="Target risk reduction percentage (0-1)"),
    maintenance_schedule: str = Query("quarterly", description="Maintenance schedule")
):
    """
    ...
    try:
        ...
        if desired_risk_reduction > 1.0 or desired_risk_reduction < 0:
            raise HTTPException(status_code=400, detail="Desired risk reduction must be between 0 and 1")
        ...
        raise HTTPException(status_code=500, detail=f"Mitigation portfolio evaluation failed: {str(e)}")
...
async def cost_benefit_analysis_endpoint(
    ...
    refuge_distance_km: float = Query(2.0, ge=0, description="Distance to nearest refuge in km"),
    effectiveness_rating: float = Query(0.7, ge=0, le=1, description="Overall effectiveness rating (0-1 scale)"),
    mitigation_cost: float = Query(..., ge=0, description="Total cost of implementing mitigation measures"),
    maintenance_schedule: str = Query("quarterly", description="Maintenance schedule")
):
    """
    ...
    try:
        ...
        result = calculate_cost_benefit_ratio(measures, risk_components, mitigation_cost)
