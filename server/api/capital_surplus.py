...
async def calculate_capital_surplus_endpoint(
    ...
    scr_score: float = Query(
        ..., ge=0, description="SCR score for climate variance calculation"
    ),
    mitigation_effect: float = Query(
        0.0, ge=0, le=1, description="Mitigation effectiveness (0-1)"
    ),
    climate_lambda: float = Query(
        0.1, ge=0, le=1, description="Climate loss probability λ_clim (0-1)"
    ),
    confidence_level: float = Query(
        0.95, ge=0.5, le=0.99, description="Confidence level (0.5-0.99)"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Capital surplus calculation failed: {str(e)}"
        )


@router.post("/capital-surplus/portfolio-analysis")
async def portfolio_capital_surplus_endpoint(
    physical_risks: List[float] = Query(
        ..., description="List of physical risks for each policy"
    ),
    transition_risks: List[float] = Query(
        ..., description="List of transition risks for each policy"
    ),
    concentration_risks: List[float] = Query(
        ..., description="List of concentration risks for each policy"
    ),
    scr_scores: List[float] = Query(
        ..., description="List of SCR scores for each policy"
    ),
    mitigation_effects: List[float] = Query(
        ..., description="List of mitigation effects (0-1) for each policy"
    ),
    portfolio_weights: Optional[List[float]] = Query(
        None, description="Weights for each policy in portfolio"
    ),
    climate_lambda: float = Query(
        0.1, ge=0, le=1, description="Climate loss probability λ_clim (0-1)"
    ),
    ...
    try:
        ...
        if not all(
            ...
            raise HTTPException(
                status_code=400, detail="All risk lists must have the same length"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Portfolio capital surplus calculation failed: {str(e)}",
        )


@router.post("/capital-surplus/optimize-mitigation")
async def optimize_mitigation_endpoint(
    target_capital_surplus: float = Query(
        ..., ge=0, description="Target capital surplus value"
    ),
    physical_risk: float = Query(..., ge=0, description="Physical risk (R_físico)"),
    transition_risk: float = Query(
        ..., ge=0, description="Transition risk (R_transição)"
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Mitigation optimization failed: {str(e)}"
        )
...
async def capital_surplus_factors_endpoint(
    ...
    concentration_risk: float = Query(
        ..., ge=0, description="Concentration risk value"
    ),
    scr_score: float = Query(..., ge=0, description="SCR score"),
    mitigation_effect: float = Query(
        0.0, ge=0, le=1, description="Mitigation effectiveness (0-1)"
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Factors analysis failed: {str(e)}"
        )
