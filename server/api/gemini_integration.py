...
async def analyze_climate_report_endpoint(
    report_text: str = Query(..., description="Text of the technical climate report to analyze"),
    focus_area: str = Query("climate_risk", description="Focus area: climate_risk, policy_language, mitigation_strategies")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Gemini climate report analysis failed: {str(e)}")

@router.post("/gemini/explain-actuarial-decision")
async def explain_actuarial_decision_endpoint(
    decision_type: str = Query("premium_calculation", description="Type of decision: premium_calculation, claim_assessment, risk_analysis"),
    decision_factors_json: str = Query(..., description="JSON string of decision factors")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid JSON in decision_factors: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Gemini actuarial decision explanation failed: {str(e)}")

@router.post("/gemini/generate-mitigation-suggestions")
async def generate_mitigation_suggestions_endpoint(
    asset_type: str = Query("property", description="Asset type: property, infrastructure, agriculture, etc."),
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid JSON in risk_profile: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Gemini mitigation suggestions generation failed: {str(e)}")

@router.post("/gemini/analyze-policy-language")
async def analyze_policy_language_endpoint(
    focus_on: str = Query("climate_exclusions", description="Focus of analysis: climate_exclusions, coverage_limits, definitions"),
    policy_text: str = Query(..., description="Text of the insurance policy to analyze")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Gemini policy language analysis failed: {str(e)}")

@router.post("/gemini/summarize-climate-data")
async def summarize_climate_data_endpoint(
    analysis_period: str = Query("12_months", description="Analysis period: 6_months, 12_months, 24_months, 36_months"),
    climate_data_json: str = Query(..., description="JSON string of climate variables data")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid JSON in climate_data: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Gemini climate data summarization failed: {str(e)}")
