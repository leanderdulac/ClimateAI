...
async def calculate_physical_risk_endpoint(
    age_years: int = Query(..., ge=0, le=200, description="Age of the property in years"),
    construction_material: str = Query("concrete", description="Construction material: concrete, wood, steel, masonry"),
    elevation_meters: float = Query(0.0, description="Elevation relative to reference level (meters)"),
    ...
    delta_temperature: float = Query(0.0, description="Temperature change from baseline (ΔT in °C)"),
    precipitation_change: float = Query(0.0, description="Precipitation change from baseline (mm)"),
    sea_level_rise: float = Query(0.0, description="Sea level rise (meters)"),
    climate_model: str = Query("ssp245", description="Climate model: rcp45, rcp85, ssp126, ssp585, ssp245"),
    scenario_year: int = Query(2040, ge=2020, le=2100, description="Target scenario year"),
    ...
    try:
        ...
        if construction_material not in valid_materials:
            raise HTTPException(status_code=400, detail=f"Invalid material. Valid options: {valid_materials}")
        
        # Validate building type
        valid_building_types = ["residential", "commercial", "industrial", "agricultural"]
        if building_type not in valid_building_types:
            raise HTTPException(status_code=400, detail=f"Invalid building type. Valid options: {valid_building_types}")
        ...
        raise HTTPException(status_code=500, detail=f"Physical risk calculation failed: {str(e)}")
...
async def scenario_comparison_endpoint(
    ...
    asset_value: float = Query(..., gt=0, description="Asset value in currency units"),
    baseline_temp: float = Query(0.0, description="Baseline temperature change (ΔT in °C)"),
    baseline_precip: float = Query(0.0, description="Baseline precipitation change (mm)"),
    ...
    try:
        ...
        result = calculate_scenario_comparison(property_char, baseline_scenario, future_scenario)
...
async def integrate_with_climate_risk_endpoint(
    climate_risk_score: float = Query(..., ge=0, le=1, description="Overall climate risk score from upstream module"),
    age_years: int = Query(..., ge=0, le=200, description="Age of the property in years"),
    construction_material: str = Query("concrete", description="Construction material"),
    elevation_meters: float = Query(0.0, description="Elevation relative to reference level (meters)"),
    latitude: float = Query(..., ge=-90, le=90, description="Property latitude"),
    ...
    try:
        ...
        # Calculate physical risk
        physical_risk_result = calculate_physical_risk(property_char, climate_scenario, "integration")
        
        # Integrate with climate risk score
        integration_result = integrate_with_climate_risk(climate_risk_score, physical_risk_result)
