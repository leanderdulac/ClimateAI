...
async def sample_posterior_parameters_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Posterior parameter sampling failed: {str(e)}"
        )
...
async def monte_carlo_simulation_endpoint(
    n_scenarios: int = Query(
        10000, ge=100, description="Number of Monte Carlo scenarios"
    ),
    ...
    try:
        ...
        return {
            "scenario_results": results[
                :100
            ],  # Return first 100 results to avoid huge responses
...
async def calculate_percentiles_endpoint(
    scenario_results: List[float] = Query(
        ..., description="Monte Carlo scenario results"
    ),
    percentiles: List[int] = Query(
        [10, 50, 90], description="Percentiles to calculate"
    ),
):
    """
...
async def calculate_value_at_risk_endpoint(
    ...
    confidence_level: float = Query(
        0.95, ge=0.5, lt=1.0, description="Confidence level for VaR"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Value at Risk calculation failed: {str(e)}"
        )
...
async def calculate_conditional_value_at_risk_endpoint(
    ...
    confidence_level: float = Query(
        0.95, ge=0.5, lt=1.0, description="Confidence level for CVaR"
    ),
):
    """
    ...
    """
    try:
        cvar_value = calculate_conditional_value_at_risk(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Conditional Value at Risk calculation failed: {str(e)}",
        )


@router.post("/bayesian-bootstrap/premium-calculation")
async def bayesian_bootstrap_premium_endpoint(
    contract_data: List[float] = Query(
        ..., description="Historical data for the contract"
    ),
    ...
    confidence_level: float = Query(
        0.95, ge=0.5, lt=1.0, description="Confidence level for VaR/CVaR"
    ),
    ...
    try:
        ...
        result_dict = {
            ...
            "statistics": {
                ...
                "skewness": (
                    float(pd.Series(result.scenario_results).skew()) if pd else 0.0
                ),
            },
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Bayesian bootstrap premium calculation failed: {str(e)}",
        )
...
async def calculate_contract_uncertainty_ranges_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Contract uncertainty ranges calculation failed: {str(e)}",
        )
