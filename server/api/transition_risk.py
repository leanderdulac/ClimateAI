...
async def calculate_transition_risk_endpoint(
    fossil_energy_exposure: float = Query(..., ge=0, le=1, description="Exposure to fossil energy (0-1)"),
    asset_age_years: int = Query(0, ge=0, le=100, description="Age of the asset in years"),
    industry_sector: str = Query("manufacturing", description="Industry sector: oil_gas, coal, utilities, transport, manufacturing, finance"),
    revenue_dependence: float = Query(0.0, ge=0, le=1, description="Revenue dependence on high-carbon assets (0-1)"),
    asset_value: float = Query(..., gt=0, description="Total asset value in currency units"),
    geographical_diversification: float = Query(0.5, ge=0, le=1, description="Geographical diversification factor (0-1)"),
    adaptation_readiness: float = Query(0.3, ge=0, le=1, description="Adaptation readiness score (0-1)"),
    delta_temperature: float = Query(1.5, description="ΔT in °C"),
    carbon_price_usd_per_tco2: float = Query(150, description="Carbon price in USD per tonne CO2e"),
    litigation_media_exposure: float = Query(0.5, ge=0, le=1, description="Media exposure score (0-1)"),
    regulatory_delay_factor: float = Query(0.2, ge=0, le=1, description="Regulatory delay factor (0-1)"),
    scenario_year: int = Query(2030, ge=2020, le=2100, description="Target scenario year"),
    baseline_year: int = Query(2020, ge=1900, le=2020, description="Baseline reference year"),
    scenario_name: str = Query("default", description="Name for the scenario"),
    beta_carbon_tax: float = Query(0.4, description="Beta weight for carbon tax component (β₁)"),
    beta_stranded_asset: float = Query(0.4, description="Beta weight for stranded asset component (β₂)"),
    beta_litigation: float = Query(0.2, description="Beta weight for litigation component (β₃)")
):
    """
    ...
    try:
        ...
        valid_sectors = ["oil_gas", "coal", "utilities", "transport", "manufacturing", "finance"]
        if industry_sector not in valid_sectors:
            raise HTTPException(status_code=400, detail=f"Invalid sector. Valid options: {valid_sectors}")
        ...
        result = calculate_transition_risk(asset_char, env_scenario, beta_weights, scenario_name)
        ...
        return {
            ...
            "component_breakdown": {
                "carbon_tax_unweighted": result.risk_components.get('carbon_tax_unweighted', 0),
                "stranded_asset_unweighted": result.risk_components.get('stranded_asset_unweighted', 0),
                "litigation_unweighted": result.risk_components.get('litigation_unweighted', 0),
                "carbon_tax_weighted": result.risk_components.get('carbon_tax_weighted', 0),
                "stranded_asset_weighted": result.risk_components.get('stranded_asset_weighted', 0),
                "litigation_weighted": result.risk_components.get('litigation_weighted', 0)
            }
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Transition risk calculation failed: {str(e)}")
...
async def scenario_comparison_endpoint(
    ...
    baseline_carbon_price: float = Query(50, description="Baseline carbon price in USD per tonne CO2e"),
    baseline_media: float = Query(0.2, ge=0, le=1, description="Baseline media exposure score (0-1)"),
    baseline_regulatory: float = Query(0.1, ge=0, le=1, description="Baseline regulatory delay factor (0-1)"),
    # Future scenario parameters
    future_temp: float = Query(2.0, description="Future ΔT in °C"),
    future_carbon_price: float = Query(150, description="Future carbon price in USD per tonne CO2e"),
    future_media: float = Query(0.7, ge=0, le=1, description="Future media exposure score (0-1)"),
    future_regulatory: float = Query(0.4, ge=0, le=1, description="Future regulatory delay factor (0-1)"),
    scenario_year: int = Query(2030, ge=2020, le=2100, description="Target scenario year"),
    baseline_year: int = Query(2020, ge=1900, le=2020, description="Baseline reference year"),
    beta_carbon_tax: float = Query(0.4, description="Beta weight for carbon tax component"),
    beta_stranded_asset: float = Query(0.4, description="Beta weight for stranded asset component"),
    beta_litigation: float = Query(0.2, description="Beta weight for litigation component")
):
    """
    ...
    try:
        ...
        result = calculate_scenario_comparison(asset_char, baseline_scenario, future_scenario, beta_weights)
...
async def sector_specific_risk_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Sector-specific risk calculation failed: {str(e)}")

@router.post("/transition-risk/integrate-with-physical-risk")
async def integrate_with_physical_risk_endpoint(
    transition_risk_score: float = Query(..., ge=0, description="Calculated transition risk from transition risk module"),
    physical_risk_score: float = Query(..., ge=0, description="Calculated physical risk from physical risk module"),
    fossil_energy_exposure: float = Query(..., ge=0, le=1, description="Exposure to fossil energy (0-1)"),
    asset_age_years: int = Query(0, ge=0, le=100, description="Age of the asset in years"),
    industry_sector: str = Query("manufacturing", description="Industry sector"),
    revenue_dependence: float = Query(0.0, ge=0, le=1, description="Revenue dependence on high-carbon assets (0-1)"),
    ...
    try:
        ...
        transition_risk_result = calculate_transition_risk(asset_char, env_scenario, beta_weights, "integration")
        
        # Integrate with physical risk score
        integration_result = integrate_with_physical_risk(transition_risk_result, physical_risk_score)
        
        return integration_result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Risk integration failed: {str(e)}")
...
async def transition_risk_info():
    ...
    return {
        ...
        "sectors_covered": ["oil_gas", "coal", "utilities", "transport", "manufacturing", "finance"],
