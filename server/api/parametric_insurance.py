...
async def calculate_wind_index_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Wind index calculation failed: {str(e)}"
        )
...
async def calculate_precipitation_index_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Precipitation index calculation failed: {str(e)}"
        )
...
async def calculate_temperature_index_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Temperature index calculation failed: {str(e)}"
        )
...
async def calculate_composite_index_endpoint(
    ...
    weights: Tuple[float, float, float] = Query(
        (0.4, 0.4, 0.2), description="Weights for [wind, precip, temp]"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Composite index calculation failed: {str(e)}"
        )
...
async def calculate_payout_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Payout calculation failed: {str(e)}"
        )
...
async def optimize_trigger_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Trigger optimization failed: {str(e)}"
        )
...
async def calculate_parametric_insurance_contract_endpoint(
    ...
    cap: float = Query(..., gt=0, description="Payout cap"),
    factor: float = Query(1.0, description="Payout factor (K)"),
    trigger: Optional[float] = Query(
        None, description="Trigger threshold (if None, will be optimized)"
    ),
    basis_risk_weight: float = Query(0.1, description="Basis risk weight (λ)"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Parametric insurance contract calculation failed: {str(e)}",
        )
...
async def parametric_insurance_status():
    ...
    """
    return {"service_available": True, "timestamp": datetime.now().isoformat()}
