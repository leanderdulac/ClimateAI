...
async def calculate_policy_valuation_endpoint(
    ...
    premium_amount: float = Query(
        ..., gt=0, description="Premium amount in currency units"
    ),
    expected_claims: float = Query(0.0, ge=0, description="Expected claims amount"),
    claim_frequency: float = Query(
        0.1, ge=0, le=1, description="Claim frequency (0-1)"
    ),
    claim_severity: float = Query(
        10000.0, ge=0, description="Claim severity (average claim size)"
    ),
    climate_risk_score: float = Query(
        500.0, ge=0, le=1000, description="Climate risk score (0-1000)"
    ),
    ...
    concentration_risk: float = Query(
        0.2, ge=0, le=1, description="Concentration risk (0-1)"
    ),
    geographic_factor: float = Query(
        0.5, ge=0, le=1, description="Geographic risk factor (0-1)"
    ),
    regulatory_factor: float = Query(
        0.3, ge=0, le=1, description="Regulatory factor (0-1)"
    ),
    economic_factor: float = Query(
        0.4, ge=0, le=1, description="Economic exposure factor (0-1)"
    ),
    policy_value: float = Query(
        None, gt=0, description="Total policy value (optional)"
    ),
):
    """
    ...
    try:
        ...
        return {
            ...
            "risk_assessment": {
                "risk_level": (
                    "high"
                    if valuation.valuation_score < 50
                    else "medium" if valuation.valuation_score < 70 else "low"
                ),
                "confidence_level": (
                    "high"
                    if valuation.calculation_timestamp >= 0.8
                    else "medium" if valuation.calculation_timestamp >= 0.6 else "low"
                ),
                "recommendation": (
                    "prioritize"
                    if valuation.valuation_tier.value in ["excellent", "good"]
                    else (
                        "review_with_caution"
                        if valuation.valuation_tier.value == "fair"
                        else "consider_avoiding"
                    )
                ),
            },
            "status": "completed",
        ...
        raise HTTPException(
            status_code=500, detail=f"Policy valuation calculation failed: {str(e)}"
        )
...
async def get_interactive_policy_analysis_endpoint(
    ...
    max_options: int = Query(
        5, ge=1, le=10, description="Maximum number of improvement options to return"
    ),
):
    """
    ...
    try:
        ...
        analysis = get_interactive_policy_analysis(
            ...
            improvement_options.append(
                {
                    ...
                    "roi_percentage": (
                        (option.expected_benefit / option.cost * 100)
                        if option.cost > 0
                        else 0
                    ),
                }
            ...
            top_recommendations.append(
                {
                    ...
                    "roi_percentage": (
                        (rec.expected_benefit / rec.cost * 100) if rec.cost > 0 else 0
                    ),
        ...
        return {
            ...
            "options_summary": {
                ...
                "highest_roi_option": (
                    max(improvement_options, key=lambda x: x["roi_percentage"])
                    if improvement_options
                    else None
                ),
                "total_cost_for_top_options": sum(
                    rec["cost"] for rec in top_recommendations
                ),
                "total_expected_benefit": sum(
                    rec["expected_benefit"] for rec in top_recommendations
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Interactive policy analysis failed: {str(e)}"
        )
    ...
    try:
        ...
        return {
            ...
            "notification_summary": {
                "excellent_policies": len(
                    [n for n in notifications if n["valuation_tier"] == "excellent"]
                ),
                "good_policies": len(
                    [n for n in notifications if n["valuation_tier"] == "good"]
                ),
                "improvement_opportunities": len(
                    [
                        n
                        for n in notifications
                        if n["valuation_tier"] in ["fair", "poor"]
                        and any(
                            "melhoria" in action.lower()
                            for action in n.get("recommended_actions", [])
                        )
                    ]
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Pending notifications retrieval failed: {str(e)}"
        )
...
async def mark_notification_as_processed_endpoint(
    notification_id: str = Query(
        ..., description="Notification identifier to mark as processed"
    )
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Notification processing marking failed: {str(e)}"
        )
...
async def get_policy_recommendations_summary_endpoint(policy_id: str):
    ...
    try:
        ...
        if not summary:
            raise HTTPException(
                status_code=404,
                detail=f"No recommendations found for policy {policy_id}",
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Policy recommendations summary retrieval failed: {str(e)}",
        )
...
async def get_valuable_policies_summary_endpoint(
    min_score: float = Query(
        70, ge=0, le=100, description="Minimum valuation score to include"
    )
):
    """
    ...
    try:
        ...
        return {
            ...
            "valuation_distribution": {
                "excellent": len(
                    [p for p in valuable_policies if p["valuation_tier"] == "excellent"]
                ),
                "good": len(
                    [p for p in valuable_policies if p["valuation_tier"] == "good"]
                ),
                "all_above_threshold": len(valuable_policies),
            },
            "summary_statistics": {
                "avg_valuation_score": (
                    sum(p["valuation_score"] for p in valuable_policies)
                    / len(valuable_policies)
                    if valuable_policies
                    else 0
                ),
                "max_valuation_score": max(
                    (p["valuation_score"] for p in valuable_policies), default=0
                ),
                "min_valuation_score": min(
                    (p["valuation_score"] for p in valuable_policies), default=0
                ),
                "total_premium_potential": (
                    sum(
                        p.get("premium_efficiency", 0)
                        * p.get("profitability_score", 0)
                        / 100
                        for p in valuable_policies
                    )
                    if valuable_policies
                    else 0
                ),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Valuable policies summary retrieval failed: {str(e)}",
        )
...
async def get_policy_valuation_service_info():
    ...
    return {
        ...
        "valuation_tiers": {
            "excellent": {
                "min_score": 80,
                "description": "High value, low risk policies - prioritize acquisition",
            },
            "good": {
                "min_score": 65,
                "description": "Good value, moderate risk - favorable acquisition",
            },
            "fair": {
                "min_score": 50,
                "description": "Average value, acceptable risk - acquire with caution",
            },
            "poor": {
                "min_score": 30,
                "description": "Low value, high risk - consider carefully",
            },
