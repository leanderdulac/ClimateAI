...
async def calculate_spi_endpoint(
    ...
    window_months: int = Query(
        3, ge=1, le=24, description="Time window in months (3, 6, 12)"
    ),
):
    """
    ...
    """
    try:
        result = calculate_standardized_precipitation_index(
...
async def calculate_rwi_endpoint(precipitation: List[float], temperature: List[float]):
    """
    ...
    try:
        ...
        return {"rwi_values": result, "n_values": len(result)}
    except Exception as e:
...
async def extract_synoptic_patterns_endpoint(
    ...
    """
    try:
        result = extract_synoptic_circulation_patterns(
            pressure_data, wind_data, lat_lon_data
        )
        return {"circulation_patterns": result, "n_patterns": len(result)}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Synoptic pattern extraction failed: {str(e)}"
        )


@router.post("/climate-risk-modeling/vertical-temperature-gradient")
async def calculate_temp_gradient_endpoint(
    temperature_profile_data: List[
        List[float]
    ],  # [temp_surface, temp_850hpa, temp_700hpa, ...]
):
    """
    ...
    try:
        ...
        return {"temperature_gradients": result, "n_gradients": len(result)}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Temperature gradient calculation failed: {str(e)}"
        )
...
async def calculate_regularized_loss_endpoint(
    ...
    try:
        ...
        if len(y_true_arr) != len(y_pred_arr):
            raise HTTPException(
                status_code=400, detail="y_true and y_pred must have same length"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Regularized loss calculation failed: {str(e)}"
        )
...
async def comprehensive_climate_risk_assessment_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Comprehensive risk assessment failed: {str(e)}"
        )
