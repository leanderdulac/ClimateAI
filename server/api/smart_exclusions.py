...
async def evaluate_climate_exclusions_endpoint(
    ...
    climate_induced_seismicity_confidence: float = Query(0.5, ge=0, le=1, description="Climate-induced seismicity model confidence"),
    litigation_maturity: float = Query(0.2, ge=0, le=1, description="Litigation model maturity"),
    required_mitigation_measures: str = Query("[]", description="JSON string of required mitigation measures"),
    ...
    try:
        ...
        return {
            ...
            "total_governance_recommendations": len(governance_recommendations_formatted)
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Climate exclusions evaluation failed: {str(e)}")
...
async def get_exclusion_decision_endpoint(policy_id: str):
    ...
    try:
        ...
        if not decision:
            raise HTTPException(status_code=404, detail=f"No exclusion decision found for policy {policy_id}")
        ...
        raise HTTPException(status_code=500, detail=f"Exclusion decision retrieval failed: {str(e)}")
...
async def get_upcoming_reviews_endpoint():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Upcoming reviews check failed: {str(e)}")
...
async def update_governance_rule_endpoint(
    ...
    new_status: str = Query(..., description="New status for the rule (active, inactive, pending)")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Governance rule update failed: {str(e)}")
...
async def get_governance_status():
    ...
    try:
        ...
        return {
            ...
            "status_summary": {
                "manual_approval_rule": next((rule for rule in active_rules if rule.rule_type.value == "aprovacao_manual_obrigatoria"), None) is not None,
                "quarterly_review_rule": next((rule for rule in active_rules if rule.rule_type.value == "comite_risco_trimestral"), None) is not None,
                "annual_audit_rule": next((rule for rule in active_rules if rule.rule_type.value == "auditoria_externa_anual"), None) is not None
            },
            "timestamp": datetime.now().isoformat()
        }
    except Exception as e:
