...
async def extreme_value_gev_analysis(
    data: List[float],
    return_period: float = Query(50.0, ge=1.0, le=1000.0)
):
    """
    ...
    try:
        if len(data) < 10:
            raise HTTPException(
                status_code=400,
                detail="Need at least 10 data points for GEV analysis"
            )
...
async def extreme_value_gpd_analysis(
    ...
    try:
        if len(data) < 5:
            raise HTTPException(
                status_code=400,
                detail="Need at least 5 data points for GPD analysis"
            )
...
async def gev_distribution_cdf_endpoint(
    ...
    try:
        ...
        return {"cdf_value": result, "z": z, "parameters": {"mu": mu, "sigma": sigma, "xi": xi}}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"GEV CDF calculation failed: {str(e)}")
...
async def calculate_climate_adapted_gev_endpoint(
    ...
    delta_precipitation: float = Query(0.0, description="Change in precipitation (ΔPrecip)"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Climate-adapted GEV calculation failed: {str(e)}")
...
async def calculate_climate_adapted_return_level_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Climate-adapted return level calculation failed: {str(e)}")
...
async def spatial_gaussian_process_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Spatial Gaussian Process model fitting failed: {str(e)}")
...
async def predict_at_new_locations_endpoint(
    ...
    try:
        ...
        result = predict_at_new_locations(fitted_model, new_coord_tuples, new_covariates)

        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Spatial Gaussian Process prediction failed: {str(e)}")
...
async def calculate_extreme_event_probability_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Event probability calculation failed: {str(e)}")
...
async def spatial_kde_analysis(
    ...
    try:
        if len(coordinates) < 2:
            raise HTTPException(
                status_code=400,
                detail="Need at least 2 coordinates for KDE analysis"
            )
...
async def spatial_correlation_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Spatial correlation analysis failed: {str(e)}")
...
async def spatial_clustering_analysis(
    ...
    try:
        ...
        result = geospatial_clustering(coord_tuples, values, eps=eps, min_samples=min_samples)
        return result
    except Exception as e:
...
async def combined_spatial_risk_assessment_endpoint(
    ...
    """
    try:
        if len(coordinates) < 2 or len(coordinates) != len(asset_values) or len(coordinates) != len(risk_scores):
            raise HTTPException(
                status_code=400,
        ...
        result = combined_spatial_risk_assessment(coord_tuples, asset_values, risk_scores)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Combined spatial risk assessment failed: {str(e)}")
...
async def fit_copula_model_endpoint(
    ...
    try:
        if len(data1) != len(data2):
            raise HTTPException(
                status_code=400,
                detail="Both datasets must have same length"
            )
...
async def regime_switching_model_endpoint(
    time_series: List[float],
    n_states: int = Query(2, ge=2, le=5)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Regime switching model failed: {str(e)}")
...
async def multivariate_climate_modeling_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Multivariate climate modeling failed: {str(e)}")
...
async def comprehensive_climate_risk_analysis(
    ...
    try:
        ...
        if len(time_series_data) < 1:
            raise HTTPException(status_code=400, detail="Need at least one time series for analysis")
        
        if len(spatial_coordinates) < 2:
            raise HTTPException(status_code=400, detail="Need at least 2 spatial locations for analysis")
        
        if len(spatial_coordinates) != len(asset_exposures) or len(spatial_coordinates) != len(risk_exposures):
            raise HTTPException(
                status_code=400,
        ...
        spatial_result = combined_spatial_risk_assessment(coord_tuples, asset_exposures, risk_exposures)
        ...
        comprehensive_result = {
            ...
            "integrated_risk_metrics": {
                "max_return_level": ext_val_result.get("risk_metrics", {}).get("var_995", 0.0) if isinstance(ext_val_result, dict) and "risk_metrics" in ext_val_result else 0.0,
                "spatial_correlation": spatial_result.get("spatial_correlation_analysis", {}).get("spatial_correlation", 0.0),
                "volatility_clustering": stochastic_result.get("univariate_models", {}).get(first_var_name, {}).get("volatility_clustering_parameter", 0.0) if isinstance(stochastic_result, dict) and "univariate_models" in stochastic_result else 0.0,
                "regime_probability": stochastic_result.get("regime_probabilities", [0.0])[0] if isinstance(stochastic_result, dict) else 0.0
            }
        }
        ...
        raise HTTPException(status_code=500, detail=f"Comprehensive risk analysis failed: {str(e)}")