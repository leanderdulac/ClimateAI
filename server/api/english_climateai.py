"""
API Router for English ClimateAI System
...
from services.i18n_service import i18n_service, Language
...
@router.get("/english-climateai/info")
async def get_english_climateai_info():
    """
    Get information about the English version of ClimateAI
    """
    return {
...
@router.post("/english-climateai/location-selection")
async def english_location_selection(
    ...
    history_years: int = Query(90, ge=1, le=120, description="Years of historical data (1-120, default 90)")
):
    """
    ...
    try:
        ...
        return {
            "location_selection_result": {
                ...
                "historical_data_records": len(location_result.historical_rainfall_data),
                "rainfall_statistics": location_result.rainfall_stats,
                "precipitation_pattern": location_result.precipitation_pattern,
                "data_year_range": f"{location_result.historical_rainfall_data[0]['date'][:4] if location_result.historical_rainfall_data else 'N/A'} to {location_result.historical_rainfall_data[-1]['date'][:4] if location_result.historical_rainfall_data else 'N/A'}" if len(location_result.historical_rainfall_data) > 0 else "No Data"
            },
            "globe_animation": {
        ...
        raise HTTPException(status_code=500, detail=f"English location selection failed: {str(e)}")

@router.post("/english-climateai/risk-assessment")
async def english_climate_risk_assessment(
    latitude: float = Query(..., ge=-90, le=90, description="Latitude (-90 to 90)"),
    longitude: float = Query(..., ge=-180, le=180, description="Longitude (-180 to 180)"),
    scr_score: float = Query(680.0, ge=0, le=1000, description="Climate Risk Score (0-1000)"),
    ...
    physical_risk: float = Query(0.40, ge=0, le=1, description="Physical risk factor (0-1)"),
    transition_risk: float = Query(0.30, ge=0, le=1, description="Transition risk factor (0-1)"),
    mitigation_effectiveness: float = Query(0.65, ge=0, le=1, description="Mitigation effectiveness (0-1)"),
    ...
    try:
        ...
        portfolio_scr_result = calculate_simple_portfolio_scr(portfolio_var_losses, portfolio_exp_losses)

        # Analyze with AI agent
        ai_analysis = climate_analytics_agent.analyze_climate_risks(climate_risk_factors)

        # Calculate climate capital charge
        climate_capital_charge = max(0, portfolio_scr_result.total_scr - expected_claims * 0.03)  # Example: subtract 3% of expected claims as reserves
        ...
        raise HTTPException(status_code=500, detail=f"English climate risk assessment failed: {str(e)}")

@router.post("/english-climateai/premium-calculation")
async def english_premium_calculation(
    ...
    expected_claims: float = Query(10000.0, gt=0, description="Expected claims amount"),
    climate_risk_score: float = Query(680.0, ge=0, le=1000, description="Climate risk score (0-1000)"),
    physical_risk: float = Query(0.40, ge=0, le=1, description="Physical risk factor (0-1)"),
    transition_risk: float = Query(0.30, ge=0, le=1, description="Transition risk factor (0-1)"),
    mitigation_effectiveness: float = Query(0.65, ge=0, le=1, description="Mitigation effectiveness (0-1)"),
    model_confidence: float = Query(0.75, ge=0, le=1, description="Model confidence (0-1)"),
    concentration_risk: float = Query(0.22, ge=0, le=1, description="Concentration risk (0-1)"),
    ...
    try:
        ...
        policy_valuation = calculate_policy_valuation(policy_id, policy_metrics, premium_amount)
        ...
        raise HTTPException(status_code=500, detail=f"English premium calculation failed: {str(e)}")

@router.post("/english-climateai/claims-assessment")
async def english_claims_assessment(
    claim_amount: float = Query(..., gt=0, description="Amount being claimed"),
    climate_risk_score: float = Query(680.0, ge=0, le=1000, description="Climate risk score"),
    physical_risk: float = Query(0.40, ge=0, le=1, description="Physical risk factor (0-1)"),
    transition_risk: float = Query(0.30, ge=0, le=1, description="Transition risk factor (0-1)"),
    mitigation_effectiveness: float = Query(0.65, ge=0, le=1, description="Mitigation effectiveness (0-1)"),
    model_confidence: float = Query(0.75, ge=0, le=1, description="Model confidence (0-1)"),
    ...
    try:
        ...
        climate_factors = AnalyticsClimateRiskFactors(
            scr_score=climate_risk_score,
            climate_var_99=climate_risk_score * 18,  # Scale appropriately
            expected_loss=policy_premium * 0.5,  # Example: 50% of premium as expected loss
            physical_risk=physical_risk,
            transition_risk=transition_risk,
        ...
        validity_probability = min(1.0, max(
            ...
            (1 - mitigation_effectiveness) * 0.3  # Poor mitigation = higher claim likelihood
        ))
        ...
        expected_vs_claimed_ratio = claim_amount / (policy_premium * 0.5)  # Compare to expected loss
        fraud_indicator = max(0.0, min(1.0, 0.5 + (expected_vs_claimed_ratio - 1) * 0.5))
        ...
        raise HTTPException(status_code=500, detail=f"English claims assessment failed: {str(e)}")

@router.post("/english-climateai/system-evaluation")
async def english_system_evaluation(
    portfolio_size: int = Query(100, ge=1, le=10000, description="Size of portfolio to evaluate"),
    total_premium: float = Query(5000000.0, gt=0, description="Total portfolio premium"),
    total_expected_claims: float = Query(2500000.0, gt=0, description="Total expected claims"),
    average_climate_risk_score: float = Query(550.0, ge=0, le=1000, description="Average climate risk score"),
    average_mitigation_score: float = Query(0.60, ge=0, le=1, description="Average mitigation score")
):
    """
    ...
    try:
        ...
        tcfd_report = {
            ...
            "climate_risk_exposure": total_expected_claims * 0.4,  # 40% of expected claims as climate risk
            "transition_risk_exposure": total_expected_claims * 0.3,  # 30% of expected claims as transition risk
            "physical_risk_exposure": total_expected_claims * 0.35,  # 35% of expected claims as physical risk
        ...
        performance_metrics = {
            "portfolio_efficiency": (total_premium - total_expected_claims) / total_premium if total_premium > 0 else 0,
            "risk_mitigation_effectiveness": average_mitigation_score,
            "capital_efficiency": (total_premium - total_expected_claims) / 52000000.0 if 52000000.0 > 0 else 0,  # Example capital
            "climate_risk_coverage_ratio": total_expected_claims / (average_climate_risk_score * portfolio_size * 10),
        ...
        raise HTTPException(status_code=500, detail=f"English system evaluation failed: {str(e)}")

@router.get("/english-climateai/translation-check")
