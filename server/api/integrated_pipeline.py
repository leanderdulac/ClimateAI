...
async def process_integrated_pipeline(
    ...
    coverage_requested: float = Query(
        ..., gt=0, description="Amount of coverage requested"
    ),
    coverage_type: str = Query(
        "property",
        description="Type of coverage (property, agriculture, infrastructure, livestock, crop)",
    ),
    asset_value: float = Query(
        ..., gt=0, description="Value of the asset to be insured"
    ),
    latitude: float = Query(
        ..., ge=-90, le=90, description="Latitude of the insured asset"
    ),
    longitude: float = Query(
        ..., ge=-180, le=180, description="Longitude of the insured asset"
    ),
...
async def epc_price(
    ...
    market_data = MarketData(
        ...
        market_average_rate=market_conditions.get(
            "market_average_rate", actuarial_premium
        ),
        market_std_rate=market_conditions.get(
            "market_std_rate", actuarial_premium * 0.2
        ),
    ...
    try:
        result = calculate_commercial_pricing(
            risk_adjusted, market_data, PricingStrategy.MARKET_MATCHING
        )
        return {
