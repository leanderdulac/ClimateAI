...
async def create_policy_uncertainty_clause(
    ...
    uncertainty_min: float = Query(
        0.35, ge=0, le=1, description="Minimum uncertainty percentage (0.35 = 35%)"
    ),
    uncertainty_max: float = Query(
        0.60, ge=0, le=1, description="Maximum uncertainty percentage (0.60 = 60%)"
    ),
    projection_horizon: int = Query(
        10, ge=1, description="Projection horizon in years (>10 years)"
    ),
    cmip_source: str = Query(
        "CMIP7", description="Climate model intercomparison project source"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Uncertainty clause creation failed: {str(e)}"
        )
...
async def get_policy_uncertainty_clause(policy_id: str):
    ...
    try:
        ...
        if not clause:
            raise HTTPException(
                status_code=404,
                detail=f"No uncertainty clause found for policy {policy_id}",
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Uncertainty clause retrieval failed: {str(e)}"
        )
...
async def update_policy_clause_for_cmip_data(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Uncertainty clause update failed: {str(e)}"
        )
...
async def register_calibration_event_endpoint(
    ...
    event_description: str = Query(
        ..., description="Description of the calibration event"
    ),
    policy_ids: List[str] = Query(
        ..., description="List of policy IDs affected by this event"
    ),
    impact_on_uncertainty: float = Query(
        0.1, ge=0, le=1, description="Impact on uncertainty (0.0 to 1.0)"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Calibration event registration failed: {str(e)}"
        )
...
async def get_policies_requiring_review_endpoint():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Policies requiring review check failed: {str(e)}"
        )


@router.post("/policy-uncertainty/calculate-adjusted-uncertainty")
async def calculate_adjusted_uncertainty_endpoint(
    base_uncertainty: float = Query(
        ..., ge=0, le=1, description="Base uncertainty value"
    ),
    event_ids: List[str] = Query(
        ..., description="List of calibration event IDs that may affect uncertainty"
    ),
):
    """
    ...
    try:
        ...
        if not calibration_events:
            raise HTTPException(
                status_code=404,
                detail="No calibration events found for the provided IDs",
            )

        adjusted_uncertainty = calculate_adjusted_uncertainty(
        ...
        return {
            ...
            "event_ids": event_ids,
            "adjustment_factor": (
                adjusted_uncertainty / base_uncertainty if base_uncertainty > 0 else 1.0
            ),
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Adjusted uncertainty calculation failed: {str(e)}"
        )
