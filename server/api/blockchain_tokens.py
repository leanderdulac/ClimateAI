...
async def mint_climate_token(request: TokenMintRequest = Body(...)):
    ...
    try:
        ...
        if not resultado["success"]:
            raise HTTPException(
                status_code=400,
                detail=f"Erro ao criar token: {resultado['error']}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro interno ao criar token: {str(e)}"
        )
...
async def get_transaction_info(tx_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao buscar transação: {str(e)}"
        )
...
async def get_wallet_tokens(wallet_address: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao buscar tokens da carteira: {str(e)}"
        )
...
async def transfer_token(request: TokenTransferRequest = Body(...)):
    ...
    try:
        ...
        if not resultado["success"]:
            raise HTTPException(
                status_code=400,
                detail=f"Erro na transferência: {resultado['error']}"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro interno na transferência: {str(e)}"
        )
        ...
        for item in blockchain_service.blockchain_registry.values():
            if isinstance(item, dict):
                if "tx_id" in item and "type" in item:
                    ...
                    total_tokens += 1
                    event_type = item["token_data"].get("climate_event", {}).get("event_type")
                    if event_type:
    ...
    try:
        ...
        for token in tokens:
            ...
            formatted_tokens.append({
                ...
                'value': token.get('initial_supply', 0),  # Simplificado - em produção usar preço real
...
async def get_wallet_transactions(wallet_address: str, limit: int = Query(50, ge=1, le=100)):
    """
    ...
    try:
            ...
            if isinstance(item, dict) and 'tx_id' in item:
                ...
                if tx.get('inputs'):
                    for inp in tx['inputs']:
                        if isinstance(inp, dict) and inp.get('address') == wallet_address:
                ...
                if not wallet_involved and tx.get('outputs'):
                    for out in tx['outputs']:
                        if isinstance(out, dict) and out.get('address') == wallet_address:
                            wallet_involved = True
                            break
                ...
                if wallet_involved:
                    ...
                    transactions.append({
                        ...
                        'amount': tx.get('outputs', [{}])[0].get('value', 0) if tx.get('outputs') else 0,
                        'from': tx.get('inputs', [{}])[0].get('address', 'system') if tx.get('inputs') else 'system',
                        'to': tx.get('outputs', [{}])[0].get('address', wallet_address) if tx.get('outputs') else wallet_address,
                        'timestamp': tx.get('timestamp', ''),
                        'status': tx.get('status', 'confirmed'),
                        'value': tx.get('outputs', [{}])[0].get('value', 0) if tx.get('outputs') else 0
                    })
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao obter transações da carteira: {str(e)}"
        )