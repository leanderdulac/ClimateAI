...
async def dynamical_climate_prediction(
    initial_conditions: List[float] = Query(..., description="Initial conditions for dynamical system"),
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error in dynamical prediction: {str(e)}")
...
async def phase_space_reconstruction(
    ...
    target_var: str = Query("temperature", description="Target variable for reconstruction")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error in phase space reconstruction: {str(e)}")


@router.post("/regime-shift-detection")
async def detect_climate_regime_shifts_endpoint(
    climate_time_series: List[float]
):
    """
    ...
    """
    try:
        result = detect_climate_regime_shifts(
            climate_time_series=climate_time_series
        )
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error in regime shift detection: {str(e)}")
...
async def climate_predictability_horizon(
    ...
    initial_uncertainty: float = Query(0.01, description="Initial uncertainty in observations")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error calculating predictability horizon: {str(e)}")
