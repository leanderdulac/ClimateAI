...
from services.lei_service import LEIResult, PropertyExposure, calculate_lei_score
...
async def calculate_lei_endpoint(
    ...
    exposed_value: float = Query(
        ..., gt=0, description="Exposed value of the property (reconstruction value)"
    ),
    property_type: str = Query(
        "residential",
        description="Property type: residential, commercial, industrial, agricultural, institutional",
    ),
    ...
    occupancy_factor: float = Query(
        1.0, ge=0, le=5, description="Occupancy-specific factor"
    ),
    ...
    custom_occupational_factor: float = Query(
        None, ge=0, le=5, description="Custom occupational factor"
    ),
    custom_gamma: float = Query(
        None,
        ge=0,
        le=5,
        description="Custom climate sensitivity coefficient γ (default 0.35)",
    ),
    custom_delta: float = Query(
        None,
        ge=0,
        le=5,
        description="Custom climate exponentiality coefficient δ (default 1.2)",
    ),
):
    """
    ...
    try:
        ...
        if property_type.lower() not in valid_property_types:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid property type. Valid options: {valid_property_types}",
            )
