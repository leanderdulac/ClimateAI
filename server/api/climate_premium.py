...
async def calculate_climate_drift_rate_endpoint(
    delta_temperature: float = Query(..., description="Temperature change from baseline ΔT_s (°C)"),
    co2_rate_change: float = Query(..., description="Rate of CO₂ change d(CO₂)/dt (ppm/year)"),
    delta_precipitation: float = Query(0.0, description="Precipitation change from baseline ΔP_s (mm/year)"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Drift rate calculation failed: {str(e)}")

@router.post("/climate-premium/climatic-inflation-factor")
async def calculate_climatic_inflation_factor_endpoint(
    time_horizon_years: float = Query(..., gt=0, description="Time horizon for calculation (t) in years"),
    initial_delta_temp: float = Query(1.0, description="Initial temperature change (ΔT_0) in °C"),
    temperature_trend: float = Query(0.2, description="Projected temperature trend (°C/year)"),
    initial_co2_rate: float = Query(2.5, description="Initial CO₂ rate change (ppm/year)"),
    ...
    try:
        ...
        scenario_func = create_default_climate_scenario(
            initial_delta_temp, temperature_trend, initial_co2_rate, co2_trend,
            0.0, 0.0  # Default precipitation values
        )
        ...
        raise HTTPException(status_code=500, detail=f"Climatic inflation factor calculation failed: {str(e)}")
...
async def calculate_climate_inclusive_premium_endpoint(
    ...
    time_horizon_years: float = Query(..., gt=0, description="Time horizon (t) in years"),
    ...
    mitigation_discount: float = Query(0.0, ge=0, lt=1, description="Mitigation discount (0 to 1)"),
    initial_delta_temp: float = Query(1.0, description="Initial temperature change (ΔT_0) in °C"),
    temperature_trend: float = Query(0.2, description="Projected temperature trend (°C/year)"),
    initial_co2_rate: float = Query(2.5, description="Initial CO₂ rate change (ppm/year)"),
    co2_trend: float = Query(0.1, description="Projected CO₂ rate trend (ppm/year)"),
    ...
    try:
        ...
        result = calculate_climate_inclusive_premium(
            expected_loss, time_horizon_years, loading_factor, operational_costs,
            mitigation_discount, initial_delta_temp, temperature_trend,
            initial_co2_rate, co2_trend
        )
        ...
        raise HTTPException(status_code=500, detail=f"Climate-inclusive premium calculation failed: {str(e)}")

@router.post("/climate-premium/multiple-scenarios")
async def calculate_premium_scenarios_endpoint(
    expected_losses: List[float] = Query(..., description="Expected losses for each scenario"),
    time_horizons: List[float] = Query(..., description="Time horizons for each scenario (years)"),
    loading_factors: List[float] = Query(..., description="Loading factors for each scenario"),
    mitigation_discounts: List[float] = Query(..., description="Mitigation discounts for each scenario"),
    operational_costs_list: Optional[List[float]] = Query(None, description="Operational costs for each scenario"),
    initial_delta_temps: List[float] = Query(..., description="Initial temperature changes for each scenario"),
    temperature_trends: List[float] = Query(..., description="Projected temperature trends for each scenario"),
    initial_co2_rates: List[float] = Query(..., description="Initial CO₂ rates for each scenario"),
    co2_trends: List[float] = Query(..., description="Projected CO₂ trends for each scenario")
):
    """
    Calculate climate-inclusive premiums for multiple scenarios
    """
    try:
        if not all(len(lst) == len(expected_losses) for lst in [
            time_horizons, loading_factors, mitigation_discounts, 
            initial_delta_temps, temperature_trends, initial_co2_rates, co2_trends]):
            raise HTTPException(
                status_code=400, 
        ...
        raise HTTPException(status_code=500, detail=f"Premium scenarios calculation failed: {str(e)}")
