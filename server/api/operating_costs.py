...
async def calculate_single_operating_costs_endpoint(
    ...
    coverage_type: str = Query(
        "property",
        description="Coverage type: 'property', 'liability', 'vehicle', etc.",
    ),
    ...
    automated_processing_enabled: bool = Query(
        True, description="Whether automated processing is enabled"
    ),
):
    """
    ...
    try:
        ...
        return {
            ...
            "cost_components_percentage": {
                "subscription_percentage": (
                    (result.subscription_cost / result.premium_issued) * 100
                    if result.premium_issued > 0
                    else 0
                ),
                "claims_percentage": (
                    (result.claims_cost / result.premium_issued) * 100
                    if result.premium_issued > 0
                    else 0
                ),
                "admin_percentage": (
                    (result.admin_cost / result.premium_issued) * 100
                    if result.premium_issued > 0
                    else 0
                ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Operating cost calculation failed: {str(e)}"
        )
...
async def portfolio_operating_costs_endpoint(
    ...
    processing_methods: List[str] = Query(
        None, description="List of processing methods (default: automated)"
    ),
    risk_categories: List[str] = Query(
        None, description="List of risk categories (default: standard)"
    ),
    coverage_types: List[str] = Query(
        None, description="List of coverage types (default: property)"
    ),
    policy_ages_months: List[int] = Query(
        None, description="List of policy ages in months (default: 0)"
    ),
    claim_history_counts: List[int] = Query(
        None, description="List of claim history counts (default: 0)"
    ),
    automated_processing_flags: List[bool] = Query(
        None, description="List of automated processing flags (default: True)"
    ),
):
    """
    ...
    try:
        ...
        if not all(
            len(lst) == n_policies for lst in [premium_values] if lst is not None
        ):
            raise HTTPException(
                status_code=400,
                detail="All required parameter lists must have the same length",
            )

        if n_policies == 0:
            raise HTTPException(
                status_code=400, detail="Must provide at least one policy"
            )
        ...
        if not all(
            ...
            for lst in [
                ...
                automated_processing_flags,
            ]
        ):
            raise HTTPException(
                status_code=400,
                detail="All optional parameter lists must have the same length as required lists",
            )
        ...
        return {
            ...
            "cost_percentage_summary": {
                "subscription_cost_percentage": (
                    (result.total_subscription_costs / result.total_premium_issued)
                    * 100
                    if result.total_premium_issued > 0
                    else 0
                ),
                "claims_cost_percentage": (
                    (result.total_claims_processing_costs / result.total_premium_issued)
                    * 100
                    if result.total_premium_issued > 0
                    else 0
                ),
                "admin_cost_percentage": (
                    (result.total_administration_costs / result.total_premium_issued)
                    * 100
                    if result.total_premium_issued > 0
                    else 0
                ),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Portfolio operating cost calculation failed: {str(e)}",
        )


@router.post("/operating-costs/efficiency-improvement-plan")
async def cost_efficiency_improvement_plan_endpoint(
    current_operating_cost: float = Query(
        ..., ge=0, le=1, description="Current operating cost ratio (0-1)"
    ),
    target_operating_cost: float = Query(
        ..., ge=0, le=1, description="Target operating cost ratio (0-1)"
    ),
    improvement_timeline_months: int = Query(
        12, ge=1, le=60, description="Timeline for improvement in months"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Cost efficiency improvement plan calculation failed: {str(e)}",
        )
...
async def calculate_breakeven_premium_endpoint(
    ...
    risk_factor_climatic: float = Query(
        0.1, ge=0, le=1, description="Climatic risk factor (0-1)"
    ),
    risk_factor_economic: float = Query(
        0.1, ge=0, le=1, description="Economic risk factor (0-1)"
    ),
    risk_factor_location: float = Query(
        0.1, ge=0, le=1, description="Location risk factor (0-1)"
    ),
    target_operating_margin: float = Query(
        0.1, ge=0, le=0.5, description="Target operating margin after costs (0-0.5)"
    ),
):
    """
    ...
    try:
        ...
        return {
            ...
            "premium_loading": (
                breakeven_premium - expected_claims
                if breakeven_premium > expected_claims
                else expected_claims * 0.2
            ),  # Estimated loading
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Breakeven premium calculation failed: {str(e)}"
        )
...
async def operating_costs_info():
    ...
    return {
        ...
        "risk_categories": {
            "low": {
                "multiplier": 0.8,
                "description": "Lower costs for low-risk policies",
            },
            "standard": {
                "multiplier": 1.0,
                "description": "Standard costs for typical policies",
            },
            "high": {
                "multiplier": 1.2,
                "description": "Higher costs for high-risk policies",
            },
            "special": {
                "multiplier": 1.5,
                "description": "Highest costs for special risk policies",
            },
        },
        "default_rates": {
