...
async def analyze_climate_risks_endpoint(
    scr_score: float = Query(500, ge=0, le=1000, description="Climate Risk Score (0-1000)"),
    ...
    concentration_risk: float = Query(0.2, ge=0, le=1, description="Concentration risk factor (0-1)"),
    mitigation_score: float = Query(0.6, ge=0, le=1, description="Mitigation effectiveness (0-1)"),
    model_confidence: float = Query(0.75, ge=0, le=1, description="Model confidence (0-1)"),
    historical_loss_ratio: float = Query(0.15, ge=0, le=1, description="Historical loss ratio (0-1)"),
    geographic_risk_factor: float = Query(0.5, ge=0, le=1, description="Geographic risk factor (0-1)"),
    seasonality_factor: float = Query(0.4, ge=0, le=1, description="Seasonal risk factor (0-1)")
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Climate risk analysis failed: {str(e)}")
...
async def calculate_premium_intelligent_endpoint(
    ...
    try:
        ...
        return {
            ...
            "premium_breakdown": {
                "base_premium_percentage": f"{(premium_calc.base_premium/premium_calc.final_premium)*100:.1f}%" if premium_calc.final_premium > 0 else "0%",
                "climate_loading_percentage": f"{(premium_calc.climate_loading/premium_calc.final_premium)*100:.1f}%" if premium_calc.final_premium > 0 else "0%",
                "uncertainty_loading_percentage": f"{(premium_calc.uncertainty_loading/premium_calc.final_premium)*100:.1f}%" if premium_calc.final_premium > 0 else "0%"
            },
            "risk_assessment": {
                "climate_risk_level": "high" if premium_calc.risk_adjusted_premium > premium_calc.base_premium * 1.5 else "medium" if premium_calc.risk_adjusted_premium > premium_calc.base_premium * 1.2 else "low",
                "confidence_level": "high" if premium_calc.confidence_score > 0.8 else "medium" if premium_calc.confidence_score > 0.6 else "low"
            },
            "recommendations": [
...
async def assess_claim_intelligent_endpoint(
    ...
    try:
        ...
        return {
            ...
            "risk_classification": {
                "validity_confidence": "high" if assessment.probability_valid > 0.8 else "medium" if assessment.probability_valid > 0.5 else "low",
                "fraud_risk_level": "high" if assessment.fraud_indicator > 0.7 else "medium" if assessment.fraud_indicator > 0.4 else "low",
                "investigation_urgency": "immediate" if assessment.investigation_priority <= 2 else "soon" if assessment.investigation_priority <= 3 else "routine"
            },
            "recommended_action": [
                "Pay" if assessment.probability_valid > 0.7 and assessment.fraud_indicator < 0.3 else 
                "Investigate Further" if assessment.investigation_priority <= 3 else 
                "Deny"
            ][0],
            "status": "completed"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Claim assessment failed: {str(e)}")

@router.post("/ia-agent/system-evaluation")
async def evaluate_system_performance_endpoint(
    premium_calculations_json: str = Query("[]", description="JSON string of premium calculations"),
    claim_assessments_json: str = Query("[]", description="JSON string of claim assessments"),
    ...
    try:
        ...
        avg_confidence = 0.75
        if premium_calcs:
            avg_vals = [pc.get('confidence_score', 0.7) for pc in premium_calcs if isinstance(pc, dict)]
            if avg_vals:
        ...
        evaluation = SystemEvaluation(
            ...
            improvement_areas=improvement_areas if improvement_areas else ['System performing well']
        )
        ...
        return {
            ...
            "metrics_summary": {
                ...
                "overall_performance_grade": "B+" if evaluation.system_performance_score >= 75 else "C+" if evaluation.system_performance_score >= 60 else "Needs Improvement"
            },
            "recommendations_summary": {
                "total_recommendations": len(evaluation.recommendations),
                "high_priority_items": len([r for r in evaluation.recommendations if r['priority'] == 'high']),
                "improvement_domains": evaluation.improvement_areas
            },
            "status": "completed"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"System evaluation failed: {str(e)}")
        ...
        "capabilities": [
            {
                ...
                "outputs": ["Composite risk score", "Factor importance", "Risk categories"]
            },
            {
            ...
            },
            {
                ...
                "inputs": ["Premium calculations", "Claim assessments", "Risk analyses"],
                "outputs": [
                    "Performance scores",
                    "Recommendations",
                    "Improvement areas"
