...
session_id_contextvar: ContextVar[Optional[str]] = ContextVar('session_id', default=None)
...
def setup_json_logging(app_name: str = "fimce", level: int = logging.INFO) -> logging.Logger:
    """
...
class StructuredLogger:
    ...
    def log_external_api_call(
        ...
        level = (
            logging.ERROR if status_code >= 500 or error
    ...
    def log_health_check_result(
        ...
        level = (
            logging.ERROR if status == 'unhealthy'
            else logging.WARNING if status == 'degraded'
            else logging.INFO
        )
