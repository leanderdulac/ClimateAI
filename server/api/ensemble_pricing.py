...
async def calculate_dynamic_weights_endpoint(
    ...
    prior_alpha: Optional[List[float]] = Query(None, description="Dirichlet prior parameters α")
):
    """
    ...
    try:
        if len(bics) != n_models:
            raise HTTPException(status_code=400, detail="BICs count must match n_models")
        
        if prior_alpha and len(prior_alpha) != n_models:
            raise HTTPException(status_code=400, detail="Prior alpha length must match n_models")
        ...
        raise HTTPException(status_code=500, detail=f"Dynamic weights calculation failed: {str(e)}")

@router.post("/ensemble-pricing/calculate")
async def calculate_ensemble_pricing_endpoint(
    model_premiums: List[float] = Query(..., description="Premiums from each individual model"),
    model_log_likelihoods: List[float] = Query(..., description="Log-likelihood for each model"),
    model_n_params: List[int] = Query(..., description="Number of parameters for each model"),
    model_n_observations: List[int] = Query(..., description="Number of observations for each model"),
    confidence_level: float = Query(0.95, ge=0.5, lt=1.0, description="Confidence level for VaR calculation"),
    dirichlet_alpha: Optional[List[float]] = Query(None, description="Dirichlet prior parameters α"),
    ...
    try:
        ...
        if not all(len(lst) == n_models for lst in [model_log_likelihoods, model_n_params, model_n_observations]):
            raise HTTPException(status_code=400, detail="All model parameter lists must have the same length")
        
        if dirichlet_alpha and len(dirichlet_alpha) != n_models:
            raise HTTPException(status_code=400, detail="Dirichlet alpha length must match number of models")
        
        if any(n <= 0 for n in model_n_params + model_n_observations):
            raise HTTPException(status_code=400, detail="Number of parameters and observations must be positive")
        
        result = calculate_ensemble_pricing(
            model_premiums, model_log_likelihoods, model_n_params, model_n_observations,
            n_models, confidence_level, dirichlet_alpha, bic_sensitivity, uncertainty_factor
        )
        
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Ensemble pricing calculation failed: {str(e)}")
...
async def update_model_performance_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Model performance update failed: {str(e)}")
...
async def get_historical_model_performance_endpoint(model_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Get model performance failed: {str(e)}")
