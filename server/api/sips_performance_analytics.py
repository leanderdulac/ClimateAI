...
async def create_performance_snapshot_endpoint(
    ...
    date: str = Query(
        datetime.now().isoformat(), description="Date of the snapshot (ISO format)"
    ),
    claim_rate: float = Query(
        ..., ge=0, le=1, description="Taxa de sinistralidade (0.0 to 1.0)"
    ),
    climate_loss_rate: float = Query(
        ..., ge=0, le=1, description="Sinistralidade climática (0.0 to 1.0)"
    ),
    net_margin: float = Query(
        ..., ge=0, le=1, description="Margem líquida (0.0 to 1.0)"
    ),
    rejection_rate: float = Query(
        ..., ge=0, le=1, description="Rejeições (0.0 to 1.0)"
    ),
    average_premium: float = Query(..., gt=0, description="Prêmio médio in currency"),
    client_retention: float = Query(
        ..., ge=0, le=1, description="Retenção clientes (0.0 to 1.0)"
    ),
    economic_capital: float = Query(
        ..., ge=0, description="Capital econômico in currency"
    ),
    baseline_period: str = Query(
        "custom",
        description="Period classification: before_sips, after_sips, monthly, quarterly",
    ),
):
    """
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Invalid date format. Use ISO format (e.g., 2023-01-01T00:00:00)",
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance snapshot creation failed: {str(e)}"
        )
...
async def calculate_performance_deltas_endpoint(
    ...
    try:
        # Get snapshots
        before_snapshot = sips_analytics_service.performance_snapshots.get(
            before_snapshot_id
        )
        after_snapshot = sips_analytics_service.performance_snapshots.get(
            after_snapshot_id
        )
        ...
        if not before_snapshot:
            raise HTTPException(
                status_code=404,
                detail=f"Before snapshot not found: {before_snapshot_id}",
            )
        if not after_snapshot:
            raise HTTPException(
                status_code=404, detail=f"After snapshot not found: {after_snapshot_id}"
            )
        ...
        return {
            ...
            "summary": {
                ...
                "improvement_percentage": (
                    (beneficial_changes / len(deltas) * 100) if deltas else 0
                ),
            },
            "analysis": {
                "positive_impacts": [
                    d["metric_name"] for d in formatted_deltas if d["improvement"]
                ],
                "negative_impacts": [
                    d["metric_name"] for d in formatted_deltas if not d["improvement"]
                ],
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance deltas calculation failed: {str(e)}"
        )
...
async def generate_performance_report_endpoint(
    ...
    try:
        # Get snapshots
        baseline_snapshot = sips_analytics_service.performance_snapshots.get(
            baseline_snapshot_id
        )
        current_snapshot = sips_analytics_service.performance_snapshots.get(
            current_snapshot_id
        )
        ...
        if not baseline_snapshot:
            raise HTTPException(
                status_code=404,
                detail=f"Baseline snapshot not found: {baseline_snapshot_id}",
            )
        if not current_snapshot:
            raise HTTPException(
                status_code=404,
                detail=f"Current snapshot not found: {current_snapshot_id}",
            )
        ...
        return {
            ...
            "report_summary": {
                ...
                "improvement_rate": (
                    sum(1 for d in report.deltas if d.improvement) / len(report.deltas)
                    if report.deltas
                    else 0
                ),
            },
    ...
    try:
        ...
        metric_analysis = {
            "taxa_sinistralidade": {
                ...
                "improvement_from_baseline": baseline_metrics[
                    PerformanceMetric.CLAIM_RATE
                ]
                - snapshot.claim_rate,
                "progress_to_target": (
                    (
                        baseline_metrics[PerformanceMetric.CLAIM_RATE]
                        - snapshot.claim_rate
                    )
                    / (
                        baseline_metrics[PerformanceMetric.CLAIM_RATE]
                        - target_metrics[PerformanceMetric.CLAIM_RATE]
                    )
                    if (
                        baseline_metrics[PerformanceMetric.CLAIM_RATE]
                        - target_metrics[PerformanceMetric.CLAIM_RATE]
            ...
            "sinistralidade_climatica": {
                ...
                "progress_to_target": (
                    (
                        baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
                        - snapshot.climate_loss_rate
                    )
                    / (
                        baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
                        - target_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
                    )
                    if (
                        baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
                        - target_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
            ...
            "margem_liquida": {
                ...
                "improvement_from_baseline": snapshot.net_margin
                - baseline_metrics[PerformanceMetric.NET_MARGIN],
                "progress_to_target": (
                    (
                        snapshot.net_margin
                        - baseline_metrics[PerformanceMetric.NET_MARGIN]
                    ...
                    / (
                        target_metrics[PerformanceMetric.NET_MARGIN]
                        - baseline_metrics[PerformanceMetric.NET_MARGIN]
                    )
                    if (
                        target_metrics[PerformanceMetric.NET_MARGIN]
        ...
        percentage_changes = {
            "claim_rate_improvement": (
                ...
                if baseline_metrics[PerformanceMetric.CLAIM_RATE] != 0
                else 0
            ),
            "climate_loss_improvement": (
                (
                    (
                        baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]
                        - snapshot.climate_loss_rate
                ...
                if baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE] != 0
                else 0
            ),
            "margin_improvement": (
                (
                ...
                if baseline_metrics[PerformanceMetric.NET_MARGIN] != 0
                else 0
            ),
        }
        ...
        return {
            ...
            "comparative_analysis": {
                "baseline_metrics": {
                    "taxa_sinistralidade": baseline_metrics[
                        PerformanceMetric.CLAIM_RATE
                    ],
                    "sinistralidade_climatica": baseline_metrics[
                        PerformanceMetric.CLIMATE_LOSS_RATE
                    ],
                ...
                "target_metrics": {
                    ...
                    "sinistralidade_climatica": target_metrics[
                        PerformanceMetric.CLIMATE_LOSS_RATE
                    ],
            ...
            "interpretation": {
                "score_rating": (
                    ...
                    if impact_score >= 80
                        ...
                        "good"
                        if impact_score >= 60
                        else "fair" if impact_score >= 40 else "needs_improvement"
                    )
                ),
                "impact_level": (
                    "high_positive"
                    if impact_score >= 85
                        ...
                        "positive"
                        if impact_score >= 70
                        else "neutral" if impact_score >= 50 else "negative"
                    )
                ),
            },
            "status": "calculated",
        ...
        raise HTTPException(
            status_code=500, detail=f"SIPS impact score calculation failed: {str(e)}"
        )
...
async def get_performance_trend_endpoint(
    metric_type: str,
    days: int = Query(
        90, ge=7, le=365, description="Number of days to analyze (7-365)"
    ),
):
    """
    ...
    try:
        ...
        if metric_type not in metric_mapping:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid metric type. Valid types: {list(metric_mapping.keys())}",
            )
        ...
        return {
            ...
            "interpretation": {
                ...
                "data_quality": (
                    "sufficient" if trend.get("data_points", 0) >= 3 else "limited"
                ),
                "statistical_reliability": (
                    "high"
                    if trend.get("data_points", 0) >= 10
                    else "moderate" if trend.get("data_points", 0) >= 5 else "low"
                ),
            },
            "status": "analyzed",
            "timestamp": datetime.now().isoformat(),
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance trend analysis failed: {str(e)}"
        )
...
async def get_dashboard_summary():
    ...
    try:
        ...
        claim_improvement = (
            (
                (earliest_snapshot.claim_rate - latest_snapshot.claim_rate)
                / earliest_snapshot.claim_rate
                * 100
            )
            if earliest_snapshot.claim_rate != 0
            else 0
        )
        climate_loss_improvement = (
            (
                (
                    earliest_snapshot.climate_loss_rate
                    - latest_snapshot.climate_loss_rate
                )
                / earliest_snapshot.climate_loss_rate
                * 100
            )
            if earliest_snapshot.climate_loss_rate != 0
            else 0
        )
        margin_improvement = (
            (
                (latest_snapshot.net_margin - earliest_snapshot.net_margin)
                / earliest_snapshot.net_margin
                * 100
            )
            if earliest_snapshot.net_margin != 0
            else 0
        )
        retention_improvement = (
            (
                (latest_snapshot.client_retention - earliest_snapshot.client_retention)
                / earliest_snapshot.client_retention
                * 100
            )
            if earliest_snapshot.client_retention != 0
            else 0
        )
        premium_growth = (
            (
                (latest_snapshot.average_premium - earliest_snapshot.average_premium)
                / earliest_snapshot.average_premium
                * 100
            )
            if earliest_snapshot.average_premium != 0
            else 0
        )
        ...
        stats = {
            "claim_rate": {
                ...
                "trend": (
                    "decreasing"
                    if all_claim_rates[-1] < all_claim_rates[0]
                    else "increasing"
                ),
            },
            "climate_loss_rate": {
                ...
                "trend": (
                    "decreasing"
                    if all_climate_losses[-1] < all_climate_losses[0]
                    else "increasing"
                ),
            },
            "net_margin": {
                ...
                "trend": (
                    "increasing"
                    if all_net_margins[-1] > all_net_margins[0]
                    else "decreasing"
                ),
            },
            "average_premium": {
                ...
                "trend": (
                    "increasing" if all_premiums[-1] > all_premiums[0] else "decreasing"
                ),
            },
        }
        ...
        return {
            "dashboard_summary": {
                "period_analyzed": {
                    ...
                    "days_spanned": (
                        latest_snapshot.date - earliest_snapshot.date
                    ).days,
                },
                "current_metrics": {
        ...
        raise HTTPException(
            status_code=500, detail=f"Dashboard summary generation failed: {str(e)}"
        )
