...
async def calculate_climate_capital_charge_endpoint(
    policy_ids: List[str] = Query(..., description="List of policy IDs in the portfolio"),
    premium_values: List[float] = Query(..., description="List of premium values for each policy"),
    climate_risk_scores: List[float] = Query(..., description="List of climate risk scores for each policy (0-1 scale)"),
    climate_correlations: List[float] = Query(..., description="List of climate correlations for each policy (0-1 scale)"),
    expected_climate_losses: List[float] = Query(..., description="List of expected climate losses for each policy"),
    climate_var_99_values: List[float] = Query(..., description="List of climate VaR 99% values for each policy"),
    climate_scenario_impacts: List[float] = Query(..., description="List of climate scenario impact factors for each policy"),
    climate_scenario: str = Query("moderate_warming", description="Climate scenario: baseline, moderate_warming, severe_warming, extreme_events, transition_shock"),
    stress_level: str = Query("moderate_stress", description="Stress level: low_stress, moderate_stress, high_stress, extreme_stress"),
    unmodeled_reserve_rate: float = Query(0.045, ge=0.03, le=0.06, description="Additional reserve rate for unmodeled climate events (3-6% of premium)")
):
    """
    ...
    try:
        ...
        if not all(len(lst) == n_policies for lst in [
            premium_values, climate_risk_scores, climate_correlations, 
            expected_climate_losses, climate_var_99_values, climate_scenario_impacts
        ]):
            raise HTTPException(
                status_code=400,
        ...
        if n_policies == 0:
            raise HTTPException(status_code=400, detail="At least one policy is required")
        ...
        result = calculate_climate_capital_charge(policies, climate_scenario, stress_level, unmodeled_reserve_rate)

        # Calculate individual policy contributions to portfolio risk
        individual_contributions = []
        total_portfolio_premium = sum(premium_values)
        for i, policy in enumerate(policies):
            premium_percentage = policy.premium_value / total_portfolio_premium if total_portfolio_premium > 0 else 0
            expected_loss_percentage = policy.expected_climate_loss / sum(expected_climate_losses) if sum(expected_climate_losses) > 0 else 0
        ...
        return {
            ...
            'capital_efficiency_ratio': result.climate_capital_charge / result.portfolio_premium if result.portfolio_premium > 0 else 0,
            'reserve_efficiency_ratio': result.total_climate_reserves / result.portfolio_premium if result.portfolio_premium > 0 else 0
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Climate capital charge calculation failed: {str(e)}")
...
async def calculate_portfolio_climate_var_endpoint(
    ...
    climate_risk_scores: List[float] = Query(..., description="List of climate risk scores (0-1)"),
    climate_correlations: List[float] = Query(..., description="List of climate correlations (0-1)"),
    expected_climate_losses: List[float] = Query(..., description="List of expected climate losses"),
    climate_scenario: str = Query("moderate_warming", description="Climate scenario to consider"),
    ...
    try:
        n_policies = len(policy_ids)
        if not all(len(lst) == n_policies for lst in [
            premium_values, climate_risk_scores, climate_correlations, expected_climate_losses
        ]):
        ...
        for i in range(n_policies):
            policy = PolicyClimateRisk(
                ...
                climate_correlation=climate_correlations[i],
                expected_climate_loss=expected_climate_losses[i],
                climate_var_99=expected_climate_losses[i] * 1.5,  # Placeholder VaR value
                climate_scenario_impact=climate_risk_scores[i]  # Placeholder impact
            )
            policies.append(policy)
        
        # Calculate portfolio climate VaR
        var_result = calculate_portfolio_climate_var(policies, climate_scenario, stress_level)
        ...
        return {
            ...
            'var_to_premium_ratio': var_result.var_99_portfolio / var_result.portfolio_premium if var_result.portfolio_premium > 0 else 0
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Portfolio climate VaR calculation failed: {str(e)}")

@router.post("/climate-capital-charge/unmodeled-events-reserve")
async def calculate_unmodeled_events_reserve_endpoint(
    total_portfolio_premium: float = Query(..., gt=0, description="Total portfolio premium amount"),
    reserve_rate: float = Query(0.045, ge=0.03, le=0.06, description="Reserve rate for unmodeled climate events (3-6% of premium)")
):
    """
    ...
    """
    try:
        reserve_amount = calculate_unmodeled_events_reserve(total_portfolio_premium, reserve_rate)
        ...
        raise HTTPException(status_code=500, detail=f"Unmodeled events reserve calculation failed: {str(e)}")
...
async def calculate_climate_reserves_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Climate reserves calculation failed: {str(e)}")
...
async def portfolio_stress_test_endpoint(
    ...
    stress_scenario: str = Query("extreme_events", description="Stress climate scenario"),
    unmodeled_reserve_rate: float = Query(0.045, ge=0.03, le=0.06, description="Additional reserve rate for unmodeled climate events (3-6% of premium)")
):
    """
    ...
    try:
        ...
        stress_result = perform_portfolio_stress_test(policies, baseline_scenario, stress_scenario, unmodeled_reserve_rate)
        
        return stress_result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Portfolio stress test failed: {str(e)}")
...
async def optimize_climate_reserves_endpoint(
    ...
    expected_climate_losses: List[float] = Query(..., description="List of expected climate losses"),
    climate_scenario: str = Query("moderate_warming", description="Climate scenario to consider"),
    target_climate_capital_charge: float = Query(0.0, description="Target climate capital charge"),
    unmodeled_reserve_rate: float = Query(0.045, ge=0.03, le=0.06, description="Additional reserve rate for unmodeled climate events (3-6% of premium)")
):
    """
    ...
    try:
        ...
        optimization_result = optimize_climate_reserves(policies, climate_scenario, target_climate_capital_charge, unmodeled_reserve_rate)
        
        return optimization_result
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Climate reserves optimization failed: {str(e)}")
