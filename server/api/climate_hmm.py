...
async def compute_regime_transition_probabilities_endpoint(
    current_forcing: List[float] = Query(
        ..., description="Climate forcing vector [CO₂, CH₄, aerosols]"
    ),
    previous_temperatures: List[float] = Query(
        ..., description="Recent temperature history"
    ),
    ...
    try:
        if len(current_forcing) != 3:
            raise HTTPException(
                status_code=400,
                detail="Climate forcing must have 3 values: [CO₂, CH₄, aerosols]",
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Regime transition probability calculation failed: {str(e)}",
        )


@router.post("/climate-hmm/emission-probabilities")
async def compute_emission_probabilities_endpoint(
    observations: List[float] = Query(
        ..., description="Observed climate data [temp, precip, pressure, ...]"
    ),
    current_forcing: List[float] = Query(
        ..., description="Climate forcing vector [CO₂, CH₄, aerosols]"
    ),
    ...
    try:
        ...
        result = compute_emission_probabilities(observations, current_forcing, n_states)
        return result
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Emission probability calculation failed: {str(e)}"
        )


@router.post("/climate-hmm/climate-regime-model")
async def compute_climate_regime_model_endpoint(
    climate_observations: List[
        List[float]
    ],  # Sequence of [temperature, precipitation, pressure, ...]
    ...
    try:
        if not climate_observations or not climate_forcings:
            raise HTTPException(
                status_code=400,
                detail="Climate observations and forcings cannot be empty",
            )

        if len(climate_observations) != len(climate_forcings):
            raise HTTPException(
                status_code=400,
                detail="Observations and forcings must have the same length",
            )
        ...
        for forcing in climate_forcings:
            if len(forcing) != 3:
                raise HTTPException(
                    status_code=400,
                    detail="Each forcing vector must have 3 values: [CO₂, CH₄, aerosols]",
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Climate regime model calculation failed: {str(e)}"
        )
