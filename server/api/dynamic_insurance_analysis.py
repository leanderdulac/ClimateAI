...
async def calculate_dynamic_premium_endpoint(
    ...
    base_loading_factor: float = Query(
        0.20, ge=0, le=1, description="Base loading factor"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Premium calculation failed: {str(e)}"
        )
...
async def add_policy_data_endpoint(
    ...
    duration_months: int = Query(
        12, ge=1, le=120, description="Policy duration in months"
    ),
    ...
    try:
        ...
        return {"message": "Policy data added successfully", "policy_id": policy_id}
    except Exception as e:
        raise HTTPException(
            status_code=400, detail=f"Failed to add policy data: {str(e)}"
        )
...
async def get_dynamic_pricing_factors_endpoint(risk_profile: Dict[str, float] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to get pricing factors: {str(e)}"
        )
