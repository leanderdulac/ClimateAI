...
async def prepare_climate_features_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Feature preparation failed: {str(e)}"
        )
...
async def train_lstm_attention_endpoint(
    ...
    enso_phase: List[float],
    targets: List[float],
    sequence_length: int = Query(
        10, ge=3, le=50, description="Length of input sequences"
    ),
    ...
    validation_split: float = Query(
        0.2, ge=0.0, le=0.5, description="Validation split fraction"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"LSTM attention model training failed: {str(e)}"
        )
...
async def predict_lstm_attention_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"LSTM attention prediction failed: {str(e)}"
        )
...
async def predict_with_attention_visualization_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Attention visualization prediction failed: {str(e)}",
        )
