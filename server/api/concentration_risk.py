...
async def calculate_concentration_risk_endpoint(
    property_ids: List[str] = Query(
        ..., description="List of property IDs in the cluster"
    ),
    premium_values: List[float] = Query(
        ..., description="List of premium values for each property"
    ),
    latitudes: List[float] = Query(
        ..., description="List of latitude values for each property"
    ),
    longitudes: List[float] = Query(
        ..., description="List of longitude values for each property"
    ),
    coverage_types: List[str] = Query(
        ..., description="List of coverage types for each property"
    ),
    asset_values: List[float] = Query(
        ..., description="List of asset values for each property"
    ),
    construction_types: List[str] = Query(
        ..., description="List of construction types for each property"
    ),
    elevations: List[float] = Query(
        ..., description="List of elevation values (meters) for each property"
    ),
    climate_zones: List[str] = Query(
        ..., description="List of climate zones for each property"
    ),
    climate_correlation: float = Query(
        None, ge=0, le=1, description="Climate correlation factor (ρ_climático)"
    ),
    hazard_type: str = Query(
        "flood", description="Type of hazard: flood, wind, fire, hail, drought"
    ),
    neighborhood_radius: float = Query(
        5.0, gt=0, description="Neighborhood radius in km for correlation calculation"
    ),
):
    """
    ...
    try:
        ...
        if len(property_ids) == 0:
            raise HTTPException(
                status_code=400, detail="At least one property is required"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Concentration risk calculation failed: {str(e)}"
        )
...
async def identify_clusters_endpoint(
    ...
    elevations: List[float] = Query(
        ..., description="List of elevation values (meters)"
    ),
    climate_zones: List[str] = Query(..., description="List of climate zones"),
    min_cluster_size: int = Query(
        3, ge=1, description="Minimum number of properties to form a cluster"
    ),
    neighborhood_radius: float = Query(
        5.0, gt=0, description="Neighborhood radius in km for clustering"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Cluster identification failed: {str(e)}"
        )
...
async def calculate_concentration_by_coverage_type_endpoint(
    ...
    climate_zones: List[str] = Query(..., description="List of climate zones"),
    coverage_type_filter: str = Query(
        ..., description="Coverage type to calculate concentration risk for"
    ),
    climate_correlation: float = Query(
        None, ge=0, le=1, description="Climate correlation factor"
    ),
    hazard_type: str = Query(
        "flood", description="Type of hazard for correlation calculation"
    ),
    min_cluster_size: int = Query(
        3, ge=1, description="Minimum number of properties to form a cluster"
    ),
    neighborhood_radius: float = Query(
        5.0, gt=0, description="Neighborhood radius in km"
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Coverage type concentration calculation failed: {str(e)}",
        )
...
async def calculate_aggregate_concentration_risk_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Aggregate concentration risk calculation failed: {str(e)}",
        )
