...
class ExtremeValueService:
    ...
    def fit_gev_distribution(
        self, data: List[float], return_period: float = 50.0
    ) -> GEVParameters:
        """
        ...
        def gev_neg_log_likelihood(params):
            ...
            if sigma <= 0:
                return np.inf
            return -np.sum(
                stats.genextreme.logpdf(data_array, -xi, loc=mu, scale=sigma)
            )
        ...
        if xi != 0:
            return_level = mu - (sigma / xi) * (
                1 - ((-np.log(1 - 1 / return_period)) ** (-xi))
            )
        else:
            # Gumbel case (ξ = 0)
            return_level = mu - sigma * np.log(-np.log(1 - 1 / return_period))

        # Calculate confidence intervals using Delta Method with Hessian inverse
        if hasattr(result, "hess_inv"):
            # Get covariance matrix from inverse Hessian
            # Note: minimize minimizes negative log-likelihood, so hess_inv is approx covariance
            try:
                cov_matrix = result.hess_inv
                if not isinstance(cov_matrix, np.ndarray):
                    cov_matrix = cov_matrix.todense()

                # Calculate gradients of return level w.r.t parameters [mu, sigma, xi]
                # z_T = mu - (sigma / xi) * (1 - y^(-xi)) where y = -log(1 - 1/T)
                y = -np.log(1 - 1 / return_period)

                if abs(xi) > 1e-5:
                    # Gradients for GEV case
                    m = -(1 - y ** (-xi)) / xi
                    grad_mu = 1.0
                    grad_sigma = m
                    grad_xi = (sigma / xi**2) * (1 - y ** (-xi)) - (sigma / xi) * (
                        y ** (-xi) * np.log(y)
                    )
                else:
                    # Gradients for Gumbel case (limit xi -> 0)
                    # z_T = mu - sigma * log(y)
                    grad_mu = 1.0
                    grad_sigma = -np.log(y)
                    grad_xi = 0.0  # Approximation near 0

                grads = np.array([grad_mu, grad_sigma, grad_xi])

                # Variance of return level = grad^T * Cov * grad
                var_return = grads.T @ cov_matrix @ grads
                se_return = np.sqrt(max(0, var_return))
            except Exception as e:
                logger.warning(f"Could not calculate precise CI: {e}")
                # Fallback to approximation
                se_return = sigma * np.sqrt(1.1) / np.sqrt(len(data))
        else:
            # Fallback if hess_inv not available
            se_return = sigma * np.sqrt(1.1) / np.sqrt(len(data))
    ...
    def gev_distribution_cdf(
        self, z: float, mu: float, sigma: float, xi: float
    ) -> float:
        """
    ...
    def calculate_climate_adapted_gev_params(
        self,
        ...
        mu_t = base_params.location * (
            1 + alpha * delta_temperature + beta * delta_precipitation
        )
    ...
    def calculate_return_level_with_climate_adaptation(
        self,
        base_params: GEVParameters,
        delta_temperature: float,
        delta_precipitation: float,
        ...
        if adapted_params.shape != 0:
            ...
            # Gumbel case (ξ = 0)
            adapted_return_level = (
                adapted_params.location
        ...
        if base_params.shape != 0:
            ...
            base_return_level = base_params.location - base_params.scale * np.log(
                -np.log(1 - 1 / return_period)
            )
    ...
    def fit_gpd_distribution(
        ...
        if len(exceedances) < 5:
            raise ValueError(
                "Need at least 5 exceedances above threshold for GPD fitting"
            )
    ...
    def calculate_extreme_event_probability(
        self, data: List[float], threshold: float, event_magnitude: float
    ) -> Dict[str, float]:
        """
        ...
        if gpd_params.shape != 0:
            gpd_prob = 1 - (1 + gpd_params.shape * excess / gpd_params.scale) ** (
                -1 / gpd_params.shape
            )
        else:
    ...
    def block_maxima_analysis(
        self, time_series: List[float], block_size: int = 365
    ) -> Dict[str, Any]:
        """
        ...
        if len(time_series) < block_size * 2:  # Need at least 2 blocks
            raise ValueError(
                f"Need at least {2 * block_size} data points for block maxima analysis"
            )

        # Extract block maxima
        blocks = [
            time_series[i : i + block_size]
        ...
        for t in return_periods:
            if gev_params.shape != 0:
                level = gev_params.location - (gev_params.scale / gev_params.shape) * (
                    1 - ((-np.log(1 - 1 / t)) ** (-gev_params.shape))
                )
            else:
                level = gev_params.location - gev_params.scale * np.log(
                    -np.log(1 - 1 / t)
                )
    ...
    def peaks_over_threshold_analysis(
        self, data: List[float], threshold: float, r: float = 0.25
    ) -> Dict[str, Any]:
        """
        ...
        thresholds_for_validation = np.linspace(
            np.percentile(data, 70), np.percentile(data, 95), 10
        )
        mean_excess_values = []
    ...
    def combined_gev_gpd_analysis(
        self, time_series: List[float], threshold: float = None
    ) -> Dict[str, Any]:
        """
        ...
        alpha = 0.995
        if gpd_shape != 0:
            var_995 = threshold + (gpd_scale / gpd_shape) * (
                ((exceedance_prob / (1 - alpha)) ** gpd_shape) - 1
            )
        else:
    ...
    def get_extreme_event_return_period(
        self, data: List[float], event_value: float, method: str = "gpd"
    ) -> Dict[str, float]:
        """
        ...
        if method == "gev":
            ...
            if gev_params.shape != 0:
                ...
                gev_cdf = np.exp(
                    -((1 + gev_params.shape * z) ** (-1 / gev_params.shape))
                )
            else:
                z = (event_value - gev_params.location) / gev_params.scale
            ...
            if gpd_params.shape != 0:
                gpd_cdf = 1 - (1 + gpd_params.shape * excess / gpd_params.scale) ** (
                    -1 / gpd_params.shape
                )
            else:
...
def block_maxima_analysis(
    time_series: List[float], block_size: int = 365
) -> Dict[str, Any]:
    """Perform block maxima analysis using GEV distribution"""
...
def combined_gev_gpd_analysis(
    time_series: List[float], threshold: float = None
) -> Dict[str, Any]:
    """Combined GEV and GPD analysis for comprehensive extreme value modeling"""
