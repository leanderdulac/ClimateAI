...
class CapitalSurplusCalculator:
    ...
    def __init__(self):
        ...
        self.default_z_alpha = stats.norm.ppf(0.95)  # Calculate 1.645 for 95% confidence
        # Climate sensitivity factor
        self.climate_sensitivity_factor = 0.001  # Adjusts impact of SCR on climate variance
    ...
    def calculate_climate_variance(self, scr_score: float, climate_lambda: Optional[float] = None) -> float:
        """
    ...
    def calculate_total_loss_variance(self, loss_params: LossDistributionParameters,
                                    climate_variance: float) -> float:
        """
        ...
        base_variance = loss_params.expected_loss_squared - (loss_params.expected_loss ** 2)
    ...
    def calculate_capital_surplus(self, 
                                loss_params: LossDistributionParameters,
                                climate_params: ClimateRiskParameters,
                                confidence_level: Optional[float] = None) -> CapitalSurplusResult:
        """
        ...
        climate_variance = self.calculate_climate_variance(
            climate_params.scr_score,
            climate_params.climate_loss_probability
        )

        # Calculate total loss variance
        total_variance = self.calculate_total_loss_variance(loss_params, climate_variance)
    ...
    def estimate_loss_distribution_parameters(self,
                                            physical_risk: float,
                                            transition_risk: float,
                                            concentration_risk: float,
                                            mitigation_effect: float = 0.0,
                                            correlation_adjustment: bool = True) -> LossDistributionParameters:
        """
    ...
    def calculate_capital_surplus_from_risks(self,
                                           physical_risk: float,
                                           transition_risk: float,
                                           concentration_risk: float,
                                           scr_score: float,
                                           ...
                                           confidence_level: Optional[float] = 0.95) -> CapitalSurplusResult:
        """
        ...
        climate_params = ClimateRiskParameters(
            scr_score=scr_score,
            climate_loss_probability=climate_lambda if climate_lambda is not None else self.default_climate_lambda
        )

        # Calculate capital surplus
        cs_result = self.calculate_capital_surplus(loss_params, climate_params, confidence_level)

        return cs_result
    
    def calculate_portfolio_capital_surplus(self,
                                          ...
                                          climate_lambda: Optional[float] = None,
                                          confidence_level: Optional[float] = 0.95) -> Dict[str, Any]:
        """
        ...
        if len(portfolio_scr_scores) != n_policies or len(portfolio_mitigation_effects) != n_policies:
            raise ValueError("All risk, SCR, and mitigation lists must have the same length")
        ...
        return {
            ...
            'climate_lambda': climate_lambda if climate_lambda is not None else self.default_climate_lambda,
            'calculation_timestamp': datetime.now().isoformat(),
    ...
    def optimize_capital_efficiency(self,
                                  ...
                                  scr_score: float,
                                  climate_lambda: Optional[float] = None,
                                  confidence_level: Optional[float] = 0.95) -> Dict[str, float]:
        """
        ...
        for _ in range(max_iterations):
            ...
            result = self.calculate_capital_surplus_from_risks(
                physical_risk, transition_risk, concentration_risk,
                scr_score, mid_mitigation, climate_lambda, confidence_level
            )
        ...
        return {
            ...
            'difference': abs(optimized_result.capital_surplus - target_capital_surplus),
            'confidence_level_used': confidence_level,
            'climate_lambda_used': climate_lambda if climate_lambda is not None else self.default_climate_lambda,
            'expected_loss_with_optimization': optimized_result.expected_loss,
            'variance_with_optimization': optimized_result.variance_loss,
            'optimization_success': abs(optimized_result.capital_surplus - target_capital_surplus) < tolerance
        }
...
def calculate_capital_surplus_from_risks(
    ...
    mitigation_effect: float = 0.0,
    climate_lambda: Optional[float] = None,
    confidence_level: float = 0.95) -> CapitalSurplusResult:
    ...
    return cs_calculator.calculate_capital_surplus_from_risks(
        physical_risk, transition_risk, concentration_risk,
        scr_score, mitigation_effect, climate_lambda, confidence_level
    )
...
def calculate_portfolio_capital_surplus(
    ...
    return cs_calculator.calculate_portfolio_capital_surplus(
        portfolio_risks, portfolio_scr_scores, portfolio_mitigation_effects,
        portfolio_weights, climate_lambda, confidence_level
    )
...
def optimize_capital_efficiency(
    ...
    return cs_calculator.optimize_capital_efficiency(
        target_capital_surplus, physical_risk, transition_risk, 
        concentration_risk, scr_score, climate_lambda, confidence_level
    )