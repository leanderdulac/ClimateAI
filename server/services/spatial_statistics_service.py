...
class SpatialStatisticsService:
    ...
    def calculate_kernel_density_estimation(self, coordinates: List[Tuple[float, float]], 
                                          values: List[float], 
        ...
        if values is not None and len(values) == len(coordinates):
            # Weight coordinates by their associated values
            weighted_coords = []
            for coord, val in zip(coords_array, values):
                # Repeat coordinate based on value to weight it
                weight = max(1, int(val * 10))  # Scale values to integer weights
                for _ in range(weight):
                    weighted_coords.append(coord)
            weighted_coords = np.array(weighted_coords)
            kde.fit(weighted_coords)
            
            # Get density for original coordinates
    ...
    def calculate_spatial_correlation(self, coordinates: List[Tuple[float, float]], 
                                   values: List[float], 
        ...
        if len(coordinates) < 2 or len(coordinates) != len(values):
            raise ValueError("Need same number of coordinates and values, at least 2 points")
        ...
        if len(valid_value_pairs) > 1:
            spatial_corr = np.corrcoef(valid_value_pairs[:, 0], valid_value_pairs[:, 1])[0, 1]
    ...
    def _calculate_correlation_by_distance(self, coords: np.ndarray, values: np.ndarray, 
                                         max_distance: float) -> Dict[float, float]:
        """
        ...
        for i in range(len(bins) - 1):
            ...
            # Get value pairs within this distance range
            valid_pairs = []
            n = len(coords)
            for x in range(n):
                for y in range(x+1, n):
                    if mask[x, y]:
                        valid_pairs.append([values[x], values[y]])
            
            if len(valid_pairs) > 1:
                pairs_array = np.array(valid_pairs)
                corr = np.corrcoef(pairs_array[:, 0], pairs_array[:, 1])[0, 1]
    ...
    def _estimate_correlation_range(self, correlation_by_distance: Dict[float, float]) -> float:
        """
    ...
    def geospatial_clustering(self, coordinates: List[Tuple[float, float]], 
        ...
        for label in unique_labels:
            ...
            cluster_values = np.array(values)[mask] if values else np.ones(len(cluster_coords))
            ...
            cluster_info = {
                ...
                'density': float(len(cluster_coords) / max(1, self._calculate_cluster_area(cluster_coords)))
            }
    ...
    def calculate_exposure_density(self, coordinates: List[Tuple[float, float]], 
                                 asset_values: List[float],
                                 radius: float = 5.0) -> List[float]:
        """
        ...
        for i, coord in enumerate(coords_array):
            ...
            indices = tree.query_ball_point(coord, r=radius/111.0)  # Approximate km to degrees
    ...
    def combined_spatial_risk_assessment(self, coordinates: List[Tuple[float, float]], 
                                       asset_values: List[float],
        ...
        return {
            ...
            'combined_risk_scores': [m.kde_density * m.exposure_density for m in combined_metrics],
            'total_exposure': float(np.sum(asset_values)),
            'weighted_risk_score': float(np.average(risk_scores, weights=asset_values)),
            'timestamp': datetime.now().isoformat()
        }

    def spatial_gaussian_process_model(self, coordinates: List[Tuple[float, float]],
                                     observations: List[float],
                                     covariates: Optional[List[List[float]]] = None,
                                     nugget: float = 0.1,
                                     range_param: float = 1.0,
                                     variance_param: float = 1.0) -> Dict[str, Any]:
        """
        ...
        if covariates is not None and len(covariates) == len(obs_array):
            ...
            try:
                # β̂ = (X^T Σ^(-1) X)^(-1) X^T Σ^(-1) Z
                cov_inv = np.linalg.inv(cov_matrix)
                XtSx_inv = np.linalg.inv(X.T @ cov_inv @ X)
                beta_est = XtSx_inv @ X.T @ cov_inv @ obs_array

                # Residuals after removing covariate effect
                Z_trend_removed = obs_array - X @ beta_est

                # Re-estimate GP parameters from residuals
                # Update covariance matrix based on residuals
                cov_matrix_residual = cov_matrix.copy()

                # Update GP parameters based on residual variance
                residual_variance = np.var(Z_trend_removed)

            except np.linalg.LinAlgError:
                # If matrix is singular, use pseudoinverse
                cov_inv = np.linalg.pinv(cov_matrix)
                XtSx = X.T @ cov_inv @ X
                if np.linalg.det(XtSx) != 0:
                    XtSx_inv = np.linalg.inv(XtSx)
                    beta_est = XtSx_inv @ X.T @ cov_inv @ obs_array
                    Z_trend_removed = obs_array - X @ beta_est
                else:
                    # Use ordinary least squares if generalized is unstable
        ...
        try:
            ...
            for i in range(n):
                ...
                try:
                    ...
                    z_others = Z_trend_removed[mask] if covariates is None else Z_trend_removed[mask]
                    ...
                    predictions.append(float(pred_value[0]) if hasattr(pred_value, '__len__') else float(pred_value))
                    ...
                    prediction_variances.append(float(nugget))  # Uncertainty of nearest point
        ...
        return {
            ...
            'covariance_matrix': {
                'min_distance': float(np.min(distances[np.triu_indices_from(distances, k=1)]) if n > 1 else 0),
    ...
    def predict_at_new_locations(self, fitted_model: Dict[str, Any],
                                new_coordinates: List[Tuple[float, float]],
                                new_covariates: Optional[List[List[float]]] = None) -> Dict[str, Any]:
        """
        ...
        all_distances = self._haversine_distances(np.vstack([coords_array, new_coords_array]))
        ...
        for j in range(n_new):
            ...
            if new_covariates is not None and j < len(new_covariates):
                ...
                if len(beta_est) > 1:
                    new_x = np.append(new_x, np.zeros(len(beta_est) - 1))  # Pad with zeros
...
def calculate_kernel_density_estimation(coordinates: List[Tuple[float, float]],
                                     values: List[float],
                                     bandwidth: float = 0.5) -> np.ndarray:
    """Calculate Kernel Density Estimation for spatial exposure modeling"""
...
def calculate_spatial_correlation(coordinates: List[Tuple[float, float]],
                               values: List[float],
                               max_distance: float = 100.0) -> Dict[str, float]:
    """Calculate spatial correlation using geographic distances"""
...
def geospatial_clustering(coordinates: List[Tuple[float, float]],
                        values: List[float] = None) -> Dict[str, Any]:
    """Perform geospatial clustering to identify risk zones"""
    return spatial_statistics_service.geospatial_clustering(
        coordinates, values, eps=5.0, min_samples=3
    )

def combined_spatial_risk_assessment(coordinates: List[Tuple[float, float]],
                                   asset_values: List[float],
                                   risk_scores: List[float]) -> Dict[str, Any]:
    """Perform combined spatial risk assessment using all methods"""
...
def spatial_gaussian_process_model(coordinates: List[Tuple[float, float]],
                                 observations: List[float],
                                 covariates: Optional[List[List[float]]] = None,
                                 nugget: float = 0.1,
                                 range_param: float = 1.0,
...
def predict_at_new_locations(fitted_model: Dict[str, Any],
                           new_coordinates: List[Tuple[float, float]],
                           new_covariates: Optional[List[List[float]]] = None) -> Dict[str, Any]:
    """Predict at new locations using a fitted spatial GP model"""
