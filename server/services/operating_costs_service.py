...
class OperatingCostCalculator:
    ...
    def calculate_operating_costs(self, policy_details: PolicyDetails) -> OperatingCostResult:
        """
        ...
        if policy_details.claim_history_count > 0:
            ...
            additional_claim_cost = policy_details.claim_history_count * 50  # R$ 50 per previous claim
        ...
        cost_breakdown = {
            'subscription_cost_base': self.subscription_cost_automated if policy_details.processing_method.lower() == 'automated' else self.subscription_cost_manual,
    ...
    def calculate_portfolio_operating_costs(self, policies: List[PolicyDetails]) -> PortfolioOperatingCosts:
        """
        ...
        portfolio_cost_breakdown = {
            'total_subscription_percentage': (total_subscription_costs / total_premium * 100) if total_premium > 0 else 0,
            'total_claims_percentage': (total_claims_costs / total_premium * 100) if total_premium > 0 else 0,
            'total_admin_percentage': (total_admin_costs / total_premium * 100) if total_premium > 0 else 0,
            'cost_ratio_by_risk_category': self._calculate_cost_ratios_by_risk_category(policies),
            'cost_ratio_by_coverage_type': self._calculate_cost_ratios_by_coverage_type(policies)
        }
    ...
    def _calculate_cost_ratios_by_risk_category(self, policies: List[PolicyDetails]) -> Dict[str, float]:
    ...
    def _calculate_cost_ratios_by_coverage_type(self, policies: List[PolicyDetails]) -> Dict[str, float]:
        ...
        return {
            coverage_type: (data['costs'] / data['premium']) if data['premium'] > 0 else 0.0
            for coverage_type, data in type_totals.items()
        }

    def calculate_cost_efficiency_improvement(self, 
                                           current_operating_cost: float,
                                           target_operating_cost: float,
                                           improvement_timeline_months: int = 12) -> Dict[str, Any]:
        """
        ...
        if improvement_needed > 0.05:
            recommendations.append("Automate underwriting processes to reduce subscription costs")
            recommendations.append("Implement AI-powered fraud detection for claims processing")
            recommendations.append("Optimize administrative workflows and systems")
        elif improvement_needed > 0.02:
            recommendations.append("Review manual processing protocols to increase automation")
            recommendations.append("Audit claims processing procedures for efficiency gains")
            recommendations.append("Evaluate administrative cost centers for optimization")
        else:
        ...
        return {
            ...
            'timeline_months': improvement_timeline_months,
            'projected_operating_cost_after_improvement': current_operating_cost - improvement_needed
        }

    def calculate_breakeven_premium(self, risk_assessment: Dict[str, float],
                                  target_operating_margin: float = 0.10) -> float:
        """
...
def calculate_portfolio_operating_costs(policies: List[PolicyDetails]) -> PortfolioOperatingCosts:
...
def calculate_cost_efficiency_improvement(current_operating_cost: float,
                                       target_operating_cost: float,
                                       improvement_timeline_months: int = 12) -> Dict[str, Any]:
...
def calculate_breakeven_premium(risk_assessment: Dict[str, float],
                              target_operating_margin: float = 0.10) -> float:
    """Convenience function to calculate breakeven premium with operating costs"""
    return operating_cost_service.calculate_breakeven_premium(risk_assessment, target_operating_margin)