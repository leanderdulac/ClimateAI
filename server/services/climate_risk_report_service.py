...
class ClimateRiskReportGenerator:
    ...
    def generate_policy_analysis_report(self, 
                                       policy_id: str,
                                       ...
                                       mitigation_impact: Optional[Dict[str, float]] = None) -> PolicyAnalysisReport:
        """
        ...
        next_review = self._calculate_next_review_date(climate_projections.get('temperature_increase_2050', 1.8))

        # Calculate standard premium (without risk adjustments)
        standard_premium = expected_claims * 1.2  # Basic 20% loading for standard premium
        ...
        climate_breakdown = {
            ...
            'mitigation_effect': {
                'points_reduction': -abs(risk_components.mitigation_effect * 100),  # Convert to points reduction
    ...
    def _calculate_premium_breakdown(self, expected_claims: float, scr_score: float, zone_concentration: float) -> PremiumBreakdown:
        ...
        security_loading = expected_loss * self.component_weights['security_loading_rate']
        contingency_loading = expected_loss * self.component_weights['contingency_loading_rate']
        margin_loading = expected_loss * self.component_weights['margin_loading_rate']
        investment_return = expected_loss * self.component_weights['investment_return_rate']
        climate_change_loading = expected_loss * self.component_weights['climate_change_rate']
        
        # Capacity adjustment based on zone concentration
        capacity_adjustment = expected_loss * zone_concentration * self.component_weights['capacity_adjustment_rate']
    ...
    def _determine_policy_decision(self, scr_score: float, implemented_mitigation_measures: List[str]) -> Tuple[str, str]:
        ...
        if scr_score < self.decision_thresholds['approve']:
            ...
            if scr_score > self.decision_thresholds['condition'] - 50:  # Close to rejection threshold
                if 'pumping_system' not in implemented_mitigation_measures and scr_score > 650:
    ...
    def _generate_discount_opportunities(self, current_premium: float, 
                                      implemented_measures: List[str]) -> List[Dict[str, Any]]:
    ...
    def _calculate_potential_premium(self, current_premium: float, 
                                  discount_opportunities: List[Dict[str, Any]]) -> float:
        """Calculate potential premium with discount opportunities applied"""
        total_discount = sum(op['discount_amount'] for op in discount_opportunities[:2])  # Apply top 2 discounts
        return max(0, current_premium - total_discount)

    def _generate_alerts(self, risk_components: ClimateRiskComponents,
                        climate_projections: Dict[str, float],
                        zone_concentration: float) -> List[str]:
        ...
        alerts.append(f'Zona com projeção de aumento de sinistralidade {risk_increase}% até {year}')
        ...
        if temp_proj > 1.5:
            alerts.append(f'Atenção: Projeção de aumento térmico acima de 1.5°C ({temp_proj}°C)')
    ...
    def _calculate_next_review_date(self, temperature_projection: float) -> datetime:
        ...
        if temperature_projection > 2.0:
            next_review = datetime.now() + relativedelta(months=6)  # 6-month review for high temp increase
            
        return next_review

    def generate_policy_comparison_report(self, 
        ...
        for policy_data in policies:
            ...
            report = self.generate_policy_analysis_report(
                ...
                climate_projections=policy_data.get('climate_projections', {
                    'temperature_increase_2050': 1.8,
                    'risk_increase_percentage': 129.0,
                    'year': 2050
                }),
                implemented_mitigation_measures=policy_data.get('implemented_mitigation_measures', []),
                mitigation_impact=policy_data.get('mitigation_impact', {
                    'points_reduction': 70,
                    'measures_list': ['drenagem', 'sensores']
                })
            )
        ...
        return {
            ...
            'portfolio_summary': {
                'high_risk_policies': risk_distribution['HIGH'] + risk_distribution['CRITICAL'],
                'conditional_approvals': sum(1 for r in reports if r.decision == 'CONDICIONADA'),
                'recommended_actions': self._aggregate_portfolio_recommendations(reports)
            },
            'generation_timestamp': datetime.now().isoformat()
        }

    def _aggregate_portfolio_recommendations(self, reports: List[PolicyAnalysisReport]) -> List[str]:
        ...
        if high_risk_count > len(reports) * 0.3:  # More than 30% high risk
            recommendations.append('Avaliar restrição de aceitação para a região devido a alta concentração de risco')
        
        if conditional_count > 0:
            recommendations.append(f'Implementar {conditional_count} políticas condicionadas com exigências de mitigação')
        
        if len(reports) > 0:
            avg_zone_concentration = np.mean([r.component_analysis['capacity_adjustment'] / r.component_analysis['expected_loss'] * 100 
                                            if r.component_analysis['expected_loss'] > 0 else 0 
                                            for r in reports])
            if avg_zone_concentration > 25:
                recommendations.append(f'Reduzir concentração geográfica (atual média {avg_zone_concentration:.1f}%)')
...
def generate_policy_analysis_report(policy_id: str,
                                  risk_components: ClimateRiskComponents,
                                  expected_claims: float,
                                  coverage_amount: float,
                                  zone_concentration: float = 0.22,
    ...
    return climate_report_generator.generate_policy_analysis_report(
        policy_id, risk_components, expected_claims, coverage_amount,
        zone_concentration, climate_projections, implemented_mitigation_measures, 
        mitigation_impact
    )
