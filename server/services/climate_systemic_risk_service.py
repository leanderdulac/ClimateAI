...
class ClimateSystemicRiskService:
    ...
    def calculate_extreme_climate_event_probability(
        self, climate_data: Dict[str, List[float]], event_type: str = "compound"
    ) -> float:
        """
    ...
    def calculate_conditional_var(
        ...
        """
        if (
            len(portfolio_returns) != len(list(climate_data.values())[0])
            if climate_data
            else 0
        ):
        ...
        extreme_prob = self.calculate_extreme_climate_event_probability(
        ...
        if extreme_prob == 0:
            ...
            n_extreme_periods = max(
                1, int(len(portfolio_returns) * 0.1)
            )  # Assume 10% extreme periods
        ...
        for _ in range(n_bootstrap):
            ...
            boot_sample = np.random.choice(
                extreme_returns, size=len(extreme_returns), replace=True
            )
            boot_var = self.calculate_portfolio_vaR(
                boot_sample.tolist(), confidence_level
            )
    ...
    def calculate_climate_neutral_benchmark(
        self,
        n_assets: int = 10,
    ...
    def calculate_climate_loading(
        ...
        portfolio_vat = self.calculate_portfolio_vaR(
            portfolio_returns, confidence_level
        )
        benchmark_vat = self.calculate_portfolio_vaR(
            benchmark_returns, confidence_level
        )
    ...
    def calculate_systemic_climate_risk(
        self,
        portfolios_data: Dict[str, List[float]],  # Multiple portfolios
        climate_data: Dict[str, List[float]],
        confidence_levels: List[float] = [0.95, 0.99],
        event_types: List[str] = ["compound", "temperature", "precipitation"],
    ) -> Dict[str, Any]:
        """
        ...
        for event_type in event_types:
            prob = self.calculate_extreme_climate_event_probability(
                climate_data, event_type
            )
        ...
        for portfolio_name, returns in portfolios_data.items():
            ...
            portfolio_loadings[portfolio_name] = (
                np.mean(avg_loadings) if avg_loadings else 0.0
            )

        # Calculate systemic metrics
        results["systemic_metrics"] = {
            "portfolio_climate_loadings": portfolio_loadings,
            "max_portfolio_loading": (
                max(portfolio_loadings.values()) if portfolio_loadings else 0.0
            ),
            "avg_portfolio_loading": (
                np.mean(list(portfolio_loadings.values()))
                if portfolio_loadings
                else 0.0
            ),
            "total_systemic_exposure": (
                sum(portfolio_loadings.values()) if portfolio_loadings else 0.0
            ),
        }
        ...
        for portfolio_name, loading in portfolio_loadings.items():
            results["climate_loading_matrix"].append(
                {"portfolio": portfolio_name, "climate_loading": loading}
            )

        # Overall systemic risk as the maximum loading across portfolios
        results["overall_systemic_risk"] = results["systemic_metrics"][
            "max_portfolio_loading"
        ]
...
def calculate_extreme_climate_event_probability(
    climate_data: Dict[str, List[float]], event_type: str = "compound"
) -> float:
    """Calculate probability of extreme climate events"""
...
def calculate_climate_neutral_benchmark(
    n_assets: int = 10, time_horizon: int = 252, volatility_target: float = 0.15
) -> List[float]:
    """Generate hypothetical climate-neutral portfolio returns"""
...
def calculate_systemic_climate_risk(
    portfolios_data: Dict[str, List[float]],
    climate_data: Dict[str, List[float]],
    confidence_levels: List[float] = [0.95, 0.99],
    event_types: List[str] = ["compound", "temperature", "precipitation"],
) -> Dict[str, Any]:
