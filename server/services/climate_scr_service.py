...
class ClimateSCRService:
    ...
    def calculate_individual_scr(self, var_995_loss: float, expected_loss: float) -> float:
        """
    ...
    def create_correlation_matrix(self, n_events: int, correlation_value: float = 0.25) -> List[List[float]]:
        """
        ...
        for i in range(n_events):
            for j in range(n_events):
                if i == j:
                    ...
                    matrix[i][j] = correlation_value  # Default correlation between different perils
        
        return matrix
    
    def calculate_portfolio_scr(self, 
                              var_995_losses: List[float], 
                              expected_losses: List[float],
                              correlation_matrix: List[List[float]] = None) -> ClimateSCR:
        """
        ...
        if correlation_matrix is None:
            correlation_matrix = self.create_correlation_matrix(n_events, self.default_correlation)
        else:
            if len(correlation_matrix) != n_events or any(len(row) != n_events for row in correlation_matrix):
        ...
        for i in range(n_events):
            for j in range(n_events):
                total_sum += correlation_matrix[i][j] * individual_scrs[i] * individual_scrs[j]
        ...
        logger.info(f"Climate SCR calculated: {portfolio_scr:,.2f} (n_events: {n_events})")
        return result
    
    def calculate_simple_portfolio_scr(self,
                                     var_995_losses: List[float],
                                     expected_losses: List[float]) -> ClimateSCR:
        """
    ...
    def calculate_peril_specific_scr(self,
                                   peril_losses: Dict[str, Dict[str, float]]) -> ClimateSCR:
        """
...
def create_correlation_matrix(n_events: int, correlation_value: float = 0.25) -> List[List[float]]:
...
def calculate_portfolio_scr(var_995_losses: List[float], 
                          expected_losses: List[float],
                          correlation_matrix: List[List[float]] = None) -> ClimateSCR:
    """Calculate portfolio-level climate SCR using correlation-based formula"""
    return climate_scr_service.calculate_portfolio_scr(var_995_losses, expected_losses, correlation_matrix)

def calculate_simple_portfolio_scr(var_995_losses: List[float], expected_losses: List[float]) -> ClimateSCR:
    """Calculate portfolio SCR with default 0.25 correlation between different perils"""
    return climate_scr_service.calculate_simple_portfolio_scr(var_995_losses, expected_losses)

def calculate_peril_specific_scr(peril_losses: Dict[str, Dict[str, float]]) -> ClimateSCR:
