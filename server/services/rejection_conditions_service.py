...
class PolicyProfile:
    ...
    mitigation_measures_implemented: List[str]  # List of implemented mitigation measures
...
class RejectionConditionsEngine:
    ...
    def evaluate_hard_stops(self, policy: PolicyProfile) -> Dict[str, Any]:
        ...
        if (policy.zone_concentration_percentage > 0.35 and
            policy.scr_score > 600.0):
            reasons.append({
        ...
        if (policy.exposed_value > 10000000.0 and
            policy.scr_score > 700.0):
            reasons.append({
        ...
        recent_claims = [claim for claim in policy.climate_claim_history
                        if self._is_recent_claim(claim, 5)]
        claim_count = len(recent_claims)

        if (claim_count >= 3 and
            len(policy.mitigation_measures_implemented) == 0):
            reasons.append({
        ...
        if (policy.flood_zone_status == '100_year' and
            policy.projected_flood_increase > 1.5 and
            policy.drainage_system_capacity == 0):
            reasons.append({
        ...
        return {
            ...
            'hard_stop_rules_checked': {
                ...
                'cluster_concentration_rule': policy.zone_concentration_percentage > 0.35 and policy.scr_score > 600.0,
                'high_value_rule': policy.exposed_value > 10000000.0 and policy.scr_score > 700.0,
                'high_claim_history_rule': claim_count >= 3 and len(policy.mitigation_measures_implemented) == 0,
                'flood_zone_rule': policy.flood_zone_status == '100_year' and policy.projected_flood_increase > 1.5 and policy.drainage_system_capacity == 0
            }
        }
    ...
    def evaluate_soft_stops(self, policy: PolicyProfile) -> List[Dict[str, Any]]:
        ...
        wind_resistance_value = self.wind_resistance_classes.get(policy.wind_resistance_class, 0)
        ...
        if (policy.scr_score > 450 and 'wind' in policy.mitigation_measures_implemented):
            if wind_resistance_value < min_required_value or not policy.structural_anchoring_certified:
                conditions.append({
        ...
        if (policy.scr_score > 300 and 'fire' in policy.mitigation_measures_implemented):
            if (policy.fire_defensible_space_meters < 30.0 or 
                not policy.flame_retardant_coating_applied):
                conditions.append({
    ...
    def make_autonomous_decision(self, policy: PolicyProfile) -> RejectionConditionsResult:
        """
        ...
        if hard_stop_result['is_rejected']:
            return RejectionConditionsResult(
                ...
                is_rejected=True,
                rejection_reason=hard_stop_result['rejection_reasons'][0]['reason'],  # First reason
                conditions_required=[],
    ...
    def calculate_conditioning_probability(self, 
                                         unmet_conditions: List[Dict[str, Any]]) -> float:
        """
...
def calculate_policy_resilience_score(
    ...
    wind_value = rejection_conditions_service.wind_resistance_classes.get(wind_resistance_class, 0)
    return rejection_conditions_service.calculate_resilience_score(
        drainage_capacity, wind_value, fire_space, 
        fire_coating, structural_certification, mitigation_measures
    )
...
def calculate_conditioning_probability(unmet_conditions: List[Dict[str, Any]]) -> float:
    ...
    return rejection_conditions_service.calculate_conditioning_probability(unmet_conditions)