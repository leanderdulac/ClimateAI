...
class SmartClimateExclusionsService:
    ...
    def evaluate_climate_exclusions(self,
                                  policy_id: str,
                                  climate_risk_factors: Dict[str, Any],
                                  mitigation_status: Optional[Dict[str, Any]] = None,
                                  model_confidence: Optional[Dict[str, float]] = None) -> ExclusionDecision:
        """
        ...
        if model_confidence and "climate_induced_seismicity" in model_confidence:
            ...
            if confidence < self.low_confidence_threshold:
                ...
                decision_reasons.append(f"Climate-induced seismicity model confidence too low: {confidence:.2f} < {self.low_confidence_threshold}")
        ...
        if mitigation_status:
            if not self._check_mitigation_implementation(mitigation_status):
                ...
                decision_reasons.append("Client failed to implement required mitigation measures")
        
        # Evaluate climate litigation risk (still immature)
        litigation_maturity = climate_risk_factors.get("litigation_maturity", 0.2)  # Default low maturity
        if litigation_maturity < 0.4:  # Considered immature
            ...
            decision_reasons.append(f"Climate litigation models not mature enough: {litigation_maturity:.2f} < 0.4 threshold")
        ...
        logger.info(f"Exclusion evaluation completed for policy {policy_id}. "
                   f"{len(excluded_risks)} risks excluded, "
                   f"{len(governance_recommendations)} governance recommendations")
        
        return decision

    def _create_exclusion(self, policy_id: str, exclusion_type: ClimateExclusionType, model_confidence: float) -> ClimateExclusion:
        ...
        if definition["confidence_threshold"] > 0 and model_confidence < definition["confidence_threshold"]:
            coverage_status = "excluded"
    ...
    def _check_mitigation_implementation(self, mitigation_status: Dict[str, Any]) -> bool:
    ...
    def _get_governance_recommendations(self, scr_score: float) -> List[GovernanceRule]:
        ...
        for rule in self.governance_rules:
            ...
            if (rule.rule_type == GovernanceRecommendation.MANUAL_APPROVAL_REQUIRED 
                and scr_score > self.manual_approval_threshold):
                recommendations.append(rule)
            # Check if quarterly/annual review applies
            elif rule.rule_type in [GovernanceRecommendation.QUARTERLY_COMMITTEE_REVIEW, 
                                   GovernanceRecommendation.ANNUAL_EXTERNAL_AUDIT]:
                recommendations.append(rule)
        
        return recommendations

    def _calculate_premium_adjustment(self, excluded_risks: List[ClimateExclusion]) -> float:
    ...
    def get_upcoming_reviews(self) -> List[Tuple[str, datetime, str]]:
        ...
        for exclusion_id, exclusion in self.active_exclusions.items():
            if exclusion.review_date <= datetime.now() + timedelta(days=30):  # Next 30 days
                upcoming_reviews.append((exclusion_id, exclusion.review_date, "exclusion"))
        ...
        for policy_id, decision in self.governance_decisions.items():
            ...
            if decision.decision_timestamp.month % 3 == 0:  # Quarterly check
                ...
                if next_quarterly_review <= datetime.now() + timedelta(days=30):
                    upcoming_reviews.append((policy_id, next_quarterly_review, "quarterly_governance"))
...
def evaluate_climate_exclusions(policy_id: str,
                              climate_risk_factors: Dict[str, Any],
                              mitigation_status: Optional[Dict[str, Any]] = None,
                              model_confidence: Optional[Dict[str, float]] = None) -> ExclusionDecision:
