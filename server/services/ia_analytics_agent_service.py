...
class ClimateAnalyticsAgent:
    ...
    def __init__(self):
        ...
        self.system_evaluation_model = RandomForestRegressor(n_estimators=50, random_state=42)
    ...
    def _initialize_models(self):
        ...
        try:
            ...
            synthetic_data[:, 2] = synthetic_data[:, 2] * 10000  # Expected loss (0-10000)
            ...
            self.claim_model.fit(X_scaled[:-100], [p for p in claim_targets[:-100]])  # Reserve 100 for validation
            
            self.model_trained = True
            logger.info("Climate Analytics Agent initialized with synthetic training data")
        ...
        try:
            ...
            if hasattr(factors, 'scr_score'):  # ClimateRiskFactors
                ...
                factor_vector = [
                    ...
                    getattr(factors, 'climate_var_99', getattr(factors, 'climate_var_score', 10000)),
                    ...
                    factors.model_confidence,
                    getattr(factors, 'historical_loss_ratio', getattr(factors, 'claim_frequency', 0.15)),
                    factors.geographic_factor,  # Map to geographic_risk_factor
                    getattr(factors, 'seasonality_factor', getattr(factors, 'seasonality_factor', 0.4))
                ]
            else:
            ...
            for i, (factor_name, weight) in enumerate(self.risk_weights.items()):
                if i < len(factor_vector):
                    ...
                    if factor_name == 'scr_score':
                        ...
                        normalized_value = factor_value / 10000  # Assume 10k as typical max
                        weighted_value = normalized_value * weight
                    else:
            ...
            final_composite = composite_risk * (1 + (1 - factors.model_confidence) * 0.5)
            
            # Calculate risk categories
            risk_categories = {
                'financial_risk': factors.climate_var_99 / 10000,  # Normalize to 0-5 scale
            ...
            analysis_results = {
                ...
                'model_confidence': getattr(factors, 'model_confidence', getattr(factors, 'model_confidence', 0.75)),
            ...
            try:
                scr_score = getattr(factors, 'scr_score', getattr(factors, 'climate_risk_score', 500))
                climate_var_99 = getattr(factors, 'climate_var_99', getattr(factors, 'climate_risk_score', 10000) * 2)
                physical_risk = getattr(factors, 'physical_risk', getattr(factors, 'physical_risk', 0.4))
                transition_risk = getattr(factors, 'transition_risk', getattr(factors, 'transition_risk', 0.3))
                concentration_risk = getattr(factors, 'concentration_risk', getattr(factors, 'concentration_risk', 0.2))
                mitigation_score = getattr(factors, 'mitigation_score', getattr(factors, 'mitigation_effectiveness', 0.6))
                model_confidence = getattr(factors, 'model_confidence', getattr(factors, 'model_confidence', 0.75))
    ...
    def calculate_premium_intelligent(self, factors) -> PremiumCalculation:
        ...
        try:
            ...
            expected_loss = getattr(factors, 'expected_loss', getattr(factors, 'expected_claims', 2000))
            ...
            try:
                ...
                confidence_score = min(1.0, max(0.1,
            ...
            try:
                ...
                var_impact = (getattr(factors, 'climate_var_99', getattr(factors, 'climate_risk_score', 10000) * 2) / 50000) * 0.20
                physical_risk_impact = getattr(factors, 'physical_risk', getattr(factors, 'physical_risk', 0.4)) * 0.15
            ...
            try:
                ...
                final_premium = base_premium * (1 + physical_risk * 0.5) * (1 + transition_risk * 0.3)
    ...
    def assess_claim_intelligent(self, claim_amount: float, 
                               risk_factors: ClimateRiskFactors,
                               policy_history: Optional[Dict[str, Any]] = None) -> ClaimAssessment:
        """
        ...
        try:
            ...
            probability_valid = min(1.0, base_validity + physical_risk_contribution + transition_risk_contribution + mitigation_adjustment)
            
            # Calculate fraud indicator
            fraud_risk_factors = []
            
            # High claim amount relative to expected loss might indicate fraud
            expected_vs_claimed = claim_amount / (risk_factors.expected_loss + 1)  # +1 to avoid division by zero
            fraud_risk_factors.append(min(1.0, (expected_vs_claimed - 1) if expected_vs_claimed > 1 else 0))
            ...
            probability_valid = min(1.0, max(0.0, risk_factors.physical_risk + risk_factors.transition_risk))
    ...
    def evaluate_system_performance(self, 
                                  ...
                                  risk_analyses: List[Dict[str, Any]]) -> SystemEvaluation:
        """
        ...
        try:
            ...
            if premium_calculations:
                avg_confidence = sum(pc.confidence_score for pc in premium_calculations) / total_premiums
            ...
            if claim_assessments:
                avg_investigation_priority = sum(ca.investigation_priority for ca in claim_assessments) / total_claims
                valid_claims = sum(1 for ca in claim_assessments if ca.probability_valid > 0.5)
                fraud_detected = sum(1 for ca in claim_assessments if ca.fraud_indicator > 0.7)
                fraud_detection_rate = fraud_detected / total_claims if total_claims > 0 else 0.15
    ...
    def _calculate_factor_importance(self, factor_vector: List[float]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            importance_factors = {}
            for i, (factor_name, base_importance) in enumerate(default_importance.items()):
                if i < len(factor_vector):
                    factor_value = factor_vector[i]
                    # Importance increases with factor magnitude
                    adjusted_importance = base_importance * (1 + min(1.0, factor_value / 100))
...
def assess_claim_intelligent(claim_amount: float, 
                          risk_factors: ClimateRiskFactors,
                          policy_history: Optional[Dict[str, Any]] = None) -> ClaimAssessment:
    """Convenience function for intelligent claim assessment"""
    return climate_analytics_agent.assess_claim_intelligent(claim_amount, risk_factors, policy_history)
...
def evaluate_system_performance(
    ...
    return climate_analytics_agent.evaluate_system_performance(premium_calculations, claim_assessments, risk_analyses)