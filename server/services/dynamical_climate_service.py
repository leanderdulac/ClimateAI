...
class DynamicalClimateService:
    ...
    def initialize_climate_attractor(self, model_type: str = 'lorenz', 
                                   parameters: Optional[Dict[str, float]] = None) -> pds.ContinuousDynamicalSystem:
        """
        ...
        if parameters is None:
            ...
            if model_type == 'lorenz':
                ...
                parameters = {'alpha': -1.0, 'beta': 1.0, 'gamma': 0.3, 'omega': 1.2, 'delta': 0.3}
            else:
        ...
        if model_type == 'lorenz':
            ...
            try:
                ...
                raise ValueError(f"Model {model_type} system not available in pynamicalsys")
    ...
    def fit_climate_phase_space(self, climate_data: List[Dict[str, float]], 
                               target_var: str = 'temperature') -> Dict[str, Any]:
        """
        ...
        if n < 50:  # Need sufficient data for phase space reconstruction
            raise ValueError("Need at least 50 data points for phase space reconstruction")
    ...
    def _estimate_correlation_dimension(self, phase_space: np.ndarray) -> float:
        ...
        correlation_integrals = np.array(correlation_integrals) / (sample_size * (sample_size - 1) / 2)
    ...
    def _calculate_recurrence_rate(self, phase_space: np.ndarray) -> float:
        ...
        avg_distances = []
        for i in range(len(sample_space)):
            distances = [np.linalg.norm(sample_space[i] - sample_space[j]) 
                         for j in range(len(sample_space)) if i != j]
            if distances:
        ...
        if avg_distances:
            ...
            for i in range(n_points):
                for j in range(i + 1, n_points):
                    if np.linalg.norm(phase_space[i] - phase_space[j]) < recurrence_radius:
                        recurrent_pairs += 1
                    total_pairs += 1
    ...
    def predict_climate_dynamics(self, initial_conditions: List[float],
                                n_steps: int = 30,
        ...
        if len(initial_conditions) != expected_dim:
            if len(initial_conditions) > expected_dim:
                ...
                padded = initial_conditions + [0.0] * (expected_dim - len(initial_conditions))
        ...
        try:
            ...
            if model_type == 'lorenz':
                ...
                trajectory = predefined_system.trajectory(
                    initial_conditions,
                    total_time=n_steps,
                    parameters=np.array([10.0, 28.0, 8.0/3.0])  # Standard Lorenz parameters
                )
            elif model_type == 'rossler':
                predefined_system = pds.ContinuousDynamicalSystem(model="rossler system")
                trajectory = predefined_system.trajectory(
                    initial_conditions,
                    total_time=n_steps,
            ...
            trajectory = self._simulate_climate_trajectory(initial_conditions, n_steps, model_type)
        ...
        try:
            ...
            lyapunov_exponents = [0.9, -0.1, -2.0]  # Typical values for chaotic attractor
        ...
        try:
            ...
            hurst_value = hurst_exp[0] if isinstance(hurst_exp, np.ndarray) and len(hurst_exp) > 0 else 0.5
        except Exception:
            hurst_value = 0.5
        ...
        return {
            ...
            'max_lyapunov_exponent': max(lyapunov_exponents) if lyapunov_exponents else 0.0,
            'n_steps': n_steps,
            'initial_conditions': initial_conditions.tolist(),
            'model_parameters': self.model_params.parameters,
            'chaos_level': 'high' if max(lyapunov_exponents) > 0.5 else 'low' if max(lyapunov_exponents) < 0.1 else 'medium'
        }
    ...
    def _map_to_climate_space(self, trajectory: np.ndarray) -> np.ndarray:
        ...
        result = np.zeros((normalized.shape[0], min(normalized.shape[1], climate_ranges.shape[0])))
        
        for i in range(result.shape[1]):
            min_range, max_range = climate_ranges[i]
            result[:, i] = normalized[:, i % normalized.shape[1]] * (max_range - min_range) + min_range
            
        return result

    def ensemble_climate_prediction(self, 
                                  n_models: int = 5,
                                  n_steps: int = 30,
                                  model_type: str = 'lorenz') -> Dict[str, Any]:
        """
        ...
        """
        predictions = []
        model_types = ['lorenz', 'rossler'] if model_type in ['lorenz', 'rossler'] else [model_type]
        ...
        if predictions:
            ...
            temp_trajectories = [pred['trajectory'] for pred in predictions if 'trajectory' in pred]
            ...
            if temp_trajectories:
                ...
                ensemble_spread = [float(np.std(temp_ts, axis=0).mean()) for temp_ts in temp_array]
                ...
                return {
                    ...
                    'confidence_intervals': self._calculate_confidence_intervals(temp_array)
                }
    ...
    def _simulate_climate_trajectory(self, initial_conditions: np.ndarray,
                                   n_steps: int,
        ...
        for i in range(1, n_steps):
            ...
            if model_type == 'lorenz':
                ...
                x, y, z = prev[0] if len(prev) > 0 else 0, prev[1] if len(prev) > 1 else 0, prev[2] if len(prev) > 2 else 0
                ...
                for j in range(len(prev)):
                    ...
                    next_vals[j] = prev[j] + 0.1 * np.sin(i * 0.1 + j) * (2 * np.random.random() - 1)

            trajectory[i] = next_vals

        return trajectory

    def _calculate_confidence_intervals(self, ensemble: np.ndarray,
                                      confidence_levels: List[float] = [0.68, 0.95]) -> Dict[str, List]:
        """
    ...
    def detect_climate_regime_shifts(self, climate_time_series: List[float]) -> Dict[str, Any]:
        """
        ...
        for i in range(len(mean_changes)):
            if abs(mean_changes[i]) > mean_threshold or abs(std_changes[i]) > std_threshold:
    ...
    def calculate_climate_predictability_horizon(self, 
                                               lyapunov_exponent: float,
                                               initial_uncertainty: float = 0.01) -> Dict[str, float]:
        """
        ...
        if initial_uncertainty > 0:
            predictability_horizon = (1.0 / lyapunov_exponent) * \
                                    np.log(final_acceptable_uncertainty / initial_uncertainty)
        else:
...
def predict_climate_dynamics(initial_conditions: List[float],
                           n_steps: int = 30,
                           model_type: str = 'lorenz',
                           parameters: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Generate climate predictions using dynamical systems models"""
...
def ensemble_climate_prediction(n_models: int = 5,
                              n_steps: int = 30,
                              model_type: str = 'lorenz') -> Dict[str, Any]:
    """Generate ensemble predictions using multiple dynamical systems models"""
...
def fit_climate_phase_space(climate_data: List[Dict[str, float]], 
                          target_var: str = 'temperature') -> Dict[str, Any]:
    """Reconstruct phase space from climate time series data for dynamical analysis"""
    return dynamical_climate_service.fit_climate_phase_space(
        climate_data, target_var
    )

def detect_climate_regime_shifts(climate_time_series: List[float]) -> Dict[str, Any]:
    """Detect climate regime shifts using dynamical systems properties"""
    return dynamical_climate_service.detect_climate_regime_shifts(
        climate_time_series
    )

def calculate_climate_predictability_horizon(lyapunov_exponent: float,
                                          initial_uncertainty: float = 0.01) -> Dict[str, float]:
    """Calculate the predictability horizon based on the largest Lyapunov exponent"""
