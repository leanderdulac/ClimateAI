...
class ClimateHMMService:
    ...
    def __init__(self):
        self.n_states = 0
        self.transition_matrices = {}  # Will store A(ΔT_t) for different temperature changes
        self.emission_means = {}  # μ_j(θ_t) - state means that depend on climate forcings
        self.emission_covariances = {}  # Σ_j - state covariances
        self.current_state_probs = None
        self.forcing_sensitivities = {}  # How sensitive transition probabilities are to forcing
    ...
    def estimate_transition_matrix(self, climate_forcing: float, 
                                 base_transition_matrix: np.ndarray,
                                 sensitivity_matrix: np.ndarray) -> np.ndarray:
        """
    ...
    def calculate_emission_mean(self, state: int, climate_forcings: np.ndarray) -> np.ndarray:
        """
        ...
        base_mean = np.array([20.0, 10.0, 1013.0])  # [temperature, precipitation, pressure] - example
        ...
        state_multiplier = np.array([1.0, 0.5, 0.8])  # Different for each observed variable
        ...
        forcing_adjustment = np.array([co2_effect + ch4_effect + aerosol_effect, 
                                     ch4_effect * 0.3,  # CH₄ also affects precipitation
                                     aerosol_effect * 0.1])  # Aerosols slightly affect pressure
        
        return base_mean * state_multiplier + forcing_adjustment
    
    def compute_regime_transition_probabilities(self, 
        ...
        co2_equiv = current_forcing[0] * 1.0 + current_forcing[1] * 25 * 0.001  # CH₄ is ~25x CO₂ over 100 years
        co2_equiv += current_forcing[2] * -0.3  # Aerosols have negative forcing
        
        # Adjust transition matrix based on cumulative forcing
        forcing_factor = max(0.1, min(2.0, 1.0 + co2_equiv * 0.01))  # Limit to 10% - 200% adjustment
        ...
        return {
            ...
            'n_states': n_states,
            'regime_names': [self.regime_descriptions.get(i, f"Regime {i}") for i in range(n_states)]
        }
    ...
    def compute_emission_probabilities(self, 
        ...
        for state in range(n_states):
            ...
            try:
                ...
                logger.warning(f"Error computing emission probability for state {state}: {e}")
        ...
        return {
            ...
            'current_forcing': current_forcing.tolist(),
            'observations': observations.tolist() if isinstance(observations, np.ndarray) else observations
        }
    
    def viterbi_decode(self, observations: List[np.ndarray], 
                      transition_matrices: List[np.ndarray],
                      emission_probs: List[Dict[str, float]],
                      initial_probs: Optional[np.ndarray] = None) -> List[int]:
        """
    ...
    def compute_climate_regime_model(self,
        ...
        for t in range(len(climate_observations)):
            ...
            if len(current_temp_hist) < 2:
                current_temp_hist = temperatures_history[:2] if len(temperatures_history) >= 2 else [20.0, 20.0]
            ...
            emission_info = self.compute_emission_probabilities(
                obs, forcing, n_states
            )
        ...
        return {
            ...
            'regime_statistics': {
                ...
                'dominant_regime': max(set(regime_sequence), key=regime_sequence.count) if regime_sequence else 0,
            ...
            'regime_descriptions': {i: self.regime_descriptions.get(i, f"Regime {i}") for i in range(n_states)},
            'n_states': n_states,
            'n_observations': len(climate_observations),
            'climate_forcing_analysis': {
                'mean_co2': np.mean([forcing[0] for forcing in climate_forcings]) if climate_forcings else 0,
                'mean_ch4': np.mean([forcing[1] for forcing in climate_forcings]) if climate_forcings else 0,
                'mean_aerosols': np.mean([forcing[2] for forcing in climate_forcings]) if climate_forcings else 0
            }
        }
