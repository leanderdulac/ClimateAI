...
class SCRModuleService:
    ...
    def calculate_climate_risk_score(
        ...
        # Wind storms risk
        risk_components[ClimateRiskType.WIND_STORMS.value] = self._calculate_wind_risk(
            climate_data.wind_data
        )

        # Flood risk based on precipitation and location
        risk_components[ClimateRiskType.FLOOD_RISK.value] = self._calculate_flood_risk(
            climate_data.precipitation_data, climate_data.location_coordinates
        )
        ...
        risk_components[ClimateRiskType.FIRE_RISK.value] = self._calculate_fire_risk(
            climate_data.temperature_data, climate_data.precipitation_data
        )
        ...
        location_profile = self._calculate_location_risk_profile(
            climate_data.location_coordinates
        )
    ...
    def _calculate_precipitation_risk(
        self, precip_data: List[Dict[str, float]]
    ) -> float:
        ...
        p95_threshold = (
            np.percentile(precip_values, 95)
            if len(precip_values) > 10
            else np.max(precip_values)
        )
    ...
    def _calculate_wind_risk(self, wind_data: List[Dict[str, float]]) -> float:
        ...
        p90_threshold = (
            np.percentile(wind_values, 90)
            if len(wind_values) > 10
            else np.max(wind_values)
        )
    ...
    def _calculate_flood_risk(
        self, precip_data: List[Dict[str, float]], coordinates: Tuple[float, float]
    ) -> float:
    ...
    def _calculate_drought_risk(self, precip_data: List[Dict[str, float]]) -> float:
        ...
        p25_threshold = (
            np.percentile(precip_values, 25)
            if len(precip_values) > 10
            else np.min(precip_values)
        )
        ...
        drought_severity = (
            max_consecutive_dry / len(precip_values) if len(precip_values) > 0 else 0
        )
        risk = min(1.0, low_precip_rate + drought_severity * 0.5)
        return risk

    def _calculate_fire_risk(
        self, temp_data: List[Dict[str, float]], precip_data: List[Dict[str, float]]
    ) -> float:
    ...
    def _calculate_weighted_score(self, risk_components: Dict[str, float]) -> float:
        ...
        for risk_type, score in risk_components.items():
            weight = self.base_weights.get(
                ClimateRiskType(risk_type), 0.16
    ...
    def _calculate_confidence(self, climate_data: ClimateData) -> float:
        ...
        if climate_data.temperature_data:
            latest_date = (
                datetime.fromisoformat(climate_data.temperature_data[-1]["date"])
                if "date" in climate_data.temperature_data[-1]
                else datetime.now()
            )
        ...
        confidence = min(
            1.0,
            (min(1.0, avg_data_points / 730) + min(1.0, 30 / max(1, days_since_last)))
            / 2,
        )
        return confidence
    ...
    def _calculate_location_risk_profile(
        ...
        profile = {
            "latitude_factor": float(
                abs(latitude) / 90.0
            ),  # Polar regions have different risks
            "proximity_tropical": (
                1.0 if abs(latitude) < 23.5 else 0.0
            ),  # Tropical regions
            "seasonal_variation": float(
                1.0 if abs(latitude) > 23.5 else 0.5
