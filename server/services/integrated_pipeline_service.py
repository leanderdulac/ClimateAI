...
from services.scr_module_service import (
    SCRRiskScoringEngine, ClimateData, ClimateRiskScore, calculate_climate_risk_score
)
from services.aat_module_service import (
    TraditionalActuarialEngine, HistoricalLossData, ActuarialAnalysisResult, 
    RiskCategory, perform_actuarial_analysis, calculate_reinsurance_requirements
)
from services.epe_module_service import (
    CommercialPricingEngine, MarketData, RiskAdjustedPremium, CommercialPricingResult,
    PricingStrategy, calculate_commercial_pricing, get_pricing_strategy_recommendation
)
from services.mds_module_service import (
    UnderwritingDecisionEngine, ApplicationData, ModuleInputs, UnderwritingDecision, 
    make_underwriting_decision, apply_policy_rules
)
...
class IntegratedPipeline:
    ...
    def process_application(self, pipeline_input: PipelineInput) -> PipelineResult:
        ...
        logger.info(f"Starting SCR analysis for application {pipeline_input.application_data.applicant_id}")
        ...
        risk_adjusted_premium = RiskAdjustedPremium(
            ...
            total_adjusted_premium=aat_result.total_premium * (1 + scr_result.overall_score * 0.1),
        ...
        mds_result = make_underwriting_decision(
            pipeline_input.application_data,
            module_inputs
        )
        ...
        logger.info(f"Pipeline processing completed for application {result.application_id} in {duration:.2f}s")
        return result
    
    def process_batch_applications(self, pipeline_inputs: List[PipelineInput]) -> List[PipelineResult]:
        """
        ...
        for pipeline_input in pipeline_inputs:
            try:
                ...
                logger.error(f"Error processing application {pipeline_input.application_data.applicant_id}: {str(e)}")
    ...
    def get_pipeline_summary(self, pipeline_result: PipelineResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'underwriting_conditions': [c.value for c in pipeline_result.mds_result.conditions],
...
def process_batch_applications(pipeline_inputs: List[PipelineInput]) -> List[PipelineResult]:
...
async def run_complete_pipeline(
    ...
    loss_data = HistoricalLossData(
        ...
        coverage_type=RiskCategory.PROPERTY if 'property' in coverage_type.lower() else RiskCategory.AGRICULTURE,
        location_coordinates=location_coordinates,
        asset_value=asset_value,
