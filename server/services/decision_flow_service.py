...
class PolicyApplication:
    ...
    mitigation_details: Optional[List[Dict[str, Any]]] = None  # Details of mitigation measures
...
class DecisionFlowService:
    ...
    def evaluate_yellow_criteria(self, application: PolicyApplication) -> Dict[str, Any]:
        """
        ...
        claim_history_ok = application.claim_history_count <= self.yellow_criteria['max_claim_history']
        
        # Check mitigation measures criteria
        mitigation_measures_ok = application.mitigation_measures_count >= self.yellow_criteria['min_mitigation_measures']
        
        # Check concentration criteria
        concentration_ok = application.zone_concentration_percentage <= self.yellow_criteria['max_concentration_percentage']
    ...
    def calculate_policy_application(self, application: PolicyApplication) -> DecisionFlowResult:
        """
        ...
        if flow_color == 'green':
            ...
            if criteria_evaluation['criteria_met']:
                ...
                if not criteria_evaluation['claim_history_ok']:
                    conditions.append(f"Reduce claim history to ≤{self.yellow_criteria['max_claim_history']} claims")
                
                if not criteria_evaluation['mitigation_measures_ok']:
                    conditions.append(f"Implement ≥{self.yellow_criteria['min_mitigation_measures']} mitigation measures")
                    
                if not criteria_evaluation['concentration_ok']:
                    conditions.append(f"Reduce zone concentration to ≤{self.yellow_criteria['max_concentration_percentage']:.0f}%")
            ...
            reinsurance_reqs = ReinsuranceRequirements(
                ...
                treaty_details={
                    ...
                    "reinsurance_type": "quota_share",
                    "coverage_amount_subject_to_reinsurance": application.coverage_amount * 0.50
                }
            )
        ...
        risk_adjustments = {
            ...
            'scr_based_risk_loading': application.scr_score / 1000.0,  # Proportional to SCR
        ...
        workflow_details = {
            ...
            'reinsurance_requirements': vars(reinsurance_reqs) if reinsurance_reqs else None,
            'yellow_criteria_evaluation': self.evaluate_yellow_criteria(application) if flow_color == 'yellow' else None
        }
    ...
    def calculate_final_premium_with_adjustments(self,
                                                 ptp: float,
                                                 ml: float,
                                                 tr: float,
                                                 cc: float,
                                                 supply_demand_adjustment: float) -> float:
        """
    ...
    def batch_process_applications(self, applications: List[PolicyApplication]) -> List[DecisionFlowResult]:
        """
    ...
    def analyze_portfolio_flow_distribution(self, applications: List[PolicyApplication]) -> Dict[str, Any]:
        """
        ...
        decisions = {'accepted': 0, 'conditionally_accepted': 0, 'requires_manual_review': 0, 'rejected': 0}
        ...
        return {
            ...
            'acceptance_rate': (decisions['accepted'] + decisions['conditionally_accepted']) / portfolio_size if portfolio_size > 0 else 0.0,
            'manual_review_rate': decisions['requires_manual_review'] / portfolio_size if portfolio_size > 0 else 0.0,
            'rejection_rate': decisions['rejected'] / portfolio_size if portfolio_size > 0 else 0.0,
...
def batch_process_policy_applications(applications: List[PolicyApplication]) -> List[DecisionFlowResult]:
    """Convenience function for batch processing"""
    return decision_flow_service.batch_process_applications(applications)

def analyze_portfolio_flow_distribution(applications: List[PolicyApplication]) -> Dict[str, Any]:
