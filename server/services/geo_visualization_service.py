...
class GeographicVisualizationService:
    ...
    def __init__(self):
        ...
        self.data_cache = {
            (-23.5507, -46.6339): {  # São Paulo coordinates
                ...
                "historical_rainfall": self._generate_sample_rainfall_data(90, -23.5507, -46.6339)
            },
            (-15.7942, -47.8822): {  # Brasília coordinates
                "location_name": "Brasília",
                "country": "Brazil", 
                "climate_zone": "Aw",  # Tropical Savanna
                "historical_rainfall": self._generate_sample_rainfall_data(90, -15.7942, -47.8822)
            },
            (-30.0346, -51.2177): {  # Porto Alegre coordinates
                "location_name": "Porto Alegre",
                "country": "Brazil",
                "climate_zone": "Cfa",  # Humid Subtropical
                "historical_rainfall": self._generate_sample_rainfall_data(90, -30.0346, -51.2177)
            }
        }

    def _generate_sample_rainfall_data(self, years: int, latitude: float, longitude: float) -> List[Dict[str, Any]]:
        ...
        if latitude < 0 and -70 < longitude < -40:  # South American region
            ...
            while current_date <= end_date:
                ...
                long_term_trend = 1.0 + 0.001 * year_count  # Slight increasing trend over decades
                rainfall = max(0, base_rainfall * seasonal_factor * long_term_trend * (1 + np.random.normal(0, 0.15)))
                
                temperature = round(np.random.normal(22 + (latitude * 0.1), 8), 1)  # Temperature varies by latitude
    ...
    def _generate_visualization_url(self, latitude: float, longitude: float, location_name: str) -> str:
    ...
    def get_location_data(self, latitude: float, longitude: float, years: int = 90) -> LocationSelectionResult:
        """
        ...
        try:
            ...
            if location_key in self.data_cache:
                ...
                historical_rainfall = self._generate_sample_rainfall_data(years, latitude, longitude)
            ...
            precipitation_values = [entry["precipitation_mm"] if "precipitation_mm" in entry else entry.get("precipitation", 0) for entry in rainfall_data]
            
            if precipitation_values and len(precipitation_values) > 0:
                rainfall_stats = {
                    "total_rainfall": round(sum(precipitation_values), 2),
                    "average_annual_rainfall": round(sum(precipitation_values) / len(precipitation_values) * 12, 2) if len(precipitation_values) > 0 else 0,
                    ...
                    "coefficient_of_variation": round(np.std(precipitation_values) / np.mean(precipitation_values) if np.mean(precipitation_values) > 0 else 0, 3),
                ...
                rainfall_stats = {"total_rainfall": 0, "average_annual_rainfall": 0, "records_count": 0}
                precipitation_pattern = "No Data"
            
            # Generate visualization URL (in a real system, this would create an actual visualization)
            visualization_url = self._generate_visualization_url(latitude, longitude, location_data["location_name"])
            ...
            logger.info(f"Location data retrieved for {latitude}, {longitude} with {len(rainfall_data)} records")
            return result
    ...
    def create_globe_animation(self, center_lat: float, center_lon: float, zoom: int = 5) -> str:
        """
    ...
    def create_climate_data_visualization(self, location_data: LocationSelectionResult) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for entry in rainfall_data:
                if "date" in entry:
                    ...
                    precipitation.append(entry.get("precipitation_mm", entry.get("precipitation", 0)))
            ...
            if len(precipitation) > 10:
                ...
                moving_avg_window = min(12, len(precipitation))  # 12-month moving average
                moving_avg = []
                for i in range(len(precipitation)):
                    start_idx = max(0, i - moving_avg_window + 1)
                    avg_val = sum(precipitation[start_idx:i+1]) / (i - start_idx + 1)
            ...
            visualization_data = {
                ...
                "metadata": {
                    ...
                    "year_range": f"{dates[0][:4]} to {dates[-1][:4]}" if dates else "None",
    ...
    def get_available_datasets(self, latitude: float, longitude: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            datasets = {
                ...
                "location": {
                    "latitude": latitude,
                    "longitude": longitude,
                    "coordinates_valid": (-90 <= latitude <= 90) and (-180 <= longitude <= 180)
                },
                "data_quality_indicators": {
                    "spatial_resolution": "0.1° x 0.1°",
                    "temporal_coverage": "1900-present",
                    "gaps_completeness": "95%",
                    "validation_sources": ["ground_stations", "satellite", "reanalysis"]
                }
            }
...
def get_location_data(latitude: float, longitude: float, years: int = 90) -> LocationSelectionResult:
...
def create_globe_animation(center_lat: float, center_lon: float, zoom: int = 5) -> str:
    ...
    return geo_visualization_service.create_globe_animation(center_lat, center_lon, zoom)

def create_climate_data_visualization(location_data: LocationSelectionResult) -> Dict[str, Any]:
