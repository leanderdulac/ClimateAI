...
class ClimatePremiumService:
    ...
    def calculate_climate_drift_rate(self, 
                                   delta_temperature: float,
                                   co2_rate_change: float,
                                   delta_precipitation: float = 0.0,
                                   custom_coefficients: Optional[Dict[str, float]] = None) -> float:
        """
        ...
        drift_rate = (
            ...
            coefficients['beta_3'] * (delta_precipitation / 100.0)  # Normalize precipitation change
        )
    ...
    def calculate_climatic_inflation_factor(self,
                                          time_horizon_years: float,
    ...
    def calculate_climate_inclusive_premium(self,
                                          expected_loss: float,
                                          time_horizon_years: float,
                                          ...
                                          co2_trend: float = 0.1) -> ClimatePremiumResult:
        """
        ...
        if climate_scenario_func is None:
            climate_scenario_func = self.create_default_climate_scenario(
                initial_delta_temp, temperature_trend,
        ...
        return ClimatePremiumResult(
            ...
            climate_sensitivity_coefficients=custom_coefficients or self.drift_coefficients
        )
    
    def calculate_premium_scenarios(self,
        ...
        for i, (exp_loss, time_horiz) in enumerate(zip(expected_losses, time_horizons)):
            scenario = scenarios[i] if i < len(scenarios) else scenarios[0]  # Cycle through scenarios if needed
            ...
            results.append({
                ...
                'premium_breakdown': {
                    ...
                    'plus_operational_costs': exp_loss * (1 + result.loading_factor) + result.operational_costs,
                    'after_mitigation': (exp_loss * (1 + result.loading_factor) + result.operational_costs) * (1 - result.mitigation_discount),
...
def calculate_climate_drift_rate(delta_temperature: float, 
                               co2_rate_change: float,
                               delta_precipitation: float = 0.0,
                               custom_coefficients: Optional[Dict[str, float]] = None) -> float:
    """Calculate climate drift rate: λ_s = β₀ + β₁·ΔT_s + β₂·d(CO₂)/dt + β₃·ΔP_s"""
...
def calculate_climatic_inflation_factor(time_horizon_years: float,
...
def create_default_climate_scenario(initial_delta_temp: float,
                                 temperature_trend: float,
                                 initial_co2_rate: float,
                                 co2_trend: float,
                                 initial_delta_precip: float = 0.0,
    ...
    return climate_premium_service.create_default_climate_scenario(
        initial_delta_temp, temperature_trend, initial_co2_rate, 
        co2_trend, initial_delta_precip, precip_trend
    )
...
def calculate_climate_inclusive_premium(
    ...
    return climate_premium_service.calculate_climate_inclusive_premium(
        ...
        mitigation_discount, None, None, initial_delta_temp, temperature_trend,
        initial_co2_rate, co2_trend
    )
