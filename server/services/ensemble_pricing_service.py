...
class EnsemblePricingService:
    ...
    def calculate_bic(self, log_likelihood: float, n_params: int, n_observations: int) -> float:
        """
    ...
    def calculate_dynamic_weights(self, bics: List[float], 
                                n_models: int,
                                prior_alpha: Optional[List[float]] = None) -> List[float]:
        """
        ...
        if len(prior_alpha) != n_models:
            raise ValueError(f"Prior alpha length ({len(prior_alpha)}) must match number of models ({n_models})")
    ...
    def calculate_ensemble_var(self, model_premiums: List[float], 
                             model_weights: List[float],
        ...
        if len(model_premiums) > 1:
            ...
            variance = sum(w**2 * (p - ensemble_mean)**2 for w, p in zip(model_weights, model_premiums))
    ...
    def calculate_ensemble_pricing(self, 
                                 model_premiums: List[float],
                                 ...
                                 uncertainty_factor: float = 1.0) -> Dict[str, Any]:
        """
        ...
        """
        if not all(len(lst) == n_models for lst in [model_premiums, model_log_likelihoods, model_n_params, model_n_observations]):
        ...
        var_ensemble = self.calculate_ensemble_var(model_premiums, weights, confidence_level)
    ...
    def update_model_performance(self, model_id: str, log_likelihood: float, 
                               n_params: int, n_observations: int):
        """
...
def calculate_bic(log_likelihood: float, n_params: int, n_observations: int) -> float:
    """Calculate Bayesian Information Criterion (BIC)"""
    return ensemble_pricing_service.calculate_bic(log_likelihood, n_params, n_observations)

def calculate_dynamic_weights(bics: List[float], 
                           n_models: int,
                           prior_alpha: Optional[List[float]] = None) -> List[float]:
    """Calculate dynamic model weights: w_m(t) ∝ exp(-η·BIC_m(t-1)) · π_m"""
    return ensemble_pricing_service.calculate_dynamic_weights(bics, n_models, prior_alpha)
...
def calculate_ensemble_pricing(
    ...
    return ensemble_pricing_service.calculate_ensemble_pricing(
        ...
        n_models, confidence_level
    )

def update_model_performance(model_id: str, log_likelihood: float, 
                           n_params: int, n_observations: int):
