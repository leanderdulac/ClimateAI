...
class GeocodingService:
    ...
    async def geocode_address(self, address: str) -> Dict:
        ...
        location = await self._run_with_retries(
            lambda: self.geolocator.geocode(
                address, exactly_one=True, addressdetails=True, language="pt-BR"
            )
        )
    ...
    async def reverse_geocode(self, latitude: float, longitude: float) -> Dict:
        ...
        location = await self._run_with_retries(
            lambda: self.geolocator.reverse(
                (latitude, longitude), language="pt-BR", addressdetails=True
            )
        )
    ...
    async def search_cities(
        self, query: str, state: Optional[str] = None
    ) -> List[Dict]:
        """
        ...
        results = [
            ...
            for entry in self.city_data
            if normalized_query in self._normalize_text(entry["city"])
            and (not state_filter or entry["state"] == state_filter)
        ]
        return results[:10]
    ...
    async def _run_with_retries(self, func, attempts: int = 3):
        ...
        for attempt in range(attempts):
            try:
                ...
                if attempt == attempts - 1:
                    raise HTTPException(
                        status_code=500, detail=f"Erro na geocodificação: {exc}"
                    ) from exc

        if last_error:
            raise HTTPException(
                status_code=500, detail=f"Erro na geocodificação: {last_error}"
            )
    ...
    def _load_city_dataset(self) -> List[Dict]:
        ...
        for entry in data:
            ...
            entry.setdefault(
                "formatted_address", f"{entry['city']} - {entry['state']}, Brasil"
            )
        return data
    ...
    def _find_city_entry(
        self, city: Optional[str], state: Optional[str]
    ) -> Optional[Dict]:
