...
class ClimateRiskModelingService:
    ...
    def calculate_standardized_precipitation_index(self, precipitation_data: List[float], 
                                                  window_months: int = 3) -> List[float]:
        """
        ...
        if len(precipitation_data) < window_months * 30:  # Assuming daily data
            raise ValueError(f"Need at least {window_months * 30} days of data for {window_months}-month SPI")
        ...
        for i in range(len(monthly_data) - window_months + 1):
            ...
            historical_window_means = [sum(monthly_data[j:j+window_months]) 
    ...
    def calculate_relative_wetness_index(self, precipitation: List[float], 
    ...
    def extract_synoptic_circulation_patterns(self, pressure_data: List[float], 
                                           wind_data: List[Tuple[float, float]],  # (speed, direction)
                                           lat_lon_data: List[Tuple[float, float]]) -> List[Dict[str, float]]:
        """
        ...
        for i in range(len(pressure_data)):
            ...
            pattern_features = {
                ...
                'low_pressure_system': 1 if pressure < 1000 else 0,  # Low pressure indicator
                
                # Wind-based indices  
                'meridional_flow': wind_speed * np.cos(np.radians(wind_dir)),  # North-south component
                'zonal_flow': wind_speed * np.sin(np.radians(wind_dir)),      # East-west component
                'wind_intensity': wind_speed,
                
                # Latitude-based circulation patterns
                'polar_front_influence': 1 if abs(lat) > 60 else 0,  # Polar front region
                'subtropical_high': 1 if 20 < abs(lat) < 40 else 0,  # Subtropical high region
            }
    ...
    def calculate_vertical_temperature_gradient(self, temperature_data: List[List[float]]) -> List[float]:
        """
        ...
        for temp_profile in temperature_data:
            if len(temp_profile) >= 2:
                ...
                height_diff = 5.0  # Approximate height difference (surface to 500 hPa ~ 5.5 km)
    ...
    def create_climate_feature_matrix(self, precipitation_data: List[float],
        ...
        if len(precipitation_data) >= 90:  # At least 3 months
            try:
                spi_3m = self.calculate_standardized_precipitation_index(precipitation_data, window_months=3)
            except ValueError:
                spi_3m = []  # Handle case where insufficient data despite check
        if len(precipitation_data) >= 180:  # At least 6 months
            try:
                spi_6m = self.calculate_standardized_precipitation_index(precipitation_data, window_months=6)
            except ValueError:
                spi_6m = []  # Handle case where insufficient data despite check
        if len(precipitation_data) >= 360:  # At least 12 months
            try:
                spi_12m = self.calculate_standardized_precipitation_index(precipitation_data, window_months=12)
            except ValueError:
                spi_12m = []  # Handle case where insufficient data despite check
        
        # Calculate RWI
        rwi = self.calculate_relative_wetness_index(precipitation_data, temperature_data)
        ...
        if spi_3m: lengths_to_consider.append(len(spi_3m))
        if spi_6m: lengths_to_consider.append(len(spi_6m))
        if spi_12m: lengths_to_consider.append(len(spi_12m))
    ...
    def regularized_loss_function(self, y_true: np.ndarray, y_pred: np.ndarray, 
    ...
    def fit_regularized_climate_model(self, feature_matrix: np.ndarray,
                                    target_values: np.ndarray,
                                    model_type: str = 'ridge',
                                    gamma: float = 0.1,
                                    lambda_reg: float = 0.01) -> Dict[str, Any]:
        """
        ...
        if hasattr(model, 'coef_'):
            ...
            model_weights = model.feature_importances_ if hasattr(model, 'feature_importances_') else np.ones(X_scaled.shape[1])
        ...
        return {
            ...
            'feature_importance': getattr(model, 'feature_importances_', 
                                        getattr(model, 'coef_', np.ones(len(model_weights)))).tolist(),
            'n_features': X_scaled.shape[1],
            'n_samples': len(target_values)
        }
    
    def predict_with_climate_model(self, model_id: str, 
                                 feature_matrix: np.ndarray) -> Dict[str, Any]:
        """
        ...
        uncertainty = [0.0] * len(predictions)  # Placeholder, could be more sophisticated
        ...
        if model_info['model_type'] in ['rf', 'gbm']:
            ...
            if model_info['model_type'] == 'rf':
                ...
                pred_matrix = np.array([est.predict(X_scaled) for est in model.estimators_])
    ...
    def comprehensive_climate_risk_assessment(self,
        ...
        feature_matrix = self.create_climate_feature_matrix(
            ...
            wind_data, lat_lon_data, temp_profile_data
        )
        
        # Fit regularized climate model
        model_results = self.fit_regularized_climate_model(
            feature_matrix, np.array(target_values),
        ...
        if len(precipitation_data) >= 90:  # At least 3 months
            try:
                spi_3m = self.calculate_standardized_precipitation_index(precipitation_data, 3)
            except ValueError:
                spi_3m = []
        if len(precipitation_data) >= 180:  # At least 6 months
            try:
                spi_6m = self.calculate_standardized_precipitation_index(precipitation_data, 6)
            except ValueError:
                spi_6m = []
        ...
        return {
            ...
            'synoptic_patterns': {
                'low_pressure_systems': sum(1 for cp in circulation_patterns if cp['low_pressure_system'] > 0),
                'polar_front_influence': sum(1 for cp in circulation_patterns if cp['polar_front_influence'] > 0),
                'subtropical_high_influence': sum(1 for cp in circulation_patterns if cp['subtropical_high'] > 0)
            },
            'risk_assessment': {
                'predicted_risk_level': float(np.mean(model_results['predictions'])),
                'risk_uncertainty': float(np.std(model_results['predictions'])),
                'model_confidence': 1.0 - (model_results['mae'] / (1.0 + np.mean(np.abs(target_values))))
            },
...
def calculate_standardized_precipitation_index(precipitation_data: List[float], 
                                            window_months: int = 3) -> List[float]:
    """Calculate Standardized Precipitation Index (SPI) for different time windows"""
...
def calculate_relative_wetness_index(precipitation: List[float], 
                                  temperature: List[float]) -> List[float]:
    """Calculate Relative Wetness Index (RWI) based on precipitation and temperature"""
...
def extract_synoptic_circulation_patterns(pressure_data: List[float], 
                                       wind_data: List[Tuple[float, float]],
                                       lat_lon_data: List[Tuple[float, float]]) -> List[Dict[str, float]]:
    """Extract synoptic circulation patterns based on pressure and wind data"""
...
def calculate_vertical_temperature_gradient(temperature_data: List[List[float]]) -> List[float]:
    """Calculate vertical temperature gradient (instability indicator)"""
    return climate_risk_modeling_service.calculate_vertical_temperature_gradient(
        temperature_data
    )

def regularized_loss_function(y_true: np.ndarray, y_pred: np.ndarray, 
                           model_weights: np.ndarray,
                           gamma: float = 1.0,
                           lambda_reg: float = 0.01,
                           loss_type: str = 'mse') -> float:
