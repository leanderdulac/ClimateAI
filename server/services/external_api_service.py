...
class ExternalAPIService:
        ...
        try:
            ...
            if cpi_response.status_code == 200:
                ...
                if cpi_data.get('observations'):
                    ...
                    indicators['inflation_rate'] = float(latest_cpi.get('value', 0)) / 100
            ...
            if gdp_response.status_code == 200:
                ...
                if gdp_data.get('observations') and len(gdp_data['observations']) >= 12:
                    ...
                    if previous_gdp > 0:
                        indicators['gdp_growth'] = (current_gdp - previous_gdp) / previous_gdp
    ...
    async def get_real_time_data(self, latitude: float, longitude: float, commodities: List[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            commodity_task = self.get_commodity_prices(commodities or ['CORN', 'SOYBEAN'])
            ...
            return {
                ...
                'commodities': self._get_mock_commodity_data(commodities or ['CORN', 'SOYBEAN']),
                'timestamp': datetime.now().isoformat(),
                'location': {'latitude': latitude, 'longitude': longitude},
                'error': str(e)
            }
...
async def get_real_time_data(latitude: float, longitude: float, commodities: List[str] = None) -> Dict[str, Any]:
    """Get comprehensive real-time data"""
    return await external_api_service.get_real_time_data(latitude, longitude, commodities)