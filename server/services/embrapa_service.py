...
class EmbrapaAPIService:
    ...
    async def get_climate_data(self, latitude: float, longitude: float, start_date: str, end_date: str) -> List[Dict]:
        ...
        if not self.is_configured:
            ...
            return await self._fallback_to_openmeteo(latitude, longitude, start_date, end_date)
    ...
    async def _fallback_to_openmeteo(self, latitude: float, longitude: float, start_date: str, end_date: str) -> List[Dict]:
        """
        ...
        try:
            ...
            dados = openmeteo_service.obter_historico(latitude, longitude, start_dt, end_dt)
    ...
    async def _fallback_forecast_openmeteo(self, latitude: float, longitude: float, days: int) -> Dict:
        """
        Fallback para previsão OpenMeteo quando Embrapa não estiver disponível
        """
        try:
            openmeteo_service = OpenMeteoService()
            dados = openmeteo_service.obter_previsao(latitude, longitude, min(days, 16))  # OpenMeteo limita a 16 dias
