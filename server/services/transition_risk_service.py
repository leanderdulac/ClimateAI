...
class AssetCharacteristics:
    ...
    asset_age_years: int
    industry_sector: str  # 'oil_gas', 'coal', 'utilities', 'transport', 'manufacturing', 'finance'
...
class EnvironmentalScenario:
    ...
    carbon_price_usd_per_tco2: float  # Carbon price in USD per ton CO₂e (default US$ 150/tCO₂e)
...
class TransitionRiskCalculator:
    ...
    def calculate_transition_risk(self,
        ...
        total_transition_risk = weighted_carbon_tax + weighted_stranded_asset + weighted_litigation
    ...
    def _calculate_carbon_tax_risk(self, fossil_exposure: float, carbon_price: float,
                                  exposure_multiplier: float, asset_value: float) -> float:
        """
        ...
        carbon_tax_risk = (adjusted_carbon_tax / 1000000) * asset_value  # Scale by asset value
        
        return max(0.0, carbon_tax_risk)  # Non-negative risk

    def _calculate_stranded_asset_risk(self, revenue_dependence: float,
                                     sector_stranded_risk: float,
                                     adaptation_readiness: float,
                                     asset_age: int, asset_value: float) -> float:
        """
        ...
        adaptation_factor = max(0.1, 1 - adaptation_readiness)  # At least 10% of original risk
        
        # Older assets may be more at risk of stranding
        age_factor = min(1.5, 1 + (asset_age / 50.0))  # Max 50% increase for very old assets
    ...
    def _calculate_litigation_risk(self, delta_temp: float, media_exposure: float,
                                 regulatory_delay: float, sensitivity: float,
        ...
        ln_delta_t = np.log(max(0.01, delta_temp)) if delta_temp > 0 else -4  # approx ln(0.01)
        ...
        expected_compensation = sensitivity * asset_value * 0.1  # 10% of asset value as proxy
    ...
    def calculate_scenario_comparison(self,
                                    ...
                                    beta_weights: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        """
        ...
        # Calculate changes
        risk_increase = future_risk.total_transition_risk - baseline_risk.total_transition_risk
        risk_ratio = future_risk.total_transition_risk / baseline_risk.total_transition_risk if baseline_risk.total_transition_risk > 0 else 1.0
    ...
    def calculate_sector_specific_risk(self, asset_char: AssetCharacteristics,
                                     env_scenario: EnvironmentalScenario) -> Dict[str, float]:
        """
    ...
    def integrate_with_physical_risk(self,
                                   transition_risk_result: TransitionRiskResult,
                                   physical_risk_score: float) -> Dict[str, float]:
        """
        ...
        total_climate_risk = transition_risk_result.total_transition_risk + physical_risk_score
        
        # Calculate risk proportions
        if total_climate_risk > 0:
            transition_proportion = transition_risk_result.total_transition_risk / total_climate_risk
        ...
        return {
            ...
            'dominant_risk_type': 'transition' if transition_proportion > physical_proportion else 'physical'
        }

# Global instance
transition_risk_service = TransitionRiskCalculator()

def calculate_transition_risk(asset_char: AssetCharacteristics,
                            env_scenario: EnvironmentalScenario,
                            beta_weights: Optional[Dict[str, float]] = None,
                            scenario_name: str = "base") -> TransitionRiskResult:
    """Convenience function to calculate transition risk"""
    return transition_risk_service.calculate_transition_risk(asset_char, env_scenario, beta_weights, scenario_name)

def calculate_scenario_comparison(asset_char: AssetCharacteristics,
                               baseline_scenario: EnvironmentalScenario,
                               future_scenario: EnvironmentalScenario,
                               beta_weights: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Convenience function to calculate scenario comparison"""
    return transition_risk_service.calculate_scenario_comparison(asset_char, baseline_scenario, future_scenario, beta_weights)

def calculate_sector_specific_risk(asset_char: AssetCharacteristics,
                                 env_scenario: EnvironmentalScenario) -> Dict[str, float]:
    """Convenience function to calculate sector-specific risk"""
    return transition_risk_service.calculate_sector_specific_risk(asset_char, env_scenario)

def integrate_with_physical_risk(transition_risk_result: TransitionRiskResult,
                               physical_risk_score: float) -> Dict[str, float]:
    """Convenience function to integrate with physical risk"""
    return transition_risk_service.integrate_with_physical_risk(transition_risk_result, physical_risk_score)