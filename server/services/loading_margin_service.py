...
class LoadingMarginCalculator:
    ...
    def calculate_allocated_capital(self, exposure_value: float, scr_score: float) -> Dict[str, float]:
        """
    ...
    def calculate_loading_margin(self, 
                               exposure_value: float,
                               scr_score: float,
                               premium_volume: float) -> LoadingMarginResult:
        """
        ...
        loading_margin = self.roe_target * capital_details['allocated_capital'] / premium_volume
    ...
    def calculate_policy_loading_info(self,
                                    policy_exposure: float,
                                    policy_scr: float,
                                    policy_premium: float) -> PolicyLoadingInfo:
        """
        ...
        if policy_premium > 0:
            ...
            calculated_loading_margin = self.roe_target * allocated_capital / policy_premium
        else:
    ...
    def calculate_portfolio_loading_analysis(self, 
                                           policies: List[Dict[str, float]],
                                           portfolio_premium: float) -> PortfolioLoadingAnalysis:
        """
        ...
        for policy in policies:
            ...
            if portfolio_premium > 0:
                policy_loading = self.roe_target * capital_details['allocated_capital'] / portfolio_premium
            else:
                policy_loading = 0.0
        ...
        if portfolio_premium > 0:
            portfolio_loading_margin = (self.roe_target * total_allocated_capital) / portfolio_premium
        else:
            portfolio_loading_margin = 0.0
            
        # Calculate capital efficiency ratio (how much capital is needed relative to premium)
        capital_efficiency_ratio = total_allocated_capital / portfolio_premium if portfolio_premium > 0 else 0.0
        
        # Calculate loading breakdown by risk category
        loading_breakdown = {}
        for category, count in risk_distribution.items():
            if count > 0:
                category_policies = [p for p in policies if self.determine_risk_category(p.get('scr_score', 0)) == category]
                category_capital = sum(
                    self.calculate_allocated_capital(p.get('exposure_value', 0), p.get('scr_score', 0))['allocated_capital']
                    for p in category_policies
                )
                category_loading = (self.roe_target * category_capital) / portfolio_premium if portfolio_premium > 0 else 0
    ...
    def calculate_risk_based_pricing_adjustment(self,
                                              base_premium: float,
                                              scr_score: float,
                                              exposure_value: float) -> Dict[str, float]:
        """
    ...
    def get_capital_efficiency_metrics(self,
                                     portfolio_policies: List[Dict[str, float]],
                                     total_portfolio_premium: float) -> Dict[str, Any]:
        """
        ...
        """
        analysis = self.calculate_portfolio_loading_analysis(portfolio_policies, total_portfolio_premium)
        
        # Calculate additional efficiency metrics
        if analysis.total_premium_volume > 0:
            capital_intensity_ratio = analysis.total_allocated_capital / analysis.total_premium_volume
            risk_premium_ratio = analysis.average_scr_score * analysis.portfolio_size / analysis.total_premium_volume if analysis.portfolio_size > 0 else 0
        else:
        ...
        potential_roe = analysis.portfolio_loading_margin  # Simplified as loading margin per premium unit
        ...
        return {
            ...
            'allocation_efficiency_score': 1.0 / (1.0 + capital_intensity_ratio),  # Higher efficiency = lower ratio
            'recommendations': self._generate_efficiency_recommendations(analysis, capital_intensity_ratio)
        }

    def _generate_efficiency_recommendations(self, 
                                           analysis: PortfolioLoadingAnalysis,
                                           capital_intensity_ratio: float) -> List[str]:
        """Generate recommendations for improving capital efficiency"""
        recommendations = []
        
        if analysis.average_scr_score > 600:
            recommendations.append("Portfolio has high average risk scores - consider risk mitigation programs")
            
        if analysis.risk_distribution.get('critical', 0) / analysis.portfolio_size > 0.1:
            recommendations.append("High concentration of critical risk policies - diversify risk exposure")
            
        if analysis.risk_distribution.get('low', 0) / analysis.portfolio_size < 0.2:
            recommendations.append("Low risk policy share is insufficient - consider expanding in lower risk segments")
            
        if capital_intensity_ratio > 0.3:
            recommendations.append("Capital intensity is high - review allocation methodology and consider reinsurance")
        elif capital_intensity_ratio < 0.05:
            recommendations.append("Capital intensity is very low - ensure adequate coverage for high-risk scenarios")
        
        # Check if loading margins are appropriately distributed across risk categories
        if analysis.loading_margin_breakdown.get('critical', 0) == 0 and analysis.risk_distribution.get('critical', 0) > 0:
            recommendations.append("Critical risk policies may not have adequate loading margins applied")
...
def calculate_loading_margin(exposure_value: float, scr_score: float, premium_volume: float) -> LoadingMarginResult:
    """Convenience function to calculate loading margin"""
    return loading_margin_service.calculate_loading_margin(exposure_value, scr_score, premium_volume)

def calculate_policy_loading_info(policy_exposure: float, policy_scr: float, policy_premium: float) -> PolicyLoadingInfo:
    """Convenience function to calculate policy loading information"""
    return loading_margin_service.calculate_policy_loading_info(policy_exposure, policy_scr, policy_premium)

def calculate_portfolio_loading_analysis(policies: List[Dict[str, float]], portfolio_premium: float) -> PortfolioLoadingAnalysis:
    """Convenience function to calculate portfolio loading analysis"""
    return loading_margin_service.calculate_portfolio_loading_analysis(policies, portfolio_premium)

def calculate_risk_based_pricing_adjustment(base_premium: float, scr_score: float, exposure_value: float) -> Dict[str, float]:
    """Convenience function to calculate risk-based pricing adjustment"""
    return loading_margin_service.calculate_risk_based_pricing_adjustment(base_premium, scr_score, exposure_value)

def get_capital_efficiency_metrics(portfolio_policies: List[Dict[str, float]], total_portfolio_premium: float) -> Dict[str, Any]:
    """Convenience function to get capital efficiency metrics"""
    return loading_margin_service.get_capital_efficiency_metrics(portfolio_policies, total_portfolio_premium)