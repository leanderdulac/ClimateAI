...
class InvestmentReturnCalculator:
    ...
    def __init__(self):
        ...
        self.climate_resilience_impact = 0.1  # How climate resilience affects investment returns
    ...
    def calculate_time_factor(self, policy_term_months: int) -> float:
        ...
        time_factor = self.base_time_factor * (1 + 0.02 * years)  # 2% per year time factor
    ...
    def calculate_investment_return(self, 
                                   policy_profile: PolicyInvestmentProfile) -> InvestmentReturnResult:
        """
        ...
        # TR = E[Retorno_investimento] × f_tempo_apólice
        total_return = policy_profile.initial_investment * expected_return_rate * time_factor
        
        # Calculate risk adjustment factor
        risk_adjustment_factor = self.calculate_risk_adjustment_factor(policy_profile.scr_score)
        
        # Apply climate resilience adjustment
        # Higher climate resilience means less risk adjustment, so less reduction in returns
        climate_resilience_factor = 1.0 - (self.climate_resilience_impact * (1.0 - policy_profile.climate_resilience_score))
        
        # Calculate adjusted return with both risk and climate resilience factors
        adjusted_return = total_return * risk_adjustment_factor * climate_resilience_factor
    ...
    def calculate_portfolio_investment_analysis(self, 
                                              policies: List[PolicyInvestmentProfile]) -> InvestmentPortfolioAnalysis:
        """
        ...
        average_return_ratio = (total_expected_returns / total_premium) if total_premium > 0 else 0.0
        risk_adjusted_return_ratio = (total_adjusted_returns / total_premium) if total_premium > 0 else 0.0
        ...
        portfolio_risk_metrics = {
            ...
            'portfolio_return_efficiency': total_adjusted_returns / (average_scr + 1) if average_scr >= 0 else 0,  # Avoid division issues
    ...
    def calculate_return_sensitivity(self, 
                                   policy_profile: PolicyInvestmentProfile,
                                   scr_scenarios: List[float]) -> Dict[str, Any]:
        """
        ...
        for scr in scr_scenarios:
            ...
            sensitivity_results[scr] = {
                ...
                'change_from_baseline': result.adjusted_return - base_result.adjusted_return,
                'change_percentage': ((result.adjusted_return - base_result.adjusted_return) / base_result.adjusted_return * 100) if base_result.adjusted_return != 0 else 0
            }
    ...
    def optimize_portfolio_allocation(self,
                                    ...
                                    target_return: float = 0.055) -> Dict[str, Any]:
        """
        ...
        infra_adjustment = (climate_resilience_target - 0.5) * 0.3  # Up to ±15% adjustment
        ...
        return {
            ...
            'recommendations': [
                f"Risk-free allocation: {adjusted_rf:.1%}" if adjusted_rf > base_rf_weight else f"Maintain current risk-free allocation: {adjusted_rf:.1%}",
                f"Increase infrastructure allocation to {adjusted_infra:.1%} for better climate resilience" if adjusted_infra > base_infra_weight else f"Maintain current infrastructure allocation: {adjusted_infra:.1%}",
                f"Equity allocation adjusted to {adjusted_eq:.1%}" if abs(adjusted_eq - base_eq_weight) > 0.01 else f"Maintain current equity allocation: {adjusted_eq:.1%}"
            ]
        }
...
def calculate_investment_return(policy_profile: PolicyInvestmentProfile) -> InvestmentReturnResult:
...
def calculate_portfolio_investment_analysis(policies: List[PolicyInvestmentProfile]) -> InvestmentPortfolioAnalysis:
...
def calculate_return_sensitivity(policy_profile: PolicyInvestmentProfile,
                               scr_scenarios: List[float]) -> Dict[str, Any]:
    """Convenience function to calculate return sensitivity analysis"""
    return investment_return_service.calculate_return_sensitivity(policy_profile, scr_scenarios)

def optimize_portfolio_allocation(scr_score: float,
                                climate_resilience_target: float = 0.7,
                                target_return: float = 0.055) -> Dict[str, Any]:
    """Convenience function to optimize portfolio allocation"""
    return investment_return_service.optimize_portfolio_allocation(scr_score, climate_resilience_target, target_return)