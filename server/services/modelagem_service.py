...
class ModelagemService:
            ...
            if latitude and longitude:
                for i in range(min(dias, 10)):  # Limitar para os primeiros 10 dias
                    ...
                    fator = {
                        ...
                        "tipo": random.choice(
                            ["temperatura", "precipitacao", "umidade"]
                        ),
    ...
    def obter_impacto_climatico(
        self, simbolo: str, latitude: float, longitude: float, periodo: int
    ) -> List[Dict]:
        """
        ...
        for i in range(0, periodo, 7):  # Semanalmente para visualização
            ...
            correlacao_temperatura = random.uniform(
                -0.3, 0.5
            )  # Mais calor pode afetar negativa/positivamente
            correlacao_precipitacao = random.uniform(
                -0.5, 0.3
            )  # Mais chuva pode afetar negativa/positivamente
            correlacao_eventos = random.uniform(
                0.3, 0.8
            )  # Eventos extremos normalmente tem impacto negativo
