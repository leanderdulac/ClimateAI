...
class AuditComplianceService:
    ...
    def __init__(
        self, db_path: str = "data/audit.db", log_retention_days: int = 2555
    ):  # 7 years
    ...
    def _cleanup_old_logs(self):
        ...
        with self._get_db_connection() as conn:
            ...
            cursor.execute(
                "DELETE FROM audit_logs WHERE created_at < ?", (cutoff_timestamp,)
            )
    ...
    def log_operation(
        ...
        self._check_for_alerts(
            audit_id, operation, status, risk_score, compliance_flags
        )

        logger.info(
            f"Audit log created: {operation} - {action} - {status} (ID: {audit_id})"
        )
        return audit_id
    ...
    def _check_compliance_violations(
        self, audit_log_id: str, flags: List[str], details: Dict[str, Any]
    ):
        ...
        with self._lock:
            with self._get_db_connection() as conn:
                ...
                for rule in rules:
                    rule_dict = dict(
                        zip([desc[0] for desc in cursor.description], rule)
                    )
                    ...
                    if self._evaluate_rule(rule_dict, flags, details):
                        ...
                        logger.warning(
                            f"Compliance violation detected: {rule_dict['rule_name']} (ID: {violation_id})"
                        )

                conn.commit()

    def _evaluate_rule(
        self, rule: Dict[str, Any], flags: List[str], details: Dict[str, Any]
    ) -> bool:
        ...
        with self._get_db_connection() as conn:
            ...
            for row in rows:
                ...
                if log_dict["compliance_flags"]:
                    log_dict["compliance_flags"] = json.loads(
                        log_dict["compliance_flags"]
                    )
    ...
    def get_compliance_report(
        ...
        with self._get_db_connection() as conn:
            ...
            # Get violations
            violations_query = " ".join([
                "SELECT",
                "v.*, r.rule_name, r.severity as rule_severity, r.description as rule_description",
            ...
            for row in cursor.fetchall():
                ...
                if violation["violation_details"]:
                    violation["violation_details"] = json.loads(
    ...
    def resolve_violation(
        self, violation_id: str, resolved_by: str, notes: Optional[str] = None
    ):
        """
        ...
        with self._lock:
            with self._get_db_connection() as conn:
                ...
                cursor.execute(
                    ...
                    WHERE id = ?
                """,
                    (datetime.now().timestamp(), resolved_by, notes, violation_id),
                )
    ...
    def _check_suspicious_patterns(
        self, operation: str, audit_log_id: str
    ) -> List[Dict[str, Any]]:
    ...
    def _create_alert(self, alert_data: Dict[str, Any]):
        ...
        with self._get_db_connection() as conn:
            ...
            # Log alert creation
            logger.warning(
                f"Alert created: {alert_data['type']} - {alert_data['message']}"
            )
