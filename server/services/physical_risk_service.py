...
class PhysicalRiskCalculator:
    ...
    def __init__(self):
        ...
        self.age_vulnerability_curve = lambda age: min(
            1.5, 1 + 0.01 * age
    ...
    def calculate_physical_risk(
        ...
        flood_lambda = self._calculate_occurrence_rate(
            "flood", climate_scenario.delta_temperature
        )
        ...
        wind_lambda = self._calculate_occurrence_rate(
            "wind", climate_scenario.delta_temperature
        )
        ...
        fire_lambda = self._calculate_occurrence_rate(
            "fire", climate_scenario.delta_temperature
        )
        ...
        hail_lambda = self._calculate_occurrence_rate(
            "hail", climate_scenario.delta_temperature
        )
        ...
        property_vulnerability = {
            ...
            "material_factor": self.vulnerability_coefs[
                property_char.construction_material
            ],
    ...
    def _calculate_event_probability(
        self, peril: str, delta_temp: float, precip_change: float, elevation: float
    ) -> float:
        """
        ...
        if peril == "flood":
            elevation_effect = max(
                0.01, min(2.0, 1.0 - elevation / 100.0)
            )  # Scaled effect
        else:
        ...
        precip_effect = (
            1 + (precip_change / 1000) * 0.5
        )  # 50% sensitivity to precipitation
    ...
    def _calculate_vulnerability(
        self, peril: str, property_char: PropertyCharacteristics
    ) -> float:
        """
        ...
        material_vuln = self.vulnerability_coefs[property_char.construction_material][
            peril
        ]
    ...
    def calculate_scenario_comparison(
        ...
        # Calculate changes
        risk_increase = (
            future_risk.total_physical_risk - baseline_risk.total_physical_risk
        )
        risk_ratio = (
            future_risk.total_physical_risk / baseline_risk.total_physical_risk
            if baseline_risk.total_physical_risk > 0
            else 1.0
        )
    ...
    def integrate_with_climate_risk(
        self, climate_risk_score: float, physical_risk_result: PhysicalRiskResult
    ) -> Dict[str, float]:
        """
        ...
        weighted_physical_risk = (
            climate_risk_score * physical_risk_result.total_physical_risk
        )

        # Calculate amplification factor
        amplification = (
            physical_risk_result.total_physical_risk / max(0.01, climate_risk_score)
            if climate_risk_score > 0
            else 1.0
        )
...
def calculate_scenario_comparison(
    ...
    return physical_risk_service.calculate_scenario_comparison(
        property_char, baseline_scenario, future_scenario
    )


def integrate_with_climate_risk(
    climate_risk_score: float, physical_risk_result: PhysicalRiskResult
) -> Dict[str, float]:
    """Convenience function to integrate with climate risk"""
    return physical_risk_service.integrate_with_climate_risk(
