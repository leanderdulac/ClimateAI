...
class MitigationResult:
    ...
    risk_classification: Optional[str] = (
        None  # Risk classification (risco_baixo, risco_moderado, etc.)
    )
...
class MitigationCalculator:
    ...
    def calculate_mitigation_score(
        self, mitigation_measures: MitigationMeasures
    ) -> MitigationResult:
        """
        ...
        drainage_factor = np.log(
            1
            + (mitigation_measures.drainage_capacity * mitigation_measures.area_drained)
            / 10000
        )
        drainage_component = self.measure_weights["drainage_system"] * drainage_factor

        resistance_factor = np.log(
            1 + mitigation_measures.structural_resistance_class / 5.0
        )  # Normalize resistance class
        resistance_component = (
            self.measure_weights["structural_resistance"] * resistance_factor
        )

        monitoring_factor = np.log(
            1 + mitigation_measures.iot_sensors_count / 10.0
        )  # Normalize sensor count
        monitoring_component = (
            self.measure_weights["monitoring_system"] * monitoring_factor
        )

        vegetation_factor = np.log(
            1 + mitigation_measures.local_ndvi
        )  # NDVI already in 0-1 range
        vegetation_component = (
            self.measure_weights["vegetation_cover"] * vegetation_factor
        )

        refuge_factor = np.log(
            1 + (1.0 / max(0.1, mitigation_measures.refuge_distance_km))
        )  # Inverse distance
        ...
        normalized_mitigation = min(
            1.0, total_mitigation_score / 2.0
        )  # Normalize based on expected max
    ...
    def calculate_final_scr_score(
        ...
        original_total_risk = (
            risk_components.physical_risk
            + risk_components.transition_risk
    ...
    def classify_scr_risk(self, scr_score: float) -> str:
        ...
        if scr_score < self.scr_risk_thresholds["low"][1]:  # < 300
            ...
            self.scr_risk_thresholds["moderate"][0]
            <= scr_score
            < self.scr_risk_thresholds["moderate"][1]
        ):  # 300 <= SCR < 600
            return "risco_moderado"
        elif (
            self.scr_risk_thresholds["high"][0]
            <= scr_score
            < self.scr_risk_thresholds["high"][1]
        ):  # 600 <= SCR < 800
            return "risco_alto"
    ...
    def evaluate_mitigation_portfolio(
        self,
        mitigation_measures: MitigationMeasures,
        desired_risk_reduction: float = 0.30,
    ) -> Dict[str, Any]:
        """
        ...
        for comp_name, weight in component_weights.items():
            ...
            effectiveness_per_component[comp_name] = {
                ...
                "effectiveness_percentage": (
                    (component_value / total_weight) * 100 if total_weight > 0 else 0
                ),
            }

        # Calculate gap to desired risk reduction
        gap_to_target = max(
            0, desired_risk_reduction - mitigation_result.mitigation_score
        )

        # Recommendations for improvement
        recommendations = self._generate_recommendations(
            mitigation_measures, gap_to_target
        )
        ...
        return {
            ...
            "portfolio_balance_score": self._calculate_portfolio_balance(
                mitigation_measures
            ),
    ...
    def _generate_recommendations(
        self, measures: MitigationMeasures, gap_to_target: float
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_portfolio_balance(self, measures: MitigationMeasures) -> float:
        ...
        drain_norm = min(
            1.0, measures.drainage_capacity * measures.area_drained / 50000
        )  # Normalize drainage
        struct_norm = (
            measures.structural_resistance_class / 10.0
        )  # Wind resistance (1-10)
        sensor_norm = min(
            1.0, measures.iot_sensors_count / 20.0
        )  # Normalized sensor count
        veg_norm = measures.local_ndvi  # NDVI already normalized
        refuge_norm = max(
            0.0, 1.0 - (measures.refuge_distance_km / 10.0)
        )  # Inverse of distance
    ...
    def calculate_cost_benefit_ratio(
        ...
        mitigation_result = self.calculate_final_scr_score(
        ...
        if mitigation_cost > 0:
            ...
            benefit_multiple = (
                risk_reduction_value / mitigation_cost if mitigation_cost > 0 else 0
            )
        else:
        ...
        roi_percentage = (
            (risk_reduction_value / mitigation_cost) * 100 if mitigation_cost > 0 else 0
        )
        ...
        return {
            ...
            "net_present_value": (
                risk_reduction_value - mitigation_cost
                if mitigation_cost > 0
                else risk_reduction_value
            ),
...
def calculate_final_scr_score(
    risk_components: ClimateRiskComponents, mitigation_measures: MitigationMeasures
) -> MitigationResult:
    """Convenience function to calculate final SCR score"""
    return mitigation_service.calculate_final_scr_score(
        risk_components, mitigation_measures
    )


def evaluate_mitigation_portfolio(
    mitigation_measures: MitigationMeasures, desired_risk_reduction: float = 0.30
) -> Dict[str, Any]:
    """Convenience function to evaluate mitigation portfolio"""
    return mitigation_service.evaluate_mitigation_portfolio(
        mitigation_measures, desired_risk_reduction
    )
...
def calculate_cost_benefit_ratio(
    ...
    return mitigation_service.calculate_cost_benefit_ratio(
        mitigation_measures, risk_components, mitigation_cost
    )
