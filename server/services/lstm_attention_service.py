...
class ClimateAttentionLSTM(nn.Module):
    ...
    def attention(self, lstm_output, final_hidden):
        ...
        # Shape: (batch_size, seq_len, hidden_size)
        tanh_scores = torch.tanh(self.W_h(lstm_output) + self.W_c(context_expanded))
...
class ClimateAttentionService:
    ...
    def prepare_climate_features(
        ...
        """
        if not all(
            len(lst) == len(temperature)
            for lst in [precipitation, pressure, nao_index, enso_phase]
        ):
    ...
    def normalize_features(self, features: np.ndarray) -> np.ndarray:
        ...
        if self.scaler["mean"] is None:
            ...
            self.scaler["std"] = np.where(
                self.scaler["std"] == 0, 1, self.scaler["std"]
            )
    ...
    def create_sequences(
        self, features: np.ndarray, targets: np.ndarray, sequence_length: int
    ...
    def train_model(
        ...
        features = self.prepare_climate_features(
            temperature, precipitation, pressure, nao_index, enso_phase
        )
        ...
        X, y = self.create_sequences(
            normalized_features, np.array(targets), sequence_length
        )
    ...
    def predict(
        ...
        if len(features) < sequence_length:
            raise ValueError(
                f"Need at least {sequence_length} data points for prediction"
            )
    ...
    def predict_with_attention_visualization(
        self,
        temperature: List[float],
        precipitation: List[float],
        pressure: List[float],
        ...
        # Create sequences
        X, y_actual = self.create_sequences(
            normalized_features, np.array(targets), sequence_length
        )
