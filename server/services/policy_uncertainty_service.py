...
class CalibrationEvent:
    ...
    event_id: str
    event_type: str  # "CMIP_data_release", "extreme_weather_event", "model_update", etc.
...
class PolicyUncertaintyService:
    ...
    def create_uncertainty_clause(self, 
        ...
        if uncertainty_min < 0 or uncertainty_max > 1 or uncertainty_min > uncertainty_max:
            raise ValueError(f"Uncertainty values must be between 0 and 1, with min <= max. Got: {uncertainty_min}, {uncertainty_max}")
        ...
        logger.info(f"Created uncertainty clause for policy {policy_id} with {uncertainty_min*100:.0f}-{uncertainty_max*100:.0f}% uncertainty")
        return clause
    
    def get_uncertainty_clause(self, policy_id: str) -> Optional[PolicyUncertaintyClause]:
        """Get the uncertainty clause for a specific policy"""
        return self.active_clauses.get(policy_id)
    
    def update_clause_for_cmip_data(self, policy_id: str, new_cmip_source: str) -> PolicyUncertaintyClause:
        """Update a policy's clause when new CMIP data is released"""
        clause = self.active_clauses.get(policy_id)
        if not clause:
            raise ValueError(f"Policy {policy_id} does not have an active uncertainty clause")
        
        # Update the CMIP source and reset the review date
        clause.cmip_data_source = new_cmip_source
        clause.last_updated = datetime.now()
        clause.next_review_date = datetime.now() + timedelta(days=365)  # Reset to annual review
        ...
        logger.info(f"Updated uncertainty clause for policy {policy_id} with new CMIP source: {new_cmip_source}")
        return clause
    
    def register_calibration_event(self, 
                                 event_type: str,
                                 event_description: str,
                                 policies_affected: List[str],
                                 impact_on_uncertainty: float = 0.1) -> CalibrationEvent:
        """
        ...
        if event_type not in self.calibration_event_types:
            raise ValueError(f"Invalid event type. Valid types: {self.calibration_event_types}")
        
        if impact_on_uncertainty < 0 or impact_on_uncertainty > 1:
            raise ValueError("Impact on uncertainty must be between 0.0 and 1.0")
        
        event_id = f"CE_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{event_type[:3].upper()}"
        ...
        for policy_id in policies_affected:
            if policy_id in self.active_clauses:
                ...
                days_to_review = max(30, int(365 * (1 - impact_on_uncertainty)))  # Higher impact = sooner review
                clause.next_review_date = datetime.now() + timedelta(days=days_to_review)
        
        logger.info(f"Registered calibration event {event_id} affecting {len(policies_affected)} policies")
        return event
    
    def get_policies_requiring_review(self, as_of_date: Optional[datetime] = None) -> List[str]:
        """
    ...
    def calculate_adjusted_uncertainty(self, 
                                    base_uncertainty: float,
                                    calibration_events: List[CalibrationEvent]) -> float:
        """
        ...
        original_clause = self.active_clauses.get(calibration_events[0].policies_affected[0]) if calibration_events else None
        if original_clause:
            min_uncertainty = original_clause.uncertainty_range_min
            max_uncertainty = original_clause.uncertainty_range_max
            adjusted_uncertainty = max(min_uncertainty, min(max_uncertainty, adjusted_uncertainty))
...
def create_uncertainty_clause(policy_id: str,
                           uncertainty_min: Optional[float] = None,
                           uncertainty_max: Optional[float] = None,
                           projection_horizon: Optional[int] = None,
                           cmip_source: Optional[str] = None) -> PolicyUncertaintyClause:
...
def update_clause_for_cmip_data(policy_id: str, new_cmip_source: str) -> PolicyUncertaintyClause:
    """Convenience function to update a clause for new CMIP data"""
    return policy_uncertainty_service.update_clause_for_cmip_data(policy_id, new_cmip_source)

def register_calibration_event(event_type: str,
                             event_description: str,
                             policies_affected: List[str],
                             impact_on_uncertainty: float = 0.1) -> CalibrationEvent:
...
def calculate_adjusted_uncertainty(base_uncertainty: float,
                                calibration_events: List[CalibrationEvent]) -> float:
    """Convenience function to calculate adjusted uncertainty"""
    return policy_uncertainty_service.calculate_adjusted_uncertainty(base_uncertainty, calibration_events)