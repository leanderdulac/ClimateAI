...
class PropertyExposure:
    ...
    property_type: str  # Property type (residential, commercial, etc.) determines f_o factor
...
class LEICalculator:
    ...
    def calculate_climate_adjusted_rate(self, base_rate: float, scr_score: float,
        ...
        capped_scr = min(normalized_scr, 5.0)  # Cap at 5x normalized value to prevent explosion
    ...
    def calculate_lei_score(self,
        ...
        gamma = custom_gamma if custom_gamma is not None else regional_data.climate_sensitivity_gamma
        delta = custom_delta if custom_delta is not None else regional_data.climate_exponentiality_delta

        # Calculate climate-adjusted rate
        climate_adjusted_rate = self.calculate_climate_adjusted_rate(base_rate, scr_score, gamma, delta)

        # Get occupational factor (f_o)
        if custom_occupational_factor is not None:
            occupational_factor = custom_occupational_factor
        else:
            base_occupational_factor = self.occupational_factors.get(property_exposure.property_type.lower(), 1.0)
            occupational_factor = base_occupational_factor * property_exposure.occupancy_factor
        ...
        max_reasonable_lei = property_exposure.exposed_value * 5.0  # Maximum 5x exposure value
...
def calculate_lei_score(property_exposure: PropertyExposure,
                      region_id: str,
                      scr_score: float,
                      custom_occupational_factor: Optional[float] = None,
                      custom_gamma: Optional[float] = None,
    ...
    return lei_calculator.calculate_lei_score(
        property_exposure, region_id, scr_score,
        custom_occupational_factor, custom_gamma, custom_delta
    )