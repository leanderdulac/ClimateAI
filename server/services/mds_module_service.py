...
class UnderwritingDecisionEngine:
    ...
    def make_underwriting_decision(self, application: ApplicationData, 
                                 module_inputs: ModuleInputs) -> UnderwritingDecision:
        """
        ...
        profit_margin = (commercial_premium - actuarial_premium) / commercial_premium if commercial_premium > 0 else 0
        
        # Determine primary decision based on risk score
        decision = self._determine_primary_decision(risk_score, application, module_inputs)
        ...
        premium_adjustment = self._calculate_premium_adjustment(risk_score, commercial_premium, decision)
        ...
        justification = self._generate_justification(
            decision, risk_score, profit_margin, conditions, 
            module_inputs, application
        )
    ...
    def _determine_primary_decision(self, risk_score: float, application: ApplicationData, 
                                  module_inputs: ModuleInputs) -> Decision:
        """Determine the primary underwriting decision based on risk score"""
        # Check automated decision limits
        if application.coverage_requested > self.automated_decision_limits['max_coverage']:
    ...
    def _determine_conditions(self, risk_score: float, decision: Decision) -> List[Condition]:
        ...
        if decision == Decision.REJECT:
            ...
            if risk_score > 0.9:
                conditions.extend([Condition.DEDUCTIBLE_INCREASE, Condition.COVERAGE_LIMITATION])
            return conditions
        else:  # Manual review
            return [Condition.ADDITIONAL_INSPECTION]

    def _calculate_premium_adjustment(self, risk_score: float, commercial_premium: float, 
    ...
    def _determine_coverage_modifications(self, risk_score: float, coverage_requested: float, 
    ...
    def _generate_justification(self, decision: Decision, risk_score: float,
                              profit_margin: float, conditions: List[Condition],
                              module_inputs: ModuleInputs, application: ApplicationData) -> List[str]:
        ...
        if risk_score < 0.5:
            ...
            justification.append("Climate risk is moderate, manageable with standard procedures")
        elif risk_score < 0.85:
            justification.append("Climate risk is elevated, requiring additional risk management")
        elif risk_score < 0.95:
            justification.append("Climate risk is high, requiring significant risk mitigation")
        else:
        ...
        if decision != Decision.REJECT:
            ...
            justification.append(f"Actuarial premium: ${act_premium:,.2f}, Commercial premium: ${comm_premium:,.2f}")
            
            if profit_margin >= self.profitability_thresholds['minimum_acceptable']:
                justification.append(f"Projected profit margin of {profit_margin:.1%} meets minimum requirements")
            else:
                justification.append(f"Projected profit margin of {profit_margin:.1%} is below minimum acceptable ({self.profitability_thresholds['minimum_acceptable']:.0%})")
        ...
        if decision == Decision.ACCEPT:
            ...
            justification.append(f"Application conditionally accepted with: {', '.join(condition_names)}")
        elif decision == Decision.ACCEPT_WITH_RISK_LOADINGS:
            justification.append(f"Significant risk loadings applied due to high climate risk")
        elif decision == Decision.REJECT:
            justification.append("Application rejected due to excessive risk or unprofitability")
        elif decision == Decision.REQUIRES_MANUAL_REVIEW:
            justification.append("Application requires manual review due to high risk or special circumstances")
        
        return justification

    def _calculate_confidence_level(self, module_inputs: ModuleInputs,
                                  application: ApplicationData, decision: Decision) -> float:
        ...
        climate_data_quality = min(1.0, module_inputs.climate_risk_score + 0.3)  # Invert risk for quality
        ...
        if claim_data:
            ...
            claim_factor = min(1.0, days_since_claim / 1095)  # Max confidence after 3 years
        else:
        ...
        confidence = (climate_data_quality * 0.25 + 
                     claim_factor * 0.25 + 
                     market_factor * 0.25 + 
                     risk_factor * 0.25)
    ...
    def _requires_manual_review(self, risk_score: float, premium_adjustment: float, 
                              application: ApplicationData, decision: Decision) -> bool:
        ...
        if (module_inputs.climate_risk_breakdown and 
            sum(1 for v in module_inputs.climate_risk_breakdown.values() if v > 0.8) > 2):
            # More than 2 risk factors above 0.8
            return True
            
        return False

    def apply_policy_rules(self, application: ApplicationData,
                          module_inputs: ModuleInputs) -> Dict[str, Any]:
        """
        ...
        if 'agriculture' in coverage_type or 'crop' in coverage_type:
            ...
            if module_inputs.climate_risk_breakdown.get('flood_risk', 0) > 0.7:
                results['additional_conditions'].append('Increased flood monitoring required')
            
            # Soil/land type considerations
            soil_quality = application.applicant_profile.get('soil_quality', 'medium')
            if soil_quality == 'poor':
                results['overrides'].append('Additional premium load for poor soil quality')
            ...
            if building_age > 50:
                results['additional_conditions'].append('Historical property inspection required')
        ...
        if abs(lat) < 23.5:  # Tropical region
            results['additional_conditions'].append('Tropical climate risk factors applied')
        
        return results

    def calculate_risk_tolerances(self, applicant_profile: Dict[str, Any], 
                                coverage_type: str) -> Dict[str, float]:
        """
        ...
        if 'experimental' in coverage_type:
            tolerances['max_climate_risk_score'] = 0.70  # Lower tolerance for experimental
        ...
        if applicant_profile.get('portfolio_size', 1) > 100:
            ...
            tolerances['max_climate_risk_score'] = min(0.95, tolerances['max_climate_risk_score'] + 0.05)
        
        return tolerances

    def get_decision_tree_path(self, risk_score: float, profit_margin: float, 
                             coverage_amount: float) -> List[str]:
        """
...
def get_decision_tree_path(risk_score: float, profit_margin: float, 
                         coverage_amount: float) -> List[str]:
