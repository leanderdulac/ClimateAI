...
class PerformanceDelta:
    ...
    improvement: bool  # True if positive for beneficial metrics, negative for harmful ones
...
class SIPSPerformanceAnalyticsService:
    ...
    def create_performance_snapshot(self,
                                  ...
                                  baseline_period: str = "custom") -> PerformanceSnapshot:
        """
    ...
    def calculate_performance_deltas(self,
                                   before: PerformanceSnapshot,
                                   after: PerformanceSnapshot) -> List[PerformanceDelta]:
        """
        ...
        for metric_enum, metric_name in metrics_to_compare:
            ...
            percentage_change = (absolute_change / before_val * 100) if before_val != 0 else 0
    ...
    def calculate_overall_impact_score(self, deltas: List[PerformanceDelta]) -> Tuple[float, float]:
        """
        ...
        for delta in deltas:
            ...
            if weight > 0 and metric_enum:
                ...
                if direction == "increase":
                    ...
                    score = delta.absolute_change * weight if delta.absolute_change > 0 else -abs(delta.absolute_change) * weight
                else:
        ...
        if total_weight > 0:
            normalized_score = ((total_score / total_weight) + 50) * 10  # Adjust scale appropriately
            overall_score = max(0, min(100, normalized_score))  # Clamp between 0 and 100
        else:
            overall_score = 50.0  # Neutral score if no weights
        
        # Calculate confidence based on number of metrics with significant changes
        significant_changes = sum(1 for d in deltas if abs(d.percentage_change) > 2.0)  # More than 2% change
        confidence_level = min(1.0, significant_changes / len(deltas)) if deltas else 0.0
    ...
    def generate_performance_report(self,
                                  report_id: str,
                                  baseline_snapshot: PerformanceSnapshot,
                                  current_snapshot: PerformanceSnapshot) -> PerformanceReport:
        """
        ...
        key_insights = self._generate_key_insights(deltas, baseline_snapshot, current_snapshot)
    ...
    def _generate_recommendations(self, deltas: List[PerformanceDelta]) -> List[str]:
        ...
        for delta in deltas:
            if delta.metric_name == "Taxa de sinistralidade" and delta.improvement:
                recommendations.append("Continue current risk assessment strategies - claim rate improvement validated")
            elif delta.metric_name == "Taxa de sinistralidade" and not delta.improvement:
                recommendations.append("Review risk assessment models - claim rate deterioration detected")
            
            if delta.metric_name == "Sinistralidade climática" and delta.improvement:
                recommendations.append("Continue climate risk modeling improvements - climate losses reduced")
            elif delta.metric_name == "Sinistralidade climática" and not delta.improvement:
                recommendations.append("Enhance climate risk modeling - climate losses increased")
            
            if delta.metric_name == "Margem líquida" and delta.improvement:
                recommendations.append("Maintain pricing strategies - net margin improvement achieved")
    ...
    def _generate_key_insights(self,
                             ...
                             current: PerformanceSnapshot) -> List[str]:
        ...
        insights.append(f"Overall: {improvements}/{total_metrics} key metrics showed improvement")
        
        # Specific metric insights
        for delta in deltas:
            if abs(delta.percentage_change) >= 5:  # Significant change (5% or more)
                change_direction = "improvement" if delta.improvement else "deterioration"
                insights.append(f"{delta.metric_name}: {change_direction} of {delta.percentage_change:+.1f}%")
        
        # ROI implications
        if baseline.average_premium != 0:
            premium_increase = ((current.average_premium - baseline.average_premium) / baseline.average_premium) * 100
            if premium_increase > 0:
                insights.append(f"Premium increase of {premium_increase:+.1f}% indicates improved risk pricing")
    ...
    def calculate_sips_impact_score(self, snapshot: PerformanceSnapshot) -> float:
        ...
        metrics = [
            (PerformanceMetric.CLAIM_RATE, snapshot.claim_rate, self.baseline_metrics[PerformanceMetric.CLAIM_RATE], self.target_metrics[PerformanceMetric.CLAIM_RATE]),
            (PerformanceMetric.CLIMATE_LOSS_RATE, snapshot.climate_loss_rate, self.baseline_metrics[PerformanceMetric.CLIMATE_LOSS_RATE], self.target_metrics[PerformanceMetric.CLIMATE_LOSS_RATE]),
            (PerformanceMetric.NET_MARGIN, snapshot.net_margin, self.baseline_metrics[PerformanceMetric.NET_MARGIN], self.target_metrics[PerformanceMetric.NET_MARGIN]),
            (PerformanceMetric.AVERAGE_PREMIUM, snapshot.average_premium, self.baseline_metrics[PerformanceMetric.AVERAGE_PREMIUM], self.target_metrics[PerformanceMetric.AVERAGE_PREMIUM]),
            (PerformanceMetric.CLIENT_RETENTION, snapshot.client_retention, self.baseline_metrics[PerformanceMetric.CLIENT_RETENTION], self.target_metrics[PerformanceMetric.CLIENT_RETENTION]),
        ]
        ...
        for metric_enum, current_val, baseline_val, target_val in metrics:
            ...
            if current_val == baseline_val:
                ...
                if self.metric_directions[metric_enum] == "decrease":
                    ...
                    progress = max(0, min(1, (baseline_distance - current_distance) / baseline_distance)) if baseline_distance != 0 else 0
                else:
    ...
    def get_performance_trend(self, 
        ...
        relevant_snapshots = [
            snap for snap in self.performance_snapshots.values()
        ...
        if len(values) < 2:
            ...
            if metric in [PerformanceMetric.CLAIM_RATE, PerformanceMetric.CLIMATE_LOSS_RATE]:
...
def create_performance_snapshot(snapshot_id: str,
                              date: datetime,
                              claim_rate: float,
                              climate_loss_rate: float,
                              net_margin: float,
    ...
    return sips_analytics_service.create_performance_snapshot(
        snapshot_id, date, claim_rate, climate_loss_rate, net_margin,
        rejection_rate, average_premium, client_retention, economic_capital, baseline_period
    )

def calculate_performance_deltas(before: PerformanceSnapshot, after: PerformanceSnapshot) -> List[PerformanceDelta]:
...
def generate_performance_report(report_id: str,
                              baseline_snapshot: PerformanceSnapshot,
                              current_snapshot: PerformanceSnapshot) -> PerformanceReport:
