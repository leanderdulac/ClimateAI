...
class PolicyPricingInput:
    ...
    zone_concentration_thresholds: Optional[Dict[str, float]] = None  # Thresholds for concentration adjustment
...
class ComprehensivePricingCalculator:
    ...
    def calculate_zone_concentration_ratio(self, zone_policies_premiums: List[float], 
                                         free_capital: float) -> float:
        """
    ...
    def calculate_comprehensive_pricing(self, pricing_input: PolicyPricingInput) -> PolicyAnalysisReport:
        """
        ...
        zone_concentration_ratio = self.calculate_zone_concentration_ratio(
            pricing_input.zone_policies_premiums,
            pricing_input.free_capital
        )

        # Calculate supply-demand adjustment
        supply_demand_adjustment = self.calculate_supply_demand_adjustment(zone_concentration_ratio)
        ...
        subscription_cost = (
            ...
            if len(pricing_input.zone_policies_premiums) > 10 else  # If zone has many policies, use automated
            self.subscription_cost_manual
        )

        claims_processing_cost = pricing_input.pure_theoretical_premium * self.claims_processing_rate
        administrative_cost = pricing_input.pure_theoretical_premium * self.administrative_rate
        
        # Determine risk level based on zone concentration
        risk_level = self._get_risk_level(zone_concentration_ratio * 1000)  # Scale to match SCR ranges
        
        # Calculate risk-adjusted scores (simplified)
        scr_score = (zone_concentration_ratio * 1000) if zone_concentration_ratio <= 1 else 800.0  # Cap SCR
        ...
        return PolicyAnalysisReport(
            ...
            climate_risk_breakdown={
                ...
                'concentration_level': self.determine_concentration_level(zone_concentration_ratio),
            ...
            decision="APPROVED" if scr_score < 700 else "CONDITIONAL" if scr_score < 800 else "REJECTED",
            ...
            processing_method="automated" if len(pricing_input.zone_policies_premiums) > 10 else "manual",
    ...
    def calculate_operating_costs(self, 
                                 ...
                                 claim_history_count: int = 0) -> Dict[str, float]:
        """
        ...
        subscription_cost = (
            self.subscription_cost_automated if processing_method.lower() == "automated"
        ...
        operating_cost_ratio = total_operating_costs / premium_issued if premium_issued > 0 else 0.0
        ...
        return {
            ...
            'cost_breakdown': {
                'subscription_percentage': subscription_cost / total_operating_costs if total_operating_costs > 0 else 0.0,
                'claims_percentage': claims_cost / total_operating_costs if total_operating_costs > 0 else 0.0,
                'admin_percentage': admin_cost / total_operating_costs if total_operating_costs > 0 else 0.0
            },
            'risk_category_multiplier': risk_factors.get(risk_category.lower(), 1.0),
            'coverage_type_multiplier': coverage_factors.get(coverage_type.lower(), 1.0)
        }

    def calculate_profitability_analysis(self, 
                                       premium_issued: float,
                                       expected_claims: float,
                                       operating_costs: float) -> Dict[str, float]:
        """
        ...
        profit_margin = max(0, net_income) / premium_issued if premium_issued > 0 else 0.0
        ...
        combined_ratio = (expected_claims + operating_costs) / premium_issued if premium_issued > 0 else float('inf')
        ...
        return {
            ...
            'premium_loading_needed': (expected_claims + operating_costs) / premium_issued if premium_issued > 0 else float('inf')
        }

# Global instance
comprehensive_pricing_service = ComprehensivePricingCalculator()

def calculate_comprehensive_premium(pricing_input: PolicyPricingInput) -> PolicyAnalysisReport:
...
def calculate_zone_concentration_ratio(zone_policies_premiums: List[float], 
                                     free_capital: float) -> float:
    """Convenience function to calculate zone concentration ratio"""
    return comprehensive_pricing_service.calculate_zone_concentration_ratio(zone_policies_premiums, free_capital)
...
def calculate_supply_demand_adjustment(concentration_ratio: float) -> float:
    ...
    return comprehensive_pricing_service.calculate_supply_demand_adjustment(concentration_ratio)

def calculate_operating_costs(premium_issued: float,
                           processing_method: str = "automated",
                           risk_category: str = "standard", 
                           coverage_type: str = "property",
                           claim_history_count: int = 0) -> Dict[str, float]:
    ...
    return comprehensive_pricing_service.calculate_operating_costs(
        premium_issued, processing_method, risk_category, coverage_type, claim_history_count
    )

def calculate_profitability_analysis(premium_issued: float,
                                   expected_claims: float,
                                   operating_costs: float) -> Dict[str, float]:
