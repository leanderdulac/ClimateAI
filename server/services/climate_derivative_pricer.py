...
class ClimateDerivativePricer:
    ...
    def fit_gaussian_process(
        self, historical_temps: np.ndarray
        ...
        kernel = RBF(length_scale=5.0, length_scale_bounds=(1e-1, 1e2)) + WhiteKernel(
            noise_level=0.1
        )
            ...
            for param_name, param_value in params.items():
                ...
                if hasattr(param_value, "bounds"):
                    ...
                    if len(bounds) == 2:
                        lower, upper = bounds
                        if (
                            abs(param_value - lower) < 1e-3
                            or abs(param_value - upper) < 1e-3
                        ):
                            boundary_warnings.append(
                                f"{param_name}: {param_value} at boundary [{lower}, {upper}]"
                            )

            if boundary_warnings:
                logger.warning(f"GP parameters at boundaries: {boundary_warnings}")
                logger.info(
                    "Consider using alternative kernel or expanding parameter bounds"
                )
    ...
    def predict_future_temperature(
        self, target_year: int = 2025, iam_adjustment: float = 0.5
    ) -> Tuple[float, float]:
        """
        ...
        if self.gp_model is None:
            raise ValueError(
                "Gaussian Process model not fitted. Call fit_gaussian_process first."
            )
        ...
        logger.info(
            f"Temperature prediction for {target_year}: mean={mean_temp[0]:.2f}°F, std={std_temp[0]:.2f}°F"
        )
        logger.info(
            f"Volatility factors: time={volatility_multiplier:.2f}, temp_change={temp_change_factor:.2f}"
        )

        return float(mean_temp[0]), float(std_temp[0])

    def analyze_capital_requirements(
        self, ask_price: float, initial_capital: float = 1000000
    ) -> Dict:
        """
        ...
        total_investment = (
            min(contracts_affordable, 1.0) * ask_price
        )  # Máximo 1 contrato
    ...
    def calculate_cdd_and_payouts(
        self, simulated_temps: np.ndarray
    ...
    def price_derivative(
        self, expected_payout: float, cvar_95: float, months_to_expiry: float = 3
    ) -> Dict[str, float]:
        """
        Calcula preços bid/ask do derivativo
        """
        # Preço base descontado
        base_price = expected_payout / (1 + self.discount_rate) ** (
            months_to_expiry / 12
        )
        ...
        risk_component = (
            cvar_95
            * self.risk_adjustment_issuer
            / (1 + self.discount_rate) ** (months_to_expiry / 12)
        )
    ...
    def sensitivity_analysis(
        self, mean_temp: float, std_temp: float, temp_changes: List[float] = [-1, 0, 1]
        ...
        for delta_temp in temp_changes:
            ...
            prices = self.price_derivative(
                risk_metrics["expected_payout"], risk_metrics["cvar_95"]
            )
    ...
    def get_inmet_data(
        ...
        try:
            url = (
                f"{self.inmet_base_url}/estacao/{start_date}/{end_date}/{station_code}"
            )
            ...
            if response.status_code == 200:
                ...
                if data:
                    ...
                    logger.info(
                        f"INMET data retrieved: {temp_mean:.2f}°C for station {station_code}"
                    )
                    return temp_mean

            logger.warning(
                f"Failed to retrieve INMET data: HTTP {response.status_code}"
            )
            return None
    ...
    def price_climate_derivative(
        ...
        """
        logger.info(
            f"Pricing climate derivative for {target_year} - Scenario: {scenario_name}"
        )
        ...
        mean_temp, std_temp = self.predict_future_temperature(
            target_year, iam_adjustment
        )
        ...
        # 7. Precificação
        prices = self.price_derivative(
            risk_metrics["expected_payout"], risk_metrics["cvar_95"], months_to_expiry
        )
        ...
        logger.info(
            f"Derivative pricing completed for {scenario_name}: Bid=${prices['bid_price']:,.2f}, Ask=${prices['ask_price']:,.2f}"
        )
        return result
...
def integrate_messageix_projection(
    base_temp_projection: float, economic_factors: Dict = None
) -> float:
    """
