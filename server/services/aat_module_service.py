...
class HistoricalLossData:
    ...
    claims_history: List[Dict[str, Any]]  # [{'date': '2023-01-01', 'amount': 10000, 'cause': 'flood'}]
...
class TraditionalActuarialEngine:
    ...
    def perform_actuarial_analysis(self, loss_data: HistoricalLossData) -> ActuarialAnalysisResult:
        """
        ...
        frequency = self._calculate_frequency(loss_data.claims_history, loss_data.coverage_period_years)
        severity = self._calculate_severity(loss_data.claims_history)
        
        # Calculate basic statistics
        expected_loss = frequency * severity
        variance = self._calculate_variance(loss_data.claims_history, frequency, severity)
        ...
        pure_premium = expected_loss / loss_data.policy_count if loss_data.policy_count > 0 else 0
        ...
        margin_of_error = z_score * (std_dev / np.sqrt(len(loss_data.claims_history))) if loss_data.claims_history else 0
        confidence_interval = (
    ...
    def _calculate_frequency(self, claims_history: List[Dict], coverage_period: float) -> float:
    ...
    def _calculate_severity(self, claims_history: List[Dict]) -> float:
        ...
        claim_amounts = [claim['amount'] for claim in claims_history if 'amount' in claim]
        if not claim_amounts:
            return 5000
        
        return np.mean(claim_amounts)

    def _calculate_variance(self, claims_history: List[Dict], frequency: float, severity: float) -> float:
        ...
        severity_var = np.var(claim_amounts) if len(claim_amounts) > 1 else severity ** 2 * 0.5
        aggregate_variance = frequency * severity_var + (severity ** 2) * frequency
        
        return aggregate_variance

    def _calculate_risk_premium(self, expected_loss: float, std_dev: float, loss_data: HistoricalLossData) -> float:
        ...
        volatility_premium = 0.05 * expected_loss * (std_dev / expected_loss if expected_loss > 0 else 0)  # Coefficient of variation factor
        
        # Geographic risk adjustment
        geo_factor = self._calculate_geographic_risk_factor(loss_data.location_coordinates)
        
        risk_premium = (variance_premium + volatility_premium) * geo_factor
        return risk_premium

    def _calculate_geographic_risk_factor(self, coordinates: Tuple[float, float]) -> float:
    ...
    def _calculate_actuarial_indicators(self, frequency: float, severity: float, 
                                     expected_loss: float, std_dev: float, 
                                     loss_data: HistoricalLossData) -> Dict[str, float]:
        """Calculate various actuarial indicators"""
        indicators = {}
        
        # Loss ratio (if premium data available)
        indicators['loss_ratio'] = expected_loss / (frequency * severity) if frequency * severity > 0 else 0.0
        
        # Combined ratio component (expense + loss ratio, simplified)
        indicators['combined_ratio_component'] = (expected_loss + severity * 0.2) / (frequency * severity * 1.3) if frequency * severity > 0 else 0.0
        
        # Pure premium indicator
        indicators['pure_premium_per_unit'] = severity * frequency
        
        # Risk load factor
        indicators['risk_load_factor'] = std_dev / expected_loss if expected_loss > 0 else 0.0
        
        # Coefficient of variation squared (measure of risk)
        indicators['variance_to_mean_ratio'] = (std_dev ** 2) / expected_loss if expected_loss > 0 else 0.0
        
        # Historical loss ratio (actual losses / total exposure)
        total_historical_losses = sum(claim.get('amount', 0) for claim in loss_data.claims_history)
        indicators['historical_loss_ratio'] = total_historical_losses / loss_data.total_exposure if loss_data.total_exposure > 0 else 0.0
    ...
    def perform_experience_rating(self, policy_id: str, historical_premiums: List[float], 
                                historical_losses: List[float]) -> float:
        """
    ...
    def calculate_reinsurance_requirements(self, expected_loss: float, std_dev: float) -> Dict[str, float]:
        """
        ...
        retention_95 = max(expected_loss * 2, 100000)  # Retain up to 2x expected loss or $100k
        retention_99 = max(expected_loss * 3, 250000)  # Higher retention for extreme events
    ...
    def get_tariff_classification(self, risk_factors: Dict[str, float], 
                                coverage_type: RiskCategory) -> str:
        """
...
def perform_actuarial_analysis(loss_data: HistoricalLossData) -> ActuarialAnalysisResult:
    """Convenience function to perform actuarial analysis"""
    return aat_engine.perform_actuarial_analysis(loss_data)

def perform_experience_rating(policy_id: str, historical_premiums: List[float], 
                            historical_losses: List[float]) -> float:
    """Convenience function to calculate experience rating"""
    return aat_engine.perform_experience_rating(policy_id, historical_premiums, historical_losses)

def calculate_reinsurance_requirements(expected_loss: float, std_dev: float) -> Dict[str, float]:
...
def get_tariff_classification(risk_factors: Dict[str, float], 
                            coverage_type: RiskCategory) -> str:
