...
class BayesianBootstrapService:
    ...
    def sample_posterior_parameters(
        self, data: List[float], prior_alpha: float = 2.0, prior_beta: float = 2.0
    ) -> Dict[str, Any]:
        """
        ...
        post_precision_beta = (
            prior_beta
            + 0.5 * np.sum((data_array - sample_mean) ** 2)
            + (n_obs * prior_alpha)
            / (n_obs + prior_alpha)
            * (sample_mean - 0.0) ** 2
            / 2.0
        ...
        sampled_mean = np.random.normal(
            post_mean, 1.0 / np.sqrt(post_mean_precision * precision)
        )

        # Sample other parameters
        sampled_variance = 1.0 / precision if precision > 0 else sample_var
        sampled_shape = max(
            0.1, np.random.gamma(2.0, 1.0)
        )  # Shape parameter for lognormal
    ...
    def monte_carlo_simulation(
        ...
        parameter_factors = np.random.normal(
            param_samples["mean_rate"], np.sqrt(param_samples["variance"]), n_scenarios
        )

        # Apply lognormal simulation for multiplicative uncertainty
        uncertainty_factors = np.random.lognormal(
            mean=0.0, sigma=max(0.01, param_samples["shape"] * 0.1), size=n_scenarios
        )

        # Calculate premiums
        simulated_premiums = base_premium * parameter_factors * uncertainty_factors

        # Ensure premiums are positive
        simulated_premiums = np.maximum(0.0, simulated_premiums)

        return simulated_premiums.tolist()

    def calculate_percentiles(
        self, scenario_results: List[float], percentiles: List[float] = [10, 50, 90]
    ) -> Dict[float, float]:
        """
    ...
    def calculate_value_at_risk(
        self, scenario_results: List[float], confidence_level: float = 0.95
    ) -> float:
        """
    ...
    def calculate_conditional_value_at_risk(
        ...
        cvar_values = (
            sorted_results[-threshold_idx:] if threshold_idx > 0 else sorted_results
        )
    ...
    def bayesian_bootstrap_premium(
        self,
        ...
        cvar_value = self.calculate_conditional_value_at_risk(
    ...
    def calculate_contract_uncertainty_ranges(
        self, contracts_data: Dict[str, Dict[str, Any]]
        ...
        for contract_id, contract_info in contracts_data.items():
            ...
            result = self.bayesian_bootstrap_premium(
                ...
                exposure,
                n_scenarios,
                contract_info.get("confidence_level", 0.95),
                contract_id,
            )
...
def sample_posterior_parameters(
    data: List[float], prior_alpha: float = 2.0, prior_beta: float = 2.0
) -> Dict[str, Any]:
    """Sample parameters from posterior distribution using conjugate priors"""
...
def calculate_percentiles(
    scenario_results: List[float], percentiles: List[float] = [10, 50, 90]
) -> Dict[float, float]:
    """Calculate percentiles from Monte Carlo results"""
    return bayesian_bootstrap_service.calculate_percentiles(
        scenario_results, percentiles
    )


def calculate_value_at_risk(
    scenario_results: List[float], confidence_level: float = 0.95
) -> float:
    """Calculate Value at Risk (VaR) for the contract"""
    return bayesian_bootstrap_service.calculate_value_at_risk(
        scenario_results, confidence_level
    )
