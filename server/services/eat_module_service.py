...
class CommercialPricingEngine:
    ...
    def calculate_commercial_pricing(self, risk_adjusted_premium: RiskAdjustedPremium, 
                                   market_data: MarketData,
                                   pricing_strategy: PricingStrategy = PricingStrategy.MARKET_MATCHING,
                                   target_volume: Optional[float] = None) -> CommercialPricingResult:
        """
        ...
        competition_factor, competitor_analysis = self._calculate_competition_adjustment(
        ...
        rationale = self._generate_pricing_rationale(
            pricing_strategy, market_factor, competition_factor, 
            profitability_factor, final_premium, base_premium
        )
    ...
    def _calculate_competition_adjustment(self, competitor_rates: Dict[str, float], 
                                        market_average: float) -> Tuple[float, Dict[str, float]]:
        """Calculate competition-based adjustments"""
        if not competitor_rates:
            return 0.05, {'market_position': 'n/a', 'avg_competitor_rate': market_average}
        ...
        competitive_pressure = min(0.1, rate_variance / np.mean(competitor_values) if np.mean(competitor_values) > 0 else 0)
    ...
    def _calculate_profitability_factor(self, base_premium: float, 
                                      economic_indicators: Dict[str, float]) -> Tuple[float, float]:
        ...
        profitability_factor = required_profit / base_premium if base_premium > 0 else 0.15
        
        return profitability_factor, adjusted_target_margin

    def _calculate_break_even_point(self, base_premium: float, market_data: MarketData) -> float:
    ...
    def _calculate_elasticity_impact(self, final_premium: float, base_premium: float, 
                                   target_volume: Optional[float] = None) -> float:
    ...
    def _determine_market_position(self, final_premium: float, market_average: float) -> str:
    ...
    def _generate_pricing_rationale(self, pricing_strategy: PricingStrategy, 
                                  market_factor: float, competition_factor: float,
                                  profitability_factor: float, final_premium: float,
                                  base_premium: float) -> List[str]:
    ...
    def optimize_pricing_for_volume(self, risk_adjusted_premium: RiskAdjustedPremium,
        ...
        def objective_function(discount_factor):
            ...
            elasticity = self.elasticity_factors['price_elasticity']
            volume_multiplier = 1 + (1 - price_ratio) * abs(elasticity)  # Lower price, higher volume
            
            # Calculate profit per policy
            profit_per_policy = discounted_premium - (base_premium * 0.7)  # Simplified cost structure
            
            # Objective: maximize revenue (volume * price)
            if profit_per_policy > 0:
                return -(volume_multiplier * discounted_premium)  # Negative for minimization
            else:
                return float('inf')  # Invalid if unprofitable

        # Optimize discount (between 0 and 30%)
        result = minimize(objective_function, x0=[0.1], bounds=[(0.0, 0.3)], method='L-BFGS-B')
    ...
    def calculate_dynamic_pricing_factors(self, customer_profile: Dict[str, Any],
                                        policy_features: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        coverage_factor = max(0.0, (coverage_limit - 100000) / 1000000 * 0.05)  # 5% per million above 100k
        factors['coverage_factor'] = coverage_factor
        
        # Deductible incentive (higher deductible = lower premium)
        deductible_factor = -(min(0.15, (deductible - 500) / 5000 * 0.15))  # Up to 15% discount
        factors['deductible_factor'] = deductible_factor
        
        # Term length factor
        term_factor = 0.02 * (policy_term - 1) if policy_term > 1 else 0  # Multi-year discount
        factors['term_factor'] = term_factor
        
        return factors

    def get_pricing_strategy_recommendation(self, market_data: MarketData,
                                          portfolio_metrics: Dict[str, float]) -> PricingStrategy:
        """
        ...
        market_concentration = len(market_data.competitor_rates) if market_data.competitor_rates else 10
        ...
        elif market_share < 0.03 and market_growth > 0.05:  # Small player in growing market
...
# Global instance
epc_engine = CommercialPricingEngine()

def calculate_commercial_pricing(risk_adjusted_premium: RiskAdjustedPremium, 
                               market_data: MarketData,
                               pricing_strategy: PricingStrategy = PricingStrategy.MARKET_MATCHING) -> CommercialPricingResult:
    """Convenience function to calculate commercial pricing"""
    return epc_engine.calculate_commercial_pricing(risk_adjusted_premium, market_data, pricing_strategy)

def optimize_pricing_for_volume(risk_adjusted_premium: RiskAdjustedPremium,
                              market_data: MarketData) -> float:
    """Convenience function to optimize pricing for volume"""
    return epc_engine.optimize_pricing_for_volume(risk_adjusted_premium, market_data)

def calculate_dynamic_pricing_factors(customer_profile: Dict[str, Any],
                                    policy_features: Dict[str, Any]) -> Dict[str, float]:
    """Convenience function to calculate dynamic pricing factors"""
    return epc_engine.calculate_dynamic_pricing_factors(customer_profile, policy_features)

def get_pricing_strategy_recommendation(market_data: MarketData,
                                      portfolio_metrics: Dict[str, float]) -> PricingStrategy:
    """Convenience function to get pricing strategy recommendation"""
    return epc_engine.get_pricing_strategy_recommendation(market_data, portfolio_metrics)