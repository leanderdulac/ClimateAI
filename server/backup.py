...
class BackupConfig:
    ...
    # Database
    DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://user:pass@localhost:5432/climateai")
...
class DatabaseBackup:
    ...
    def create_backup(self) -> Path:
        ...
        try:
            ...
            self.logger.info(f"Backup criado: {backup_file} ({backup_size / 1024 / 1024:.2f} MB)")
    ...
    def compress_backup(self, backup_file: Path) -> Path:
        ...
        try:
            ...
            self.logger.info(f"Compressão: {compressed_size / 1024 / 1024:.2f} MB (ratio: {compression_ratio:.1f}%)")
        ...
        try:
            ...
            if backup_file.suffix == '.gz':
                with gzip.open(backup_file, 'rb') as f:
                    data = f.read()
                self.logger.info(f"Arquivo gzip válido ({len(data) / 1024 / 1024:.2f} MB)")
                return True
...
class BackupStorage:
    ...
    def upload_to_s3(self, backup_file: Path) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Fazendo upload para S3: s3://{BackupConfig.S3_BUCKET}/{key}")
    ...
    def upload_to_gcs(self, backup_file: Path) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Fazendo upload para GCS: gs://{BackupConfig.GCS_BUCKET}/{backup_file.name}")
...
class BackupNotification:
    ...
    def send_slack(self, status: str, message: str, backup_file: Optional[Path] = None) -> bool:
...
class BackupOrchestrator:
    ...
    def run_backup(self) -> bool:
        ...
        try:
            ...
            if not backup_file:
                self.notification.send_slack("failure", "Falha ao criar backup do banco de dados")
                return False
            
            # 2. Verificar integridade
            if not self.db_backup.verify_backup(backup_file):
                self.notification.send_slack("failure", "Falha na verificação de integridade do backup")
                return False
            ...
            self.notification.send_slack("success", "Backup completado com sucesso", backup_file)
...
if __name__ == "__main__":
    ...
    restore_parser.add_argument("--database", default="climateai_restored", help="Nome do banco de dados destino")
